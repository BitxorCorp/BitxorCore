...
namespace bitxorcore { namespace io {
	...
	namespace {
		...
		// region platform-independent bxor file io wrappers

		FileOperationResult<bool> bxorClose(int fd) {
		...
		FileOperationResult<size_t> bxorWrite(int fd, const RawBuffer& data) {
			return ProcessInBlocks(write, Write_Error, fd, data);
		}

		FileOperationResult<size_t> bxorRead(int fd, const MutableRawBuffer& data) {
			return ProcessInBlocks(read, Read_Error, fd, data);
		}

		FileOperationResult<bool> bxorSeekSet(int fd, int64_t offset) {
			return -1 == lseek(fd, offset, SEEK_SET) ? MakeFailureResult(false) : MakeSuccessResult(true);
		}

		FileOperationResult<bool> bxorTruncate(int fd, int64_t offset) {
			return -1 == ftruncate(fd, offset) ? MakeFailureResult(false) : MakeSuccessResult(true);
		}

		FileOperationResult<bool> bxorFileSize(int fd, uint64_t& fileSize) {
			StatStruct st;
			fileSize = 0;
			if (0 != fstat(fd, &st))
		...
		FileOperationResult<int> bxorOpen(int& fd, const char* name, OpenMode mode, LockMode lockMode) {
	...
	RawFile::FileDescriptorHolder::~FileDescriptorHolder() {
		...
		auto closeResult = bxorClose(m_fd);
	...
	int RawFile::FileDescriptorHolder::raw() const {
		if (!isValid())
			BITXORCORE_THROW_FILE_IO_ERROR(Error_Desc);
...
#define BITXORCORE_CHECK_FILE_OPERATION_RESULT(MESSAGE, OPERATION_RESULT) \
	do { \
		if (!OPERATION_RESULT.IsSuccess) { \
			BITXORCORE_LOG(error) \
					<< MESSAGE << " " << m_pathname << ": " \
					<< OPERATION_RESULT.Message << " (" << OPERATION_RESULT.ErrorCode << ")"; \
			BITXORCORE_THROW_FILE_IO_ERROR(MESSAGE); \
		} \
	} while (false)
	...
	RawFile::RawFile(const std::string& pathname, OpenMode mode, LockMode lockMode)
			...
			, m_fileSize(0)
			, m_position(0) {
		auto openResult = bxorOpen(m_fd.rawRef(), m_pathname.c_str(), mode, lockMode);
		BITXORCORE_CHECK_FILE_OPERATION_RESULT(Error_Open, openResult);

		auto fileSizeResult = bxorFileSize(m_fd.raw(), m_fileSize);
		BITXORCORE_CHECK_FILE_OPERATION_RESULT(Error_Size, fileSizeResult);
	}
	...
	void RawFile::read(const MutableRawBuffer& dataBuffer) {
		auto readResult = bxorRead(m_fd.raw(), dataBuffer);
		BITXORCORE_CHECK_FILE_OPERATION_RESULT(Error_Read, readResult);

		m_position += readResult.Value;
	}

	void RawFile::write(const RawBuffer& dataBuffer) {
		auto writeResult = bxorWrite(m_fd.raw(), dataBuffer);
		BITXORCORE_CHECK_FILE_OPERATION_RESULT(Error_Write, writeResult);
	...
	void RawFile::seek(uint64_t position) {
		...
		if (position > size()) {
			BITXORCORE_LOG(error) << Error_Seek_Outside << " " << m_pathname;
			BITXORCORE_THROW_FILE_IO_ERROR(Error_Seek_Outside);
		}

		auto seekSetResult = bxorSeekSet(m_fd.raw(), static_cast<int64_t>(position));
		BITXORCORE_CHECK_FILE_OPERATION_RESULT(Error_Seek, seekSetResult);

		m_position = position;
	}

	void RawFile::truncate() {
		auto truncateResult = bxorTruncate(m_fd.raw(), static_cast<int64_t>(m_position));
		BITXORCORE_CHECK_FILE_OPERATION_RESULT(Error_Truncate, truncateResult);
