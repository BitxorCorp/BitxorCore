...
namespace bitxorcore { namespace io {
	...
	public:
		...
		/// Throws bitxorcore_file_io_error exception if requested amount of data could not be read.
		void read(const MutableRawBuffer& dataBuffer);

		/// Writes data pointed to by \a dataBuffer to the file.
		/// If proper amount of data could not be written bitxorcore_file_io_error exception will be thrown.
		void write(const RawBuffer& dataBuffer);

		/// Seeks to given absolute position.
		/// Throws bitxorcore_file_io_error exception if seek has failed.
