...
namespace bitxorcore { namespace io {
		...
		public:
			...
			void read(const MutableRawBuffer& buffer) override {
				if (buffer.Size + m_pStream->position() > m_endPosition) {
					...
					BITXORCORE_THROW_FILE_IO_ERROR(out.str().c_str());
				}
	...
	FileDatabase::FileDatabase(const config::BitxorCoreDirectory& directory, const Options& options)
			: m_directory(directory)
			, m_options(options) {
		if (0 == m_options.BatchSize)
			BITXORCORE_THROW_INVALID_ARGUMENT("batch size must be nonzero");
	}
	...
	std::string FileDatabase::getFilePath(uint64_t id, bool createDirectories) const {
		...
		auto storageDirectory = config::BitxorCoreDataDirectory(m_directory.path()).storageDir(groupId);

		if (createDirectories)
			config::BitxorCoreDirectory(storageDirectory.str()).create();
