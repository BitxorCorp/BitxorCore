...
namespace bitxorcore { namespace io {
	...
	std::unique_ptr<RawFile> FixedSizeValueStorage<TKey, TValue>::openStorageFile(TKey key, OpenMode openMode) const {
		auto storageDir = config::BitxorCoreStorageDirectoryPreparer::Prepare(m_dataDirectory, key);
		...
		if (key.unwrap() < Files_Per_Storage_Directory && sizeof(TValue) * 2 > pStorageFile->size())
			BITXORCORE_THROW_RUNTIME_ERROR_2("storage file has invalid size", filePath, pStorageFile->size());
