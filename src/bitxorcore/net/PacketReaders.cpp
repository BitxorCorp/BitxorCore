	...
	namespace {
		...
		public:
			InsertOperation insert(
				...
				if (m_readers.end() == insertedReaderIter) {
					...
					BITXORCORE_LOG(warning) << "rejecting incoming connection from " << identity << " (max connections in use)";
					pSocket->close();
				...
				operation.Abort = [this, qualifiedReaderId]() {
					BITXORCORE_LOG(warning) << "aborting incoming connection from " << qualifiedReaderId.first;
		...
		private:
			...
			bool closeSingle(const model::NodeIdentity& identity, uint32_t id) {
				...
				BITXORCORE_LOG(debug) << "closing connection to " << identity << " - " << id;
		...
		public:
				...
				m_pClientConnector->accept(socketInfo, [pThis = shared_from_this(), host = socketInfo.host(), callback](
					...
					if (operation.IsPending) {
						if (callback(connectResult)) {
							BITXORCORE_LOG(debug) << "accepted connection from '" << verifiedSocketInfo.host() << "' as " << identityKey;
							operation.Commit();
							return;
						}
					} else {
			...
			void shutdown() override {
				BITXORCORE_LOG(info) << "closing all connections in PacketReaders";
