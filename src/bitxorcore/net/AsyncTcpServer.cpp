	...
	namespace {
		...
		void BindAcceptor(
			...
			if (allowAddressReuse) {
				BITXORCORE_LOG(info) << "configuring AsyncTcpServer to reuse addresses";
		...
		public:
			DefaultAsyncTcpServer(
				...
				BITXORCORE_LOG(info) << "AsyncTcpServer created around " << endpoint;
			}
		...
		public:
			void start() {
				...
				BITXORCORE_LOG(trace) << "AsyncTcpServer waiting for threads to enter pending accept state";
				while (!m_hasPendingAccept) {}
				BITXORCORE_LOG(info) << "AsyncTcpServer spawned pending accept";
			}
			...
			void shutdown() override {
				...
				BITXORCORE_LOG(info) << "AsyncTcpServer stopping";
				boost::asio::dispatch(m_acceptorStrand, [pThis = shared_from_this()]() {
					pThis->closeAcceptor();
				});

				while (m_hasPendingAccept) {}
				BITXORCORE_LOG(info) << "AsyncTcpServer stopped";
			}
		...
		private:
			...
			void tryStartAccept() {
				if (m_isStopped) {
					BITXORCORE_LOG(trace) << "bypassing Accept because server is stopping";
					return;
				}
				...
				if (numOpenConnectionSlots <= 0) {
					BITXORCORE_LOG(debug) << "bypassing Accept due to limit (numOpenConnectionSlots=" << numOpenConnectionSlots << ")";
					return;
				}

				if (m_hasPendingAccept) {
					BITXORCORE_LOG(trace) << "bypassing Accept due to current outstanding accept";
					return;
				}
