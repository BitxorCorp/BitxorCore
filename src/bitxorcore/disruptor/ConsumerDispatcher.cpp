...
namespace bitxorcore { namespace disruptor {
	...
	namespace {
		const ConsumerDispatcherOptions& CheckOptions(const ConsumerDispatcherOptions& options) {
			if (!options.DispatcherName || 0 == options.DisruptorSlotCount || utils::FileSize() == options.DisruptorMaxMemorySize)
				BITXORCORE_THROW_INVALID_ARGUMENT("consumer dispatcher options are invalid");
	...
	ConsumerDispatcher::ConsumerDispatcher(
		...
		BITXORCORE_LOG(info) << m_options.DispatcherName << " ConsumerDispatcher spawned " << m_threads.size() << " workers";
	}
	...
	bool ConsumerDispatcher::canProcessNextElement() const {
		...
		BITXORCORE_LOG(warning) << "disruptor is full (minPosition = " << minPosition << ", maxPosition = " << maxPosition << ")";
	...
	DisruptorElementId ConsumerDispatcher::processElement(ConsumerInput&& input, const ProcessingCompleteFunc& processingComplete) {
		if (input.empty()) {
			BITXORCORE_LOG(trace) << "dispatcher is ignoring empty input (" << input << ")";
			return 0;
		}
		...
		if (isFull) {
			if (m_options.ShouldThrowWhenFull)
				BITXORCORE_THROW_RUNTIME_ERROR("consumer is too far behind");
