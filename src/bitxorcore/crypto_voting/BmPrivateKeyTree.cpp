...
namespace bitxorcore { namespace crypto {
	...
	BmTreeSignature BmPrivateKeyTree::sign(const BmKeyIdentifier& keyIdentifier, const RawBuffer& dataBuffer) {
		BITXORCORE_LOG(trace) << "signing with key identifier " << keyIdentifier;

		if (!canSign(keyIdentifier))
			BITXORCORE_THROW_INVALID_ARGUMENT_1("sign called with invalid key identifier", keyIdentifier);
	...
	void BmPrivateKeyTree::wipe(const BmKeyIdentifier& keyIdentifier) {
		BITXORCORE_LOG(trace) << "wiping key identifier " << keyIdentifier;

		if (!check(keyIdentifier, m_lastWipeKeyIdentifier))
			BITXORCORE_THROW_INVALID_ARGUMENT_1("wipe called with invalid key identifier", keyIdentifier);
	...
	bool BmPrivateKeyTree::check(const BmKeyIdentifier& keyIdentifier, const BmKeyIdentifier& referenceKeyIdentifier) const {
		if (keyIdentifier < referenceKeyIdentifier) {
			BITXORCORE_LOG(warning) << "rejecting key identifier " << keyIdentifier << " less than reference " << referenceKeyIdentifier;
			return false;
		}
