...
namespace bitxorcore { namespace plugins {
	...
	cache::BitxorCoreCache PluginManager::createCache() {
	...
	void PluginManager::addHandlers(ionet::ServerPacketHandlers& handlers, const cache::BitxorCoreCache& cache) const {
	...
	void PluginManager::addDiagnosticHandlers(ionet::ServerPacketHandlers& handlers, const cache::BitxorCoreCache& cache) const {
		ApplyAll(handlers, m_diagnosticHandlerHooks, cache);
	}

	void PluginManager::addDiagnosticCounters(std::vector<utils::DiagnosticCounter>& counters, const cache::BitxorCoreCache& cache) const {
	...
	void PluginManager::addTokenResolver(const TokenResolver& resolver) {
		m_tokenResolvers.push_back(resolver);
	}
	...
	model::ResolverContext PluginManager::createResolverContext(const cache::ReadOnlyBitxorCoreCache& cache) const {
		auto tokenResolver = BuildResolver<TokenId>(m_tokenResolvers);
		...
		return model::ResolverContext(bindResolverToCache(tokenResolver), bindResolverToCache(addressResolver));
	}
	...
	PluginManager::PublisherPointer PluginManager::createNotificationPublisher(model::PublicationMode mode) const {
		return model::CreateNotificationPublisher(
				...
				model::GetUnresolvedCurrencyTokenId(m_config),
