...
namespace bitxorcore { namespace api {
	...
	namespace {
		bitxorcore_api_error CreateHeightException(const char* message, Height height) {
			return bitxorcore_api_error(message) << exception_detail::Make<Height>::From(height);
		}
