...
#include "bitxorcore/thread/StrandOwnerLifetimeExtender.h"
#include "bitxorcore/thread/TimedCallback.h"
#include "bitxorcore/utils/StackTimer.h"
...
namespace bitxorcore { namespace ionet {
	...
	namespace {
		...
		SocketOperationCode mapReadErrorCodeToSocketOperationCode(const boost::system::error_code& ec) {
			...
			if (isEof)
				BITXORCORE_LOG(info) << "eof reading from socket: " << ec.message();
			else
				BITXORCORE_LOG(error) << "failed when reading from socket: " << ec.message();
		...
		SocketOperationCode mapWriteErrorCodeToSocketOperationCode(const boost::system::error_code& ec) {
			...
			BITXORCORE_LOG(error) << "failed when writing to socket: " << ec.message();
			...
			void close() {
				...
				m_socket.async_shutdown(wrap([](const auto& ec) {
					if (ec && boost::asio::error::operation_aborted != ec && boost::asio::error::bad_descriptor != ec)
						BITXORCORE_LOG(warning) << "async_shutdown returned an error: " << ec.message();
				}));
				...
				boost::asio::async_write(m_socket, asioBuffer, wrap([this](const auto& ec, auto) {
					if (ec && !IsProtocolShutdown(ec))
						BITXORCORE_LOG(warning) << "async_write returned an error: " << ec.message();
		...
		public:
			void write(const PacketPayload& payload, const PacketSocket::WriteCallback& callback) {
				if (!IsPacketDataSizeValid(payload.header(), m_maxPacketDataSize)) {
					BITXORCORE_LOG(warning) << "bypassing write of malformed " << payload.header();
		...
		private:
			...
			void checkAndHandleError(PacketExtractResult extractResult, const PacketSocket::ReadCallback& callback, bool allowMultiple) {
				...
				BITXORCORE_LOG(error) << "failed processing malformed packet: " << extractResult;
		...
		public:
			...
			~StrandedPacketSocket() override {
				...
				BITXORCORE_LOG(debug) << "socket close triggered by destruction " << m_id;
				m_socket.close();
			}
		...
		private:
			...
			void postCloseOnce(const char* operationName, THandler handler) {
				post([&id = m_id, operationName, handler](auto& socket) {
					...
					BITXORCORE_LOG(debug) << "socket " << operationName << " triggered by owner " << id;
					handler(socket);
				});
			}
	...
	namespace {
		...
		private:
			void handleAccept(const boost::system::error_code& ec, NetworkSocket&& socket) {
				if (ec) {
					BITXORCORE_LOG(warning) << "async_accept returned an error: " << ec.message();
				...
				if (remoteEndpointEc) {
					BITXORCORE_LOG(warning) << "unable to determine remote endpoint: " << remoteEndpointEc;
					return m_accept(PacketSocketInfo());
				}
				...
				pTimedCallback->setTimeoutHandler([pSocket = m_pSocket]() {
					...
					boost::asio::post(pSocket->strand(), [pSocket]() {
						BITXORCORE_LOG(debug) << "accept handshake failed due to timeout " << pSocket->id();
					});
				});
			...
			void handleHandshake(const boost::system::error_code& ec) {
				if (ec) {
					BITXORCORE_LOG(warning) << "async_handshake returned an error: " << ec.message();
					return m_accept(PacketSocketInfo());
				}

				BITXORCORE_LOG(debug) << "invoking user callback after successful async_accept " << m_pSocket->id();
				m_pSocket->markOpen();
...
#pragma warning(pop)
		...
		private:
			...
			void handleHandshake(const boost::system::error_code& ec) {
				...
				BITXORCORE_LOG(info) << "connected to " << m_host << " [" << m_endpoint << "] " << m_pSocket->id();
