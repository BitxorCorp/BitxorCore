...
namespace bitxorcore { namespace ionet { struct NodeData; } }
...
namespace bitxorcore { namespace ionet {
	...
	const NodeInfo& NodeContainerView::getNodeInfo(const model::NodeIdentity& identity) const {
		...
		if (!pNodeData)
			BITXORCORE_THROW_INVALID_ARGUMENT_1("cannot get node info for unknown identity", identity);
	...
	bool NodeContainerModifier::add(const Node& node, NodeSource source) {
		...
		if (NodeVersion() != nodeVersion && !m_nodeContainerData.VersionPredicate(nodeVersion)) {
			BITXORCORE_LOG(warning) << "rejecting node " << node.identity() << " with version " << utils::HexFormat(nodeVersion);
			return false;
		}

		auto& nodeDataContainer = m_nodeContainerData.NodeDataContainer;
		if (m_bannedNodes.isBanned(node.identity())) {
			BITXORCORE_LOG(warning) << "rejecting banned node " << node.identity();
			return false;
		}
	...
	ConnectionState& NodeContainerModifier::provisionConnectionState(ServiceIdentifier serviceId, const model::NodeIdentity& identity) {
		...
		if (!pNodeData)
			BITXORCORE_THROW_INVALID_ARGUMENT_1("cannot provision connection state for unknown node", identity);
