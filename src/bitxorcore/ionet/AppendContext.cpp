...
namespace bitxorcore { namespace ionet {
	...
	void AppendContext::commit(size_t size) {
		...
		if (size > m_appendSize)
			BITXORCORE_THROW_RUNTIME_ERROR_2("cannot commit more than reserved append size", size, m_appendSize);
	...
	void AppendContext::assertNotCommitted() const {
		if (m_isCommitted)
			BITXORCORE_THROW_RUNTIME_ERROR("append context was already committed");
	}
}}
