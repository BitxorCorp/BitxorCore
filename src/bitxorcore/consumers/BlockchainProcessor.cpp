	...
	namespace {
		...
		public:
			ValidationResult operator()(
				...
				for (auto& element : elements) {
					...
					if (!IsValidationResultSuccess(result)) {
						BITXORCORE_LOG(warning) << "batch processing of block " << block.Height << " failed with " << result;
						return result;
					}
		...
		private:
			...
			static validators::ValidationResult CheckGenerationHash(
					...
					const cache::ReadOnlyBitxorCoreCache& readOnlyCache) {
				const auto& block = element.Block;
				...
				if (Hash512() == vrfVerifyResult) {
					BITXORCORE_LOG(warning) << "vrf proof does not validate at height " << block.Height;
				...
				if (!blockHitPredicate(parentBlock, block, element.GenerationHash)) {
					BITXORCORE_LOG(warning) << "block " << block.Height << " failed hit";
			...
			static bool CheckReceiptsHash(
				...
				if (block.ReceiptsHash != calculatedReceiptsHash) {
					...
					if (element.OptionalStatement)
						BITXORCORE_LOG(info) << FormatBlockStatementLog(*element.OptionalStatement);
