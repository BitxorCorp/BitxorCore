...
#include "bitxorcore/validators/AggregateValidationResult.h"
...
namespace bitxorcore { namespace consumers {
	...
	namespace {
		...
		auto MakeValidationConsumer(TExtractAndProcess extractAndProcess) {
			return [extractAndProcess](auto& elements) {
				...
				BITXORCORE_LOG_LEVEL(validators::MapToLogLevel(result)) << "validation consumer failed: " << result;
	...
	disruptor::TransactionConsumer MakeTransactionValidationConsumer(
		...
		return MakeValidationConsumer([failedTransactionSink, process](auto& elements) {
			...
			BITXORCORE_LOG(trace) << "all " << numSkippedElements << " transaction(s) skipped in Transaction validation consumer";
