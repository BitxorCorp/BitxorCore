...
namespace bitxorcore { namespace consumers {
	...
	namespace {
		...
		public:
			...
			SyncState(cache::BitxorCoreCache& cache, Height localFinalizedHeight, Timestamp localFinalizedTime)
					: m_pOriginalCache(&cache)
					, m_pCacheDelta(std::make_unique<cache::BitxorCoreCacheDelta>(cache.createDelta()))
		...
		public:
			...
			const cache::BitxorCoreCacheDelta& cacheDelta() const {
		...
		private:
			cache::BitxorCoreCache* m_pOriginalCache;
			std::unique_ptr<cache::BitxorCoreCacheDelta> m_pCacheDelta; // unique_ptr to allow explicit release of lock in commit
		...
		private:
			...
			ConsumerResult preprocess(const BlockElements& elements, InputSource source, SyncState& syncState) const {
				...
				if (peerScore <= localScore && !isFinalizedBlockUpdate) {
					BITXORCORE_LOG(warning) << "peer score (" << peerScore << ") is not better than local score (" << localScore << ")";
			...
			ConsumerResult process(BlockElements& elements, SyncState& syncState) const {
				...
				if (!validators::IsValidationResultSuccess(processResult)) {
					BITXORCORE_LOG(warning) << "processing of peer chain failed with " << processResult;
		...
		private:
			...
			static constexpr bool IsLinked(Height peerStartHeight, Height localChainHeight, InputSource source) {
				// peer should never return genesis block
