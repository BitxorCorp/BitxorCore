...
namespace bitxorcore { namespace config {
	...
	BitxorCoreKeys::BitxorCoreKeys() : m_nodeKeyPair(crypto::KeyPair::FromPrivate(crypto::PrivateKey()))
	{}

	BitxorCoreKeys::BitxorCoreKeys(const std::string& directory)
	...
	BitxorCoreKeys::BitxorCoreKeys(Key&& caPublicKey, crypto::KeyPair&& nodeKeyPair)
			: m_caPublicKey(std::move(caPublicKey))
			, m_nodeKeyPair(std::move(nodeKeyPair))
	{}

	const Key& BitxorCoreKeys::caPublicKey() const {
		return m_caPublicKey;
	}

	const crypto::KeyPair& BitxorCoreKeys::nodeKeyPair() const {
