...
namespace bitxorcore { namespace cache {
	...
	public:
		...
		auto statistics(Height height, size_t count) const {
			if (m_set.empty()) {
				// note: this should not happen since the genesis block is available from the beginning
				BITXORCORE_THROW_RUNTIME_ERROR("block statistic cache is empty");
			}

			if (Height(0) == height || 0 == count)
				BITXORCORE_THROW_INVALID_ARGUMENT("specified height or count out of range");
			...
			if (iterableStatistics.end() == lastIter)
				BITXORCORE_THROW_INVALID_ARGUMENT_1("element with specified height not found", height);
