...
namespace bitxorcore { namespace cache {
	...
	TokenId BasicAccountStateCacheDelta::harvestingTokenId() const {
	...
	namespace {
		...
		TIterator PrepareMutableIterator(TIterator&& iter, const AccountStateCacheTypes::Options& options) {
			// in order to guarantee deterministic sorting of tokens, CurrencyTokenId always needs to be set as the optimized token id
			// before the iterator is returned because that information is lost during serialization when the AccountState doesn't
			// contain any CurrencyTokenId balance
			if (iter.tryGet())
				iter.get().Balances.optimize(options.CurrencyTokenId);
	...
	void BasicAccountStateCacheDelta::addAccount(const state::AccountState& accountState) {
		...
		m_pStateByAddress->find(accountState.Address).get()->Balances.optimize(m_options.CurrencyTokenId);
	}
