...
namespace bitxorcore { namespace config { class BitxorCoreKeys; } }
...
namespace bitxorcore { namespace extensions {
	...
	public:
		...
		explicit ServiceLocator(const config::BitxorCoreKeys& keys) : m_keys(keys)
		{}

		/// Destroys the locator.
		~ServiceLocator() {
			BITXORCORE_LOG(info) << "destroying " << m_rootedServices.size() << " rooted services";

			// destroy rooted services in reverse order of registration
			auto i = 1u;
			while (!m_rootedServices.empty()) {
				BITXORCORE_LOG(debug) << i << ": destroying rooted service " << m_rootedServices.back().first;
	...
	public:
		/// Gets the local keys.
		const config::BitxorCoreKeys& keys() const {
			return m_keys;
		}
		...
		std::shared_ptr<TService> service(const std::string& serviceName) const {
			...
			if (!tryGetService(serviceName, pService))
				BITXORCORE_THROW_INVALID_ARGUMENT_1("requested service is not registered", serviceName);
	...
	public:
		...
		void registerService(const std::string& serviceName, const std::shared_ptr<void>& pService) {
			if (!m_services.emplace(serviceName, pService).second)
				BITXORCORE_THROW_INVALID_ARGUMENT_1("service is already registered", serviceName);
		}
	...
	private:
		const config::BitxorCoreKeys& m_keys;
