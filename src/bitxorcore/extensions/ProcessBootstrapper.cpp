...
namespace bitxorcore { namespace extensions {
	...
	void ForceSymbolInjection() {
		BITXORCORE_LOG(debug) << "forcibly injecting symbol: " << typeid(T).name() << " => " << typeid(T).hash_code();
	}
#endif
	...
	const config::BitxorCoreConfiguration& ProcessBootstrapper::config() const {
		return m_config;
	}
	...
	void ProcessBootstrapper::loadExtensions() {
		for (const auto& extension : m_config.Extensions.Names) {
			...
			BITXORCORE_LOG(info) << "registering dynamic extension " << extension;
