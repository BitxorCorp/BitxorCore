	...
	namespace {
		...
		AccountState LoadAccountStateWithoutHistory(io::InputStream& input, ImportanceReader& importanceReader) {
			...
			if (AccountStateFormat::High_Value == format) {
				...
				BITXORCORE_THROW_INVALID_ARGUMENT_1("cannot load account state with unsupported format", static_cast<uint16_t>(format));
			}

			// read tokens
			auto numTokens = io::Read16(input);
			for (auto i = 0u; i < numTokens; ++i) {
				auto tokenId = io::Read<TokenId>(input);
				auto amount = io::Read<Amount>(input);
				accountState.Balances.credit(tokenId, amount);

				if (0 == i)
					accountState.Balances.optimize(tokenId);
			}
