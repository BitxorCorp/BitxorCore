...
namespace bitxorcore { namespace utils {
	...
	DiagnosticCounterId::DiagnosticCounterId(const std::string& name) : m_name(name) {
		if (Max_Counter_Name_Size < m_name.size())
			BITXORCORE_THROW_INVALID_ARGUMENT_1("counter name is too long", m_name);

		if (!m_name.empty() && (' ' == m_name.front() || ' ' == m_name.back()))
			BITXORCORE_THROW_INVALID_ARGUMENT_1("counter cannot contain leading or trailing whitespace", m_name);
		...
		for (auto ch : m_name) {
			...
			if ('A' <= ch && ch <= 'Z')
				...
				BITXORCORE_THROW_INVALID_ARGUMENT_1("counter name contains invalid char", ch);
		}
	}
	...
	DiagnosticCounterId::DiagnosticCounterId(uint64_t value) : m_value(value) {
		...
		if (0 != value)
			BITXORCORE_THROW_INVALID_ARGUMENT_1("counter value is too large", m_value);
	}
}}
