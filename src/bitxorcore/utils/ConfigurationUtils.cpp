...
namespace bitxorcore { namespace utils {
	...
	std::string GetIniPropertyName(const char* cppVariableName) {
		if (!cppVariableName || strlen(cppVariableName) < 2)
			BITXORCORE_THROW_INVALID_ARGUMENT("cpp variable name must be at least two characters");

		auto firstChar = cppVariableName[0];
		if (!std::isalpha(firstChar))
			BITXORCORE_THROW_INVALID_ARGUMENT("cpp variable name must start with a letter");
	...
	void VerifyBagSizeExact(const ConfigurationBag& bag, size_t expectedSize) {
		...
		BITXORCORE_THROW_INVALID_ARGUMENT_2(Error_Message, expectedSize, bag.size());
	}
