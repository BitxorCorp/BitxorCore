...
namespace bitxorcore { namespace utils {
	...
	class PLUGIN_API_EXCEPTION property_not_found_error : public bitxorcore_invalid_argument {
	public:
		using bitxorcore_invalid_argument::bitxorcore_invalid_argument;
	};

	/// Exception class that is thrown when a configuration property is malformed.
	class PLUGIN_API_EXCEPTION property_malformed_error : public bitxorcore_runtime_error {
	public:
	...
	public:
		...
		bool tryGet(const ConfigurationKey& key, T& value) const {
			...
			if (!TryParseValue(*pUnparsedValue, value)) {
				...
				BITXORCORE_THROW_AND_LOG_2(property_malformed_error, message, std::string(key.Section), std::string(key.Name));
			}
		...
		T get(const ConfigurationKey& key) const {
			...
			if (!tryGet(key, value)) {
				...
				BITXORCORE_THROW_AND_LOG_2(property_not_found_error, message, std::string(key.Section), std::string(key.Name));
			}
