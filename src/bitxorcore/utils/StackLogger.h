...
namespace bitxorcore { namespace utils {
	...
	public:
		...
		StackLogger(const char* message, LogLevel level)
				: m_message(message)
				, m_level(level) {
			BITXORCORE_LOG_LEVEL(m_level) << "pushing scope '" << m_message << "'";
		}

		/// Destructor.
		~StackLogger() {
			auto elapsedMillis = m_timer.millis();
			BITXORCORE_LOG_LEVEL(m_level) << "popping scope '" << m_message << "' (" << elapsedMillis << "ms)";
		}
	...
	public:
		...
		~SlowOperationLogger() {
			...
			BITXORCORE_LOG_LEVEL(m_level) << out.str();
		}
