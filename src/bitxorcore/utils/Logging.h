...
namespace bitxorcore { namespace utils {
	...
	/// BitxorCore log levels.
	/// \note For nicer BITXORCORE_LOG construction, use severity_level names from boost::log::trivial.
	enum class LogLevel {
	...
	/// BitxorCore (console) log color modes.
	...
	namespace log {
		...
		/// BitxorCore logger type.
		class bitxorcore_logger :
			...
			BOOST_LOG_FORWARD_LOGGER_MEMBERS_TEMPLATE(bitxorcore_logger)
		};

		/// Make the bitxorcore logger the global, default logger.
		BOOST_LOG_INLINE_GLOBAL_LOGGER_DEFAULT(global_logger, bitxorcore::utils::log::bitxorcore_logger)
	}

	// endregion
}}

#define BITXORCORE_LOG_WITH_LOGGER_LEVEL_TAG(LOGGER, LEVEL, TAG) \
	BOOST_LOG_STREAM_WITH_PARAMS( \
		(LOGGER), \
		(::bitxorcore::utils::log::keywords::file = (::bitxorcore::utils::ExtractFilename(__FILE__))) \
		(::bitxorcore::utils::log::keywords::line = (static_cast<unsigned int>(__LINE__))) \
		(::bitxorcore::utils::log::keywords::subcomponent = (TAG)) \
		(::bitxorcore::utils::log::keywords::loglevel = (static_cast<::bitxorcore::utils::LogLevel>(LEVEL))))

/// Writes a log entry to \a LOGGER with \a LEVEL severity.
#define BITXORCORE_LOG_WITH_LOGGER_LEVEL(LOGGER, LEVEL) \
	BITXORCORE_LOG_WITH_LOGGER_LEVEL_TAG(LOGGER, LEVEL, (::bitxorcore::utils::ExtractDirectoryName(__FILE__)))

/// Writes a log entry to the default logger with \a LEVEL severity.
#define BITXORCORE_LOG_LEVEL(LEVEL) \
	BITXORCORE_LOG_WITH_LOGGER_LEVEL(::bitxorcore::utils::log::global_logger::get(), LEVEL)

/// Writes a log entry to the default logger with \a SEV severity.
#define BITXORCORE_LOG(SEV) \
	BITXORCORE_LOG_WITH_LOGGER_LEVEL( \
			::bitxorcore::utils::log::global_logger::get(), \
			(::bitxorcore::utils::LogLevel::SEV))
