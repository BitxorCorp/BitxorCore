...
namespace bitxorcore { namespace utils {
	...
	void Base32Encode(const RawBuffer& data, const MutableRawString& encodedData) {
		...
		case EncodeResult::OutputBufferTooSmall:
			BITXORCORE_THROW_RUNTIME_ERROR("base32 output buffer is too small");

		case EncodeResult::InvalidSize:
			BITXORCORE_THROW_RUNTIME_ERROR_1("byte array size is not multiple", Decoded_Block_Size);
		}
	}
	...
	void Base32Decode(const RawString& encodedData, const MutableRawBuffer& data) {
		...
		case DecodeResult::IllegalCharacter:
			BITXORCORE_THROW_RUNTIME_ERROR("illegal character in base32 string");

		case DecodeResult::OutputBufferTooSmall:
			BITXORCORE_THROW_RUNTIME_ERROR("base32 output buffer is too small");

		case DecodeResult::InvalidSize:
			BITXORCORE_THROW_RUNTIME_ERROR_1("string length is not multiple", Encoded_Block_Size);
		}
	}
}}
