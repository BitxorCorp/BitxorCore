...
namespace bitxorcore { namespace tree {
	...
	namespace {
		...
		auto DeserializePath(io::InputStream& input) {
			...
			if (1 == numNibbles % 2 && 0 != path.nibbleAt(path.size() - 1))
				BITXORCORE_THROW_RUNTIME_ERROR("filler nibble in path must be 0");
	...
	std::string PatriciaTreeSerializer::SerializeValue(const TreeNode& value) {
		if (!value.isLeaf() && !value.isBranch())
			BITXORCORE_THROW_INVALID_ARGUMENT("invalid node passed to serializer");
	...
	TreeNode PatriciaTreeSerializer::DeserializeValue(const RawBuffer& buffer) {
		...
		default:
			BITXORCORE_THROW_INVALID_ARGUMENT_1("invalid marker of serialized node", nodeMarker);
		}
	}
}}
