...
namespace bitxorcore { namespace tree {
	...
	void TreeNode::setPath(const TreeNodePath& path) {
		if (isLeaf())
			...
			BITXORCORE_THROW_RUNTIME_ERROR("cannot change path of empty node");
	}

	const LeafTreeNode& TreeNode::asLeafNode() const {
		if (!isLeaf())
			BITXORCORE_THROW_RUNTIME_ERROR("tree node is not a leaf node");
	...
	const BranchTreeNode& TreeNode::asBranchNode() const {
		if (!isBranch())
			BITXORCORE_THROW_RUNTIME_ERROR("tree node is not a branch node");
