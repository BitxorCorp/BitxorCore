...
namespace bitxorcore { namespace tree {
	...
	public:
		...
		BasePatriciaTree(TDataSource& dataSource, const Hash256& rootHash) : BasePatriciaTree(dataSource) {
			if (!m_tree.tryLoad(rootHash))
				BITXORCORE_THROW_RUNTIME_ERROR_1("unable to load tree with root hash", rootHash);
		}
	...
	public:
		...
		std::shared_ptr<DeltaType> rebase() {
			if (m_pWeakDelta.lock())
				BITXORCORE_THROW_RUNTIME_ERROR("only a single attached delta is allowed at a time");
	...
	public:
		...
		void commit() {
			...
			if (!pDelta)
				BITXORCORE_THROW_RUNTIME_ERROR("attempting to commit changes to a tree without any outstanding attached deltas");
