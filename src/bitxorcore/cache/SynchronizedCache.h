...
namespace bitxorcore { namespace cache {
	...
	public:
		...
		LockedCacheDelta<CacheDeltaType> createDelta() {
			...
			if (m_pWeakDeltaPair.lock())
				BITXORCORE_THROW_RUNTIME_ERROR("only a single attached delta of a cache is allowed at a time");
		...
		void commit() {
			...
			if (!pDeltaPair)
				BITXORCORE_THROW_RUNTIME_ERROR("attempting to commit changes to a cache without any outstanding attached deltas");
	...
	public:
		...
		void init(TArgs&&... args) {
			if (m_isInitCalled)
				BITXORCORE_THROW_RUNTIME_ERROR("init has already been called for cache");
