...
namespace bitxorcore { namespace model {
	...
	public:
		/// Creates a notification around \a sender, \a tokenId and \a amount.
		BasicBalanceNotification(const ResolvableAddress& sender, UnresolvedTokenId tokenId, Amount amount)
	...
	public:
		...
		/// Signature.
		const bitxorcore::Signature& Signature;
	...
	/// Notification of a required token.
	struct TokenRequiredNotification : public Notification {
	public:
		/// Matching notification type.
		static constexpr auto Notification_Type = Core_Token_Required_Notification;

	public:
		/// Creates a notification around \a owner, \a tokenId and optional \a propertyFlagMask.
		TokenRequiredNotification(const ResolvableAddress& owner, const ResolvableTokenId& tokenId, uint8_t propertyFlagMask = 0)
				: Notification(Notification_Type, sizeof(TokenRequiredNotification))
				, Owner(owner)
				, TokenId(tokenId)
	...
	public:
		/// Token owner (resolvable).
		ResolvableAddress Owner;

		/// Token id (resolvable).
		ResolvableTokenId TokenId;

		/// Mask of required property flags that must be set on the token.
