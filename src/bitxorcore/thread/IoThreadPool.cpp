		...
		public:
			void start() override {
				if (0 != m_numWorkerThreads)
					BITXORCORE_THROW_RUNTIME_ERROR_1("cannot restart running thread pool", m_numWorkerThreads);

				// spawn the number of configured threads
				BITXORCORE_LOG(trace) << "spawning threads" << m_tag;
				...
				BITXORCORE_LOG(trace) << "waiting for threads to be spawned" << m_tag;
				while (m_numWorkerThreads < m_numConfiguredWorkerThreads) {}
				BITXORCORE_LOG(info) << "spawned " << m_pContext->numThreads() << " workers" << m_tag;
			}
			...
			void join() override {
				...
				BITXORCORE_LOG(debug) << "waiting for " << m_numWorkerThreads << " thread pool threads to exit" << m_tag;
				m_pContext.reset();
				BITXORCORE_LOG(info) << "all thread pool threads exited" << m_tag;
			}

		private:
			void ioWorkerFunction() {
				BITXORCORE_LOG(trace) << "worker thread started" << m_tag;

				auto incrementDecrementGuard = utils::MakeIncrementDecrementGuard(m_numWorkerThreads);
				m_ioContext.run();

				BITXORCORE_LOG(trace) << "worker thread finished" << m_tag;
			}
