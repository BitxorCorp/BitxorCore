	...
	namespace {
		...
		public:
			BasicTaskWrapper(boost::asio::io_context& ioContext, const Task& task, TCallbackWrapper& wrapper)
					...
					, m_isStopped(false) {
				BITXORCORE_LOG(debug) << "task '" << m_task.Name << "' is scheduled in " << task.StartDelay;
			}
		...
		private:
			void startWait() {
				if (m_isStopped) {
					BITXORCORE_LOG(trace) << "bypassing start of stopped timer";
					return;
				}
			...
			void handleWait(const boost::system::error_code& ec) {
				if (ec) {
					...
					BITXORCORE_THROW_EXCEPTION(boost::system::system_error(ec));
				}
			...
			void handleCompletion(TaskResult result) {
				if (result == TaskResult::Break) {
					BITXORCORE_LOG(warning) << "task '" << m_task.Name << "' broke and will be stopped";
					return;
				}

				auto nextDelay = m_task.NextDelay();
				BITXORCORE_LOG(trace) << "task '" << m_task.Name << "' will continue in " << nextDelay;
		...
		public:
			void addTask(const Task& task) override {
				if (m_isStopped)
					BITXORCORE_THROW_RUNTIME_ERROR("cannot add new scheduled task because scheduler has shutdown");
			...
			void shutdown() override {
				...
				BITXORCORE_LOG(trace) << "Scheduler stopping";
				m_tasks.clear();
				BITXORCORE_LOG(info) << "Scheduler stopped";
			}
