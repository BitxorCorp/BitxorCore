...
#define TEST_CLASS TokenAddressRestrictionTransactionTests
...
#undef FIELD
		...
		void AssertTransactionHasExpectedProperties() {
			// Assert:
			EXPECT_EQ(Entity_Type_Token_Address_Restriction, T::Entity_Type);
...
#undef TRANSACTION_FIELDS
	...
	ADD_BASIC_TRANSACTION_SIZE_PROPERTY_TESTS(TokenAddressRestriction)

	// endregion

	TEST(TEST_CLASS, CanCalculateRealSizeWithReasonableValues) {
		// Arrange:
		TokenAddressRestrictionTransaction transaction;
		transaction.Size = 0;

		// Act:
		auto realSize = TokenAddressRestrictionTransaction::CalculateRealSize(transaction);

		// Assert:
		EXPECT_EQ(sizeof(TokenAddressRestrictionTransaction), realSize);
	}
}}
