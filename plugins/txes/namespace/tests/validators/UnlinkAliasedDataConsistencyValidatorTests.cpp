...
#define DEFINE_ALIAS_TRANSACTION_PLUGIN_TESTS(TEST_CLASS, TRAITS_NAME) \
	...
	namespace {
		...
		public:
			...
			using InvalidAliasType = TokenId;
	...
	// region unlink aliased token id consistency validator tests

	namespace {
		struct TokenIdTraits {
		public:
			using NotificationType = model::AliasedTokenIdNotification;
			using AliasType = TokenId;
			using InvalidAliasType = Address;

			static auto CreateValidator() {
				return CreateUnlinkAliasedTokenIdConsistencyValidator();
			}
		};
	}

	DEFINE_COMMON_VALIDATOR_TESTS(UnlinkAliasedTokenIdConsistency,)

	DEFINE_ALIAS_TRANSACTION_PLUGIN_TESTS(UnlinkAliasedTokenIdConsistencyValidatorTests, TokenIdTraits)
