...
#pragma pack(pop)
		...
		static size_t WriteNamespaceData(
			...
			for (const auto& seed : seeds) {
				...
				*pData++ = utils::to_underlying_type(seed.Alias.type());
				...
				case state::AliasType::Token:
					reinterpret_cast<TokenId&>(*pData) = seed.Alias.tokenId();
					aliasDataSize = sizeof(TokenId);
					break;
	...
	public:
		...
		static void AssertCannotLoadEmptyHistory() {
			...
			// Act + Assert:
			TTraits::template AssertCannotLoad<bitxorcore_runtime_error>(stream);
		}
	...
	public:
		...
		static void AssertCanLoadHistoryWithDepthOneWithoutChildren_RootTokenAlias() {
			// Arrange:
			auto owner = test::CreateRandomOwner();
			std::vector<uint8_t> buffer(sizeof(NamespaceHistoryHeader) + sizeof(RootNamespaceHeader) + sizeof(TokenId));
			auto offset = WriteDepthOneWithoutChildrenHeaders(buffer, owner, state::AliasType::Token);

			WriteAliasData(buffer, offset, TokenId(876));
			mocks::MockMemoryStream stream(buffer);

			// Act:
			TTraits::AssertCanLoadHistoryWithDepthOneWithoutChildren(stream, owner, state::NamespaceAlias(TokenId(876)));
		}
	...
	public:
		...
		static void AssertCanLoadHistoryWithDepthOneWithChildren_WithAliases() {
			...
			AssertCanLoadHistoryWithDepthOneWithChildren(aliasDataSize, {
				state::NamespaceAlias(TokenId(555)),
				state::NamespaceAlias(test::GenerateRandomByteArray<Address>()),
				state::NamespaceAlias(TokenId(888))
			});
		}
	...
	public:
		...
		static void AssertCanLoadHistoryWithDepthGreaterThanOneSameOwner_WithAliases() {
			// Assert:
			auto aliasDataSize = 2 * sizeof(TokenId) + 2 * Address::Size;
	...
	private:
		static void AssertCannotLoadWithBadData(const NamespaceData& badData) {
			...
			// Act + Assert:
			TTraits::template AssertCannotLoad<bitxorcore_invalid_argument>(stream);
		}
...
#define DEFINE_ROOT_NAMESPACE_HISTORY_LOAD_COMMON_TESTS(TRAITS_NAME, POSTFIX) \
	...
	MAKE_ROOT_NAMESPACE_HISTORY_LOAD_TEST(TRAITS_NAME, CanLoadHistoryWithDepthOneWithoutChildren_RootTokenAlias, POSTFIX) \
