...
#define SERIALIZER_TEST(TEST_NAME) \
	...
	namespace {
		void AssertRootHeader(
			...
			if (AliasType::Token == alias.type()) {
				auto tokenId = reinterpret_cast<const TokenId&>(*pAliasData);
				EXPECT_EQ(alias.tokenId(), tokenId) << message;
				pAliasData += sizeof(TokenId);
			} else {
		...
		void AssertNamespaceData(
			...
			for (auto i = 0u; i < expected.size(); ++i) {
				...
				case AliasType::Token:
					EXPECT_EQ(expectedAlias.tokenId(), reinterpret_cast<const TokenId&>(*pData)) << "token alias" << message.str();
					pData += sizeof(TokenId);
					break;
	...
	TEST(TEST_CLASS, CannotSaveEmptyHistory_NonHistorical) {
		...
		// Act + Assert:
		EXPECT_THROW(NonHistoricalTraits::Serializer::Save(history, stream), bitxorcore_runtime_error);
	}
	...
	SERIALIZER_TEST(CanSaveHistoryWithDepthOneWithoutChildren_RootTokenAlias) {
		AssertCanSaveHistoryWithDepthOneWithoutChildren<TTraits>(NamespaceAlias(TokenId(567)), sizeof(TokenId));
	}
	...
	SERIALIZER_TEST(CanSaveHistoryWithDepthOneWithChildren_WithAliases) {
		...
		std::vector<NamespaceDataWithAliasPayload> expectedChildNamespaceData{
			{ NamespaceId(124), NamespaceId(), NamespaceAlias(TokenId(444)) },
			{ NamespaceId(124), NamespaceId(125), NamespaceAlias(aliasedAddress) },
			{ NamespaceId(126), NamespaceId(), NamespaceAlias(TokenId(987)) }
		};

		// Assert:
		auto aliasDataSize = 2 * sizeof(TokenId) + Address::Size;
		AssertCanSaveHistoryWithDepthOneWithChildren<TTraits>(expectedChildNamespaceData, aliasDataSize, [&aliasedAddress](auto& history) {
			// Arrange:
			history.back().setAlias(NamespaceId(124), NamespaceAlias(TokenId(444)));
			history.back().setAlias(NamespaceId(125), NamespaceAlias(aliasedAddress));
			history.back().setAlias(NamespaceId(126), NamespaceAlias(TokenId(987)));
		});
	}
	...
	namespace {
		...
		struct LoadTraits {
			...
			static void AssertCannotLoadHistoryWithDepthOneOutOfOrderChildren(io::InputStream& inputStream) {
				// Act + Assert:
				EXPECT_THROW(TTraits::Serializer::Load(inputStream), bitxorcore_invalid_argument);
			}
	...
	SERIALIZER_TEST(CanRoundtripHistoryWithDepthOneWithoutChildren_RootTokenAlias) {
		AssertCanRoundtripHistoryWithDepthOneWithoutChildren<TTraits>(NamespaceAlias(TokenId(567)));
	}
