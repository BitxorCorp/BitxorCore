...
#define TEST_CLASS NamespaceLifetimeTests
	...
	TEST(TEST_CLASS, CannotCreateNamespaceLifetimeWithZeroLifetime) {
		EXPECT_THROW(NamespaceLifetime(Height(123), Height(123)), bitxorcore_invalid_argument);

		EXPECT_THROW(NamespaceLifetime(Height(123), Height(123), BlockDuration(50)), bitxorcore_invalid_argument);
	}

	TEST(TEST_CLASS, CannotCreateNamespaceLifetimeWithNegativeLifetime) {
		EXPECT_THROW(NamespaceLifetime(Height(125), Height(124)), bitxorcore_invalid_argument);
		EXPECT_THROW(NamespaceLifetime(Height(125), Height(63)), bitxorcore_invalid_argument);

		EXPECT_THROW(NamespaceLifetime(Height(125), Height(124), BlockDuration(50)), bitxorcore_invalid_argument);
		EXPECT_THROW(NamespaceLifetime(Height(125), Height(63), BlockDuration(50)), bitxorcore_invalid_argument);
	}

	TEST(TEST_CLASS, CannotCreateNamespaceWithGracePeriodEndOverflow) {
		auto maxValue = std::numeric_limits<uint64_t>::max();
		EXPECT_THROW(NamespaceLifetime(Height(123), Height(234), BlockDuration(maxValue - 234 + 1)), bitxorcore_invalid_argument);
		EXPECT_THROW(NamespaceLifetime(Height(123), Height(234), BlockDuration(maxValue - 123 + 1)), bitxorcore_invalid_argument);
		EXPECT_THROW(NamespaceLifetime(Height(123), Height(234), BlockDuration(maxValue)), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, IsActiveExcludingGracePeriodFailsWhenGracePeriodIsGreaterThanOrEqualToLifetime) {
		...
		for (auto gracePeriodDuration : { 161u, 162u, 259u, 260u, 5000u, 100000u }) {
			EXPECT_THROW(lifetime.isActiveExcludingGracePeriod(Height(222), BlockDuration(gracePeriodDuration)), bitxorcore_invalid_argument)
					<< "with grace period duration " << gracePeriodDuration;
		}
	}
