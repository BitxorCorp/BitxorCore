...
#define DELTA_VIEW_BASED_TEST(TEST_NAME) \
	...
	DELTA_VIEW_BASED_TEST(GetThrowsWhenNamespaceIsUnknown) {
		...
		TTraits::RunTest([](const auto& view) {
			...
			// Assert:
			EXPECT_THROW(namespaceIter.get(), bitxorcore_invalid_argument);
		});
	}
	...
	TEST(TEST_CLASS, CannotInsertChildWhenParentIsUnknown) {
		...
		// Act + Assert:
		EXPECT_THROW(delta->insert(state::Namespace(test::CreatePath({ 123, 126, 127 }))), bitxorcore_invalid_argument);
		EXPECT_THROW(delta->insert(state::Namespace(test::CreatePath({ 125, 127 }))), bitxorcore_invalid_argument);
		EXPECT_THROW(delta->insert(state::Namespace(test::CreatePath({ 122 }))), bitxorcore_invalid_argument);
	}
	...
	namespace {
		...
		void AssertCanSetNamespace(NamespaceId namespaceId) {
...

			{
				...
				// Act:
				delta->setAlias(namespaceId, state::NamespaceAlias(TokenId(444)));
				cache.commit();
			}
			...
			// Assert:
			EXPECT_EQ(state::AliasType::Token, alias.type());
			EXPECT_EQ(TokenId(444), alias.tokenId());
		}
	}
	...
	TEST(TEST_CLASS, CannotSetUnknownNamespaceAlias) {
		...
		// Act + Assert:
		EXPECT_THROW(delta->setAlias(NamespaceId(129), state::NamespaceAlias(TokenId(444))), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, CannotRemoveUnknownNamespace) {
		...
		// Act + Assert:
		EXPECT_THROW(delta->remove(NamespaceId(12)), bitxorcore_invalid_argument);
		EXPECT_THROW(delta->remove(NamespaceId(123)), bitxorcore_invalid_argument);
		EXPECT_THROW(delta->remove(NamespaceId(3579)), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, CannotRemoveRootNamespaceWithChildrenWhenHistoryDepthIsOne) {
		...
		EXPECT_THROW(delta->remove(NamespaceId(3)), bitxorcore_runtime_error);
	}
	...
	TEST(TEST_CLASS, RemoveRestoresAliasesFromHistory) {
		...
		NamespaceCacheMixinTraits::CacheType cache;
		{
			...
			delta->insert(state::Namespace(test::CreatePath({ 123, 111 })));
			delta->insert(state::Namespace(test::CreatePath({ 123, 222 })));
			delta->setAlias(NamespaceId(123), state::NamespaceAlias(TokenId(1)));
			delta->setAlias(NamespaceId(111), state::NamespaceAlias(TokenId(2)));
			delta->setAlias(NamespaceId(222), state::NamespaceAlias(TokenId(3)));

			// Act: renew root once
			delta->insert(state::RootNamespace(NamespaceId(123), test::CreateRandomOwner(), test::CreateLifetime(345, 456)));
			delta->insert(state::Namespace(test::CreatePath({ 123, 222 })));
			delta->insert(state::Namespace(test::CreatePath({ 123, 333 })));
			delta->setAlias(NamespaceId(123), state::NamespaceAlias(TokenId(4)));
			delta->setAlias(NamespaceId(222), state::NamespaceAlias(TokenId(5)));
			delta->setAlias(NamespaceId(333), state::NamespaceAlias(TokenId(6)));
		...
		{
			auto view = cache.createView();
			EXPECT_EQ(TokenId(4), GetNamespaceAliasFromCache(*view, NamespaceId(123)).tokenId());
			EXPECT_EQ(TokenId(5), GetNamespaceAliasFromCache(*view, NamespaceId(222)).tokenId());
			EXPECT_EQ(TokenId(6), GetNamespaceAliasFromCache(*view, NamespaceId(333)).tokenId());
		}
		...
		// Assert: only children from the older root are contained
		auto view = cache.createView();
		EXPECT_EQ(TokenId(1), GetNamespaceAliasFromCache(*view, NamespaceId(123)).tokenId());
		EXPECT_EQ(TokenId(2), GetNamespaceAliasFromCache(*view, NamespaceId(111)).tokenId());
		EXPECT_EQ(TokenId(3), GetNamespaceAliasFromCache(*view, NamespaceId(222)).tokenId());
	}
