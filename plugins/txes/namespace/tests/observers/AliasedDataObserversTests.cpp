...
#define TEST_CLASS AliasedDataObserversTests
	...
	DEFINE_COMMON_OBSERVER_TESTS(AliasedTokenId,)
	...
	namespace {
		...
		struct TokenIdTraits {
			...
			static auto CreateObserver() {
				return CreateAliasedTokenIdObserver();
			}
		};
	}
...
#define MAKE_ALIASED_DATA_OBSERVER_TEST(TEST_NAME) \
	...
	TEST(TEST_CLASS, TEST_NAME##_Commit_TokenId) { TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<TokenIdTraits, CommitTraits>(); } \
	TEST(TEST_CLASS, TEST_NAME##_Rollback_Address) { TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<AddressTraits, RollbackTraits>(); } \
	TEST(TEST_CLASS, TEST_NAME##_Rollback_TokenId) { TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<TokenIdTraits, RollbackTraits>(); } \
	template<typename TTraits, typename TDirectionTraits> void TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)()
