...
namespace bitxorcore { namespace validators {
	...
	/// - namespace name has a maximum size of \a maxNameSize
	/// - namespace name consists only of allowed characters
	DECLARE_STATELESS_VALIDATOR(NamespaceIsoCode, model::NamespaceNameNotification)(uint8_t maxNameSize);
	...
	/// - namespace is available and can be created
	/// - child namespace depth does not exceed \a maxNamespaceDepth
	DECLARE_STATEFUL_VALIDATOR(IsoCodeNamespaceAvailability, model::IsoCodeNamespaceNotification)(uint8_t maxNamespaceDepth);
	...
	/// Validator that applies to aliased token id notifications and validates that:
	/// - unlink operation matches existing link
	DECLARE_STATEFUL_VALIDATOR(UnlinkAliasedTokenIdConsistency, model::AliasedTokenIdNotification)();
