...
namespace bitxorcore { namespace plugins {
	...
	struct NamespaceAliasFeeConfiguration {
		/// Currency token id.
		...
		/// Unlink rental fee.
		Amount UnlinkRentalFee;
	};
	/// Creates a token alias transaction plugin.
	PLUGIN_API
	std::unique_ptr<model::TransactionPlugin> CreateTokenAliasTransactionPlugin(const NamespaceAliasFeeConfiguration& config);
}}
