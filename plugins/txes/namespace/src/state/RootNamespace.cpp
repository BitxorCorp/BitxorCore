...
namespace bitxorcore { namespace state {
	...
	Namespace RootNamespace::child(NamespaceId id) const {
		...
		if (m_pChildren->cend() == iter)
			BITXORCORE_THROW_INVALID_ARGUMENT_1("unknown child namespace (id) ", id);
	...
	void RootNamespace::add(const Namespace& ns) {
		...
		if (m_pChildren->cend() != m_pChildren->find(ns.id()))
			BITXORCORE_THROW_INVALID_ARGUMENT_1("child namespace already exists (id)", ns.id());

		// parent must be known and must be on same level
		if (id() != ns.parentId()) {
			auto iter = m_pChildren->find(ns.parentId());
			if (m_pChildren->cend() == iter)
				BITXORCORE_THROW_INVALID_ARGUMENT_2("child namespace has no parent, (child id, parent id)", ns.id(), ns.parentId());

			if (iter->second.Path.size() != ns.path().size() - 1)
				BITXORCORE_THROW_INVALID_ARGUMENT_1("child has incorrect path (id)", ns.id());
		}

		// must have same root
		if (id() != ns.rootId())
			BITXORCORE_THROW_INVALID_ARGUMENT_2("child namespace has incorrect root, (child id, root id)", ns.id(), ns.rootId());
	...
	void RootNamespace::remove(NamespaceId id) {
		auto iter = m_pChildren->find(id);
		if (m_pChildren->cend() == iter)
			BITXORCORE_THROW_INVALID_ARGUMENT_1("cannot remove child namespace (id)", id);
		...
		if (hasDescendents)
			BITXORCORE_THROW_INVALID_ARGUMENT_1("cannot remove child namespace because it has decendents (id)", id);
	...
	void RootNamespace::setAlias(NamespaceId id, const NamespaceAlias& alias) {
		...
		auto iter = m_pChildren->find(id);
		if (m_pChildren->cend() == iter)
			BITXORCORE_THROW_INVALID_ARGUMENT_2("namespace (id) being linked is not under (root)", id, m_id);
