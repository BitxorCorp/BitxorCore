...
namespace bitxorcore { namespace state {
	...
	void RootNamespaceHistoryNonHistoricalSerializer::Save(const RootNamespaceHistory& history, io::OutputStream& output) {
		...
		if (0 == history.historyDepth())
			BITXORCORE_THROW_RUNTIME_ERROR_1("cannot save empty namespace history", history.id());
	...
	namespace {
		...
		NamespaceAlias LoadAlias(io::InputStream& input) {
			...
			switch (aliasType) {
			case AliasType::Token:
				return NamespaceAlias(io::Read<TokenId>(input));
