...
namespace bitxorcore { namespace state {
	...
	public:
		...
		static void AssertCannotPushLockInfoWithDifferentIdentifier() {
			...
			EXPECT_THROW(history.push_back(TTraits::CreateLockInfo(Height(99), hash2)), bitxorcore_invalid_argument);
		...
		static void AssertCannotPopLockInfoWhenEmpty() {
			...
			// Act + Assert:
			EXPECT_THROW(history.pop_back(), bitxorcore_invalid_argument);
		}
