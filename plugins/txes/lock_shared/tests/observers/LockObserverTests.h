...
namespace bitxorcore { namespace observers {
	...
	public:
		...
		static void AssertObserverCannotAddToHistoryWhenInfoIsActive() {
			...
			// Act + Assert:
			auto pObserver = TTraits::CreateObserver();
			EXPECT_THROW(test::ObserveNotification(*pObserver, notification, observerContext), bitxorcore_invalid_argument);
		}
	...
	private:
		static void AssertLockInfo(const NotificationType& notification, const LockInfoType& lockInfo) {
			...
			EXPECT_EQ(notification.Token.TokenId, test::UnresolveXor(lockInfo.TokenId));
			EXPECT_EQ(notification.Token.Amount, lockInfo.Amount);
		...
		static void AssertStatement(const NotificationType& notification, const model::BlockStatement& statement) {
			...
			EXPECT_EQ(notification.Token.TokenId, test::UnresolveXor(receipt.Token.TokenId));
			EXPECT_EQ(notification.Token.Amount, receipt.Token.Amount);
