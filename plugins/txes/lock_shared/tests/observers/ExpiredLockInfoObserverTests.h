...
namespace bitxorcore { namespace observers {
	...
	public:
		...
		static void RunBalanceTest(
			...
			auto notificationBlockHarvester = context.addBlockHarvester(harvesterType, harvesterPublicKey, TokenId(500), Amount(200));
		...
		static void RunReceiptTest(
			...
			for (const auto& expectedReceipt : expectedReceipts) {
				...
				EXPECT_EQ(expectedReceipt.TokenId, receipt.Token.TokenId) << message;
				EXPECT_EQ(expectedReceipt.LockAmount, receipt.Token.Amount) << message;
	...
	private:
		...
		static std::vector<SeedTuple> GenerateSeeds(size_t count) {
			...
			for (auto i = 0u; i < count; ++i)
				seeds.push_back({ test::GenerateRandomByteArray<Address>(), TokenId(2 * (i + 1)), Amount(100 + i), Amount(i) });
	...
	private:
		...
		public:
			...
			Address addBlockHarvester(HarvesterType harvesterType, const Key& harvesterPublicKey, TokenId tokenId, Amount amount) {
				...
				harvesterAccountStateIter.get().Balances.credit(tokenId, amount);
			...
			void addLockInfo(const SeedTuple& seed, Height height) {
				...
				if (!accountStateCache.contains(seed.OwnerAddress)) {
					...
					accountStateCache.find(seed.OwnerAddress).get().Balances.credit(seed.TokenId, seed.InitialBalance);
				}
			}
		...
		public:
			...
			void assertBalances(const std::vector<SeedTuple>& seeds, const std::string& message) {
				...
				for (const auto& seed : seeds) {
					...
					EXPECT_EQ(seed.InitialBalance, balances.get(seed.TokenId)) << message << " at " << i;
					++i;
				}
			}
