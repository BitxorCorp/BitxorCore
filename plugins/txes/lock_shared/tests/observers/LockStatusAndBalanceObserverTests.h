	...
	private:
		static void AssertObserverSetsStatusToUsedAndCreditsBalanceOnCommit(Amount initialAmount) {
			...
			RunTest(std::move(observerContext), initialAmount, Amount(100), [&](
				...
				EXPECT_EQ(initialAmount + Amount(100), accountState.Balances.get(lockInfo.TokenId));
				...
				EXPECT_EQ(originalLockInfo.TokenId, receipt.Token.TokenId);
				EXPECT_EQ(originalLockInfo.Amount, receipt.Token.Amount);
	...
	public:
		...
		static void AssertObserverSetsStatusToUnusedAndDebitsBalanceOnRollback() {
			...
			RunTest(std::move(observerContext), Amount(1000), Amount(100), [&](
				...
				EXPECT_EQ(Amount(900), accountState.Balances.get(originalLockInfo.TokenId));
	...
	private:
		...
		static void RunTest(
			...
			if (Amount(0) != initialAmount)
				accountState.Balances.credit(originalLockInfo.TokenId, initialAmount);
