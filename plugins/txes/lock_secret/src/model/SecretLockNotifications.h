...
#undef DEFINE_SECRET_LOCK_NOTIFICATION
	...
	public:
		/// Creates secret lock notification around \a owner, \a token, \a duration, \a hashAlgorithm, \a secret and \a recipient.
		SecretLockNotification(
				const Address& owner,
				const UnresolvedToken& token,
