...
namespace bitxorcore { namespace test {
	...
	Hash256 CreateMetadataUniqueKeyFromSeed(uint8_t seed) {
		...
		key[0] = seed;
		key[key.size() - 1] = utils::to_underlying_type(model::MetadataType::Token);
	...
	state::MetadataKey GenerateRandomMetadataKey() {
		...
		case model::MetadataType::Token:
			return state::MetadataKey(partialKey, TokenId(targetId));

		case model::MetadataType::Namespace:
			return state::MetadataKey(partialKey, NamespaceId(targetId));
		}

		BITXORCORE_THROW_INVALID_ARGUMENT_1("cannot create MetadataKey with unexpected type", static_cast<uint16_t>(metadataType));
	}
	...
	model::MetadataValueNotification CreateMetadataValueNotification(
		...
		if (model::MetadataType::Token == metadataKey.metadataType())
			targetId = UnresolveXor(TokenId(targetId)).unwrap();
