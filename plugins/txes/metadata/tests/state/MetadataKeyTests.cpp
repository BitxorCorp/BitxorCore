...
#pragma pack(pop)
	...
	TEST(TEST_CLASS, CanCreateMetadataKey_Account) {
		...
		EXPECT_THROW(key.tokenTarget(), bitxorcore_invalid_argument);
		EXPECT_THROW(key.namespaceTarget(), bitxorcore_invalid_argument);
	...
	TEST(TEST_CLASS, CanCreateMetadataKey_Token) {
		// Act:
		auto tokenId = test::GenerateRandomValue<TokenId>();
		RunMetadataKeyTestWithTarget(model::MetadataType::Token, tokenId, [tokenId](const auto& key) {
			// Assert:
			EXPECT_EQ(tokenId, key.tokenTarget());
			EXPECT_THROW(key.namespaceTarget(), bitxorcore_invalid_argument);
		});
	}
	...
	TEST(TEST_CLASS, CreateMetadataKey_CanCreateFromTokenTarget) {
		AssertCanCreateMetadataKey(model::MetadataType::Token, [](auto seedTargetId, auto keyTargetId) {
	...
	TEST(TEST_CLASS, CreateMetadataKey_CannotCreateFromOtherTarget) {
		...
		// Act + Assert:
		EXPECT_THROW(CreateMetadataKey(partialKey, target), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, ResolveMetadataKey_CanResolveFromTokenTarget) {
		AssertCanResolveMetadataKey(model::MetadataType::Token, [](auto seedTargetId, auto keyTargetId) {
			EXPECT_EQ(UnresolvedTokenId(seedTargetId), test::UnresolveXor(TokenId(keyTargetId)));
		});
	}
	...
	TEST(TEST_CLASS, ResolveMetadataKey_CannotResolveFromOtherTarget) {
		...
		// Act + Assert:
		EXPECT_THROW(ResolveMetadataKey(UnresolveXor(partialKey), target, test::CreateResolverContextXor()), bitxorcore_invalid_argument);
	}
