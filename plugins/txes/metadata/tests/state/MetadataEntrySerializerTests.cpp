...
#pragma pack(pop)
	...
	namespace {
		...
		struct TokenTraits {
			static constexpr auto Metadata_Type = model::MetadataType::Token;

			static MetadataKey CreateKeyWithTargetId(const model::PartialMetadataKey& partialKey, uint64_t targetId) {
				return MetadataKey(partialKey, TokenId(targetId));
			}
		};
...
#define METADATA_TRAITS_BASED_TEST(TEST_NAME) \
	...
	METADATA_TRAITS_BASED_TEST(CannotLoadWithInvalidMetadataType) {
		...
		// Act + Assert:
		EXPECT_THROW(MetadataEntrySerializer::Load(stream), bitxorcore_invalid_argument);
	}
