...
#define TEST_CLASS MetadataTransactionTests
	...
	namespace {
		...
		struct TokenTraits {
			using TransactionType = TokenMetadataTransaction;
			using EmbeddedTransactionType = EmbeddedTokenMetadataTransaction;
		};
...
#define METADATA_TYPE_BASED_TEST(TEST_NAME) \
	...
	TEST(TokenMetadataTransactionTests, TEST_NAME) { TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<TokenTraits>(); } \
...
#undef FIELD
		...
		using TokenMetadataFlag = std::integral_constant<MetadataType, MetadataType::Token>;
		...
		struct MetadataTypeAccessor<
				T,
				utils::traits::is_type_expression_t<decltype(reinterpret_cast<const T*>(1)->TargetTokenId)>>
				: public TokenMetadataFlag
		{};
...
#undef FIELD
			...
			if constexpr (std::is_base_of_v<TokenMetadataFlag, MetadataTypeAccessor<T>>)
				EXPECT_ALIGNED(T, TargetTokenId);
...
#undef TRANSACTION_FIELDS
	...
	ADD_BASIC_TRANSACTION_SIZE_PROPERTY_TESTS_WITH_ARGS(TokenMetadata, Entity_Type_Token_Metadata, sizeof(uint64_t))
	...
	DEFINE_ATTACHMENT_POINTER_TESTS(TokenMetadataTransactionTests, MetadataTransactionTraits<TokenTraits>) // ValuePtr
