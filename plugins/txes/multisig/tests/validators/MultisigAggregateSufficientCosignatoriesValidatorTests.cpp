...
#define NON_MULTISIG_TRAITS_TEST(TEST_NAME) \
	...
	namespace {
		...
		void AssertMinApprovalLimit(
			...
			// Assert:
			BITXORCORE_LOG(debug) << "running test with " << expectedLimit - 1 << " cosignatories (insufficient)";
			...
			BITXORCORE_LOG(debug) << "running test with " << expectedLimit << " cosignatories (sufficient)";
	...
	TEST(TEST_CLASS, MinRemovalLimitIsAppliedAcrossMultipleLevels) {
		...
		BITXORCORE_LOG(debug) << "running test with 3 + 3 cosignatories (insufficient)";
		...
		BITXORCORE_LOG(debug) << "running test with 2 + 4 cosignatories (insufficient)";
		...
		BITXORCORE_LOG(debug) << "running test with 3 + 4 cosignatories (sufficient)";
	...
	namespace {
		...
		void AddSingleLevelMultisig(cache::BitxorCoreCache& cache, const Key& multisig, const std::vector<Key>& cosignatories) {
			auto cacheDelta = cache.createDelta();
