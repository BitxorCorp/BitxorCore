...
namespace bitxorcore { namespace plugins {
	...
	void RegisterMultisigSubsystem(PluginManager& manager) {
		...
		auto config = model::LoadPluginConfiguration<config::MultisigConfiguration>(manager.config(), "bitxorcore.plugins.multisig");
...
void RegisterSubsystem(bitxorcore::plugins::PluginManager& manager) {
	bitxorcore::plugins::RegisterMultisigSubsystem(manager);
}
