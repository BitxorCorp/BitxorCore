...
#define TEST_CLASS TransferMessageObserverTests
	...
	DEFINE_COMMON_OBSERVER_TESTS(TransferMessage, 0, Address(), config::BitxorCoreDirectory(""))
	...
	namespace {
		...
		public:
			void observe(const Address& recipient, const model::TransferMessageNotification& notification) {
				auto pObserver = CreateTransferMessageObserver(0x1122334455667788, recipient, config::BitxorCoreDirectory(m_tempDir.name()));
