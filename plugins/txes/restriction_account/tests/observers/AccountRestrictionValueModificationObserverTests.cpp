...
#define TEST_CLASS AccountRestrictionValueModificationObserverTests
	...
	DEFINE_COMMON_OBSERVER_TESTS(AccountTokenRestrictionValueModification,)
	...
	namespace {
		...
		struct AccountTokenRestrictionTraits : public test::BaseAccountTokenRestrictionTraits {
			static constexpr auto CreateObserver = CreateAccountTokenRestrictionValueModificationObserver;

			using NotificationType = model::ModifyAccountTokenRestrictionValueNotification;
		};
		...
		void AssertObserverDoesNotRemoveAccountRestrictionsWithValues(NotifyMode notifyMode) {
...

			{
				...
				TOperationTraits::Add(
						restrictions.restriction(model::AccountRestrictionFlags::TokenId),
						test::GenerateRandomVector(sizeof(TokenId)));
			}
			...
			// Act: cache entry is not removed, account address restriction is empty but account token restriction is not
			test::ObserveNotification(*pObserver, notification, context);
			...
			EXPECT_EQ(1u, restrictions.restriction(model::AccountRestrictionFlags::TokenId).values().size());
		}
...
#define TRAITS_BASED_TEST(TEST_NAME) \
	...
	TEST(TEST_CLASS, TEST_NAME##_Token_Allow) { \
		TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<test::AllowTraits, AccountTokenRestrictionTraits>(); \
	} \
	TEST(TEST_CLASS, TEST_NAME##_Token_Block) { \
		TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<test::BlockTraits, AccountTokenRestrictionTraits>(); \
	} \
