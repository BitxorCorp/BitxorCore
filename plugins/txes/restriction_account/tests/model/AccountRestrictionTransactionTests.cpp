...
#define TEST_CLASS AccountRestrictionTransactionTests
	...
	namespace {
		...
		struct TokenTraits {
			using TransactionType = AccountTokenRestrictionTransaction;
			using RestrictionType = UnresolvedTokenId;
		};
...
#define RESTRICTION_TYPE_BASED_TEST(TEST_NAME) \
	...
	TEST(AccountTokenRestrictionTransactionTests, TEST_NAME) { TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<TokenTraits>(); } \
...
#undef TRANSACTION_FIELDS
	...
	ADD_BASIC_TRANSACTION_SIZE_PROPERTY_TESTS_WITH_ARGS(AccountTokenRestriction, Entity_Type_Account_Token_Restriction)
	...
	DEFINE_DUAL_ATTACHMENT_POINTER_TESTS(AccountTokenRestrictionTransactionTests, AccountRestrictionTransactionTraits<TokenTraits>)
