	...
	namespace {
		size_t GetValueSize(model::AccountRestrictionFlags restrictionFlags) {
			...
			case model::AccountRestrictionFlags::TokenId:
				return sizeof(TokenId);
			case model::AccountRestrictionFlags::TransactionType:
				return sizeof(model::EntityType);
			default:
				BITXORCORE_THROW_INVALID_ARGUMENT_1("invalid restriction flags", utils::to_underlying_type(restrictionFlags));
			}
		}
		...
		void AssertOrderedRestrictions() {
			...
			std::vector<model::AccountRestrictionFlags> allFlags{
				...
				model::AccountRestrictionFlags::TokenId, //                          0x0002 & ~Block == 0x0002
			...
			restrictions.restriction(allFlags[1]).allow({ Add, test::GenerateRandomVector(sizeof(TokenId)) });
