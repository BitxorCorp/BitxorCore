...
#define TEST_CLASS AccountOperationRestrictionNoSelfBlockingTests
	...
	namespace {
		...
		void AddToCache(cache::BitxorCoreCache& cache, const Address& address, const std::vector<model::EntityType>& values) {
		...
		void RunValidator(ValidationResult expectedResult, cache::BitxorCoreCache& cache, const Notification& notification) {
			// Arrange:
