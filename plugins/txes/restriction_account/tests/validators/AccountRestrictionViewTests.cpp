...
#define TEST_CLASS AccountRestrictionViewTests
	...
	namespace {
		...
		auto PopulateCache(const PopulateOptions& options, cache::BitxorCoreCache& cache) {
	...
	TEST(TEST_CLASS, GetReturnsExpectedAccountRestriction) {
		...
		RunTest(5, [](auto& view, const auto& address) {
			...
			// Act:
			const auto& restriction = view.get(model::AccountRestrictionFlags::TokenId);

			// Assert:
			EXPECT_EQ(5u, restriction.values().size());
			for (auto i = 0u; i < 5; ++i)
				EXPECT_TRUE(restriction.contains(state::ToVector(TokenId(i))));
		});
	}
	...
	TEST(TEST_CLASS, GetThrowsWhenInitializeFailed) {
		...
		RunTest(5, [](auto& view, const auto&) {
			...
			// Act + Assert:
			EXPECT_THROW(view.get(model::AccountRestrictionFlags::TokenId), bitxorcore_invalid_argument);
		});
	}
	...
	TEST(TEST_CLASS, IsAllowedReturnsTrueWhenAccountRestrictionIsEmpty) {
		...
		RunTest(0, [](auto& view, const auto& address) {
			...
			// Act:
			auto isAllowed = view.isAllowed(model::AccountRestrictionFlags::TokenId, TokenId(10));
	...
	TEST(TEST_CLASS, IsAllowedReturnsTrueWhenOperationTypeIsAllowAndValueIsContained) {
		...
		RunTest(5, [](auto& view, const auto& address) {
			...
			// Act:
			auto isAllowed = view.isAllowed(model::AccountRestrictionFlags::TokenId, TokenId(3));
	...
	TEST(TEST_CLASS, IsAllowedReturnsFalseWhenOperationTypeIsAllowAndValueIsNotContained) {
		...
		RunTest(5, [](auto& view, const auto& address) {
			...
			// Act:
			auto isAllowed = view.isAllowed(model::AccountRestrictionFlags::TokenId, TokenId(7));
