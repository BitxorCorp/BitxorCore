...
#define TEST_CLASS PosImportanceCalculatorTests
	...
	namespace {
		...
		struct AccountSeed {
		public:
			AccountSeed(bitxorcore::Amount amount, const std::vector<state::AccountActivityBuckets::ActivityBucket>& buckets)
					: Amount(amount)
					, Buckets(buckets)
			{}
		...
		public:
			void seedDelta(const std::vector<AccountSeed>& accountSeeds, model::ImportanceHeight importanceHeight) {
				...
				for (const auto& accountData : accountSeeds) {
					...
					accountState.Balances.credit(Harvesting_Token_Id, accountData.Amount);
		...
		void AssertCumulativeImportance(cache::AccountStateCacheDelta& cache) {
			...
			for (uint8_t i = 1; i <= Num_Account_States; ++i) {
				...
				if (config.MinHarvesterBalance <= accountState.Balances.get(Harvesting_Token_Id))
	...
	struct NoActivityTraits {
		...
		static void IncreaseImportance(state::AccountState& accountState) {
			accountState.Balances.credit(Harvesting_Token_Id, Amount(1'000'000));
		}
	};
...
#define ACTIVITY_BASED_TEST(TEST_NAME) \
	...
	ACTIVITY_BASED_TEST(ImportanceIsSetAtGenesisHeight) {
		// Arrange:
		auto config = TTraits::CreateConfiguration();
	...
	TEST(TEST_CLASS, PosGivesAccountsImportanceProportionalToBalance) {
		...
		auto referenceAmount = referenceAccountState.Balances.get(Harvesting_Token_Id).unwrap();
		...
		for (uint8_t i = 1; i <= Num_Account_States; ++i) {
			...
			const auto& accountState = holder.get(Key{ { i } });
			EXPECT_EQ(Amount(referenceAmount * i * i), accountState.Balances.get(Harvesting_Token_Id));
	...
	namespace {
		...
		void RunActivityBucketCreationRemovalTest(TAction action) {
			...
			for (uint8_t i = 1u; i <= Num_Account_States; ++i) {
				auto& accountState = holder.get(Key{ { i } });
				if (1 == i % 2) {
					accountState.Balances.debit(Harvesting_Token_Id, Amount(1));
					if (1 == i % 4)
