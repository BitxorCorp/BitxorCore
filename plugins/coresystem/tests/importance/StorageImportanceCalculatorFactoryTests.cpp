...
#define TEST_CLASS StorageImportanceCalculatorFactoryTests
	...
	namespace {
		constexpr auto Harvesting_Token_Id = TokenId(2222);
		constexpr auto Backup_Harvesting_Token_Id = TokenId(3333);
...
#pragma pack(pop)
		...
		private:
			void process(state::AccountState& accountState, ImportanceRollbackMode mode, model::ImportanceHeight importanceHeight) const {
				...
				if (Amount(0) == accountState.Balances.get(Harvesting_Token_Id)) {
		...
		public:
			void addAccounts(const std::vector<Address>& addresses, const std::vector<Amount>& balances) {
				...
				for (const auto& address : addresses) {
					...
					accountStateIter.get().Balances.credit(Harvesting_Token_Id, balances[i]);
					accountStateIter.get().Balances.credit(Backup_Harvesting_Token_Id, balances[i]);
					++i;
				}
			...
			void clearBalances(const std::vector<Address>& addresses) {
				for (const auto& address : addresses) {
					auto accountStateIter = delta().find(address);
					accountStateIter.get().Balances.debit(Harvesting_Token_Id, accountStateIter.get().Balances.get(Harvesting_Token_Id));
				}
		...
		private:
			static cache::AccountStateCacheTypes::Options CreateCacheOptions() {
				auto options = test::CreateDefaultAccountStateCacheOptions(TokenId(1111), Harvesting_Token_Id);
		...
		private:
			...
			config::BitxorCoreDirectory directory() const {
				return config::BitxorCoreDirectory(m_tempDir.name());
			}
	...
	TEST(TEST_CLASS, ReadCalculatorFailsWhenIndexFileIsNotPresent) {
		...
		EXPECT_THROW(pCalculator->recalculate(Default_Calculation_Mode, Importance_Height, holder.get()), bitxorcore_runtime_error);
	}
	...
	namespace {
		...
		void ToggleBalance(state::AccountState& accountState, bool enable) {
			auto currentBalance = accountState.Balances.get(Harvesting_Token_Id);
			if (enable) {
				auto originalBalance = accountState.Balances.get(Backup_Harvesting_Token_Id);
				accountState.Balances.credit(Harvesting_Token_Id, originalBalance - currentBalance);
			} else {
				accountState.Balances.debit(Harvesting_Token_Id, currentBalance);
			}
		}
		...
		void PrepareRollbackTest(TAction action) {
			...
			// - recalculate at all heights from genesis
		...
		void AssertCanRoundtripToPointAfterGenesis() {
			// Arrange:
		...
		template<typename TTraits>
		void AssertCanRoundtripToGenesis() {
			// Arrange:
			...
			PrepareRollbackTest<TTraits>([](const auto& addresses, const auto& context, auto& holder) {
				...
				// - check additional accounts when enabled
				//   (3 is not present in genesis but test doesn't prune removedAddresses realistically)
				if (TTraits::Should_Seed_Additional_Accounts) {
	...
	TEST(TEST_CLASS, CanRoundtripToPointAfterGenesis) {
		AssertCanRoundtripToPointAfterGenesis<MinimumAccountsTraits>();
	}

	TEST(TEST_CLASS, CanRoundtripToPointAfterGenesisWithOtherAccountsInImportanceFiles) {
		AssertCanRoundtripToPointAfterGenesis<AdditionalAccountsTraits>();
	}

	TEST(TEST_CLASS, CanRoundtripToGenesis) {
		AssertCanRoundtripToGenesis<MinimumAccountsTraits>();
	}

	TEST(TEST_CLASS, CanRoundtripToGenesisWithOtherAccountsInImportanceFiles) {
		AssertCanRoundtripToGenesis<AdditionalAccountsTraits>();
	}
