	...
	namespace {
		...
		struct DebitTraits {
			static ValidationResult Validate(const model::BalanceTransferNotification& notification, const ValidatorContext& context) {
				...
				auto debitNotification = model::BalanceDebitNotification(notification.Sender, notification.TokenId, notification.Amount);
		...
		void SetDynamicFeeMultiplier(cache::BitxorCoreCache& cache, BlockFeeMultiplier dynamicFeeMultiplier) {
...
#define VARIABLE_BALANCE_TEST(TEST_NAME) \
	...
	TRANSFER_OR_DEBIT_TEST(FailureWhenTransactionSignerIsUnknown) {
		...
		auto notification = model::BalanceTransferNotification(sender, recipient, test::UnresolveXor(Currency_Token_Id), Amount(0));
	...
	TEST(TRANSFER_TEST_CLASS, SuccessWhenFeeMultiplierIsZero_Dynamic) {
		...
		auto cache = test::CreateCache(sender, { { Currency_Token_Id, Amount(0) } });
	...
	TEST(TRANSFER_TEST_CLASS, FailureWhenEffectiveBalanceOverflows_Dynamic) {
		...
		auto cache = test::CreateCache(sender, { { Currency_Token_Id, Amount(234) } });
	...
	VARIABLE_BALANCE_TEST(Currency) {
		...
		auto sender = test::GenerateRandomByteArray<Address>();
		auto recipient = test::GenerateRandomUnresolvedAddress();
		auto notification = model::BalanceTransferNotification(sender, recipient, test::UnresolveXor(Currency_Token_Id), Amount(234));
		auto cache = test::CreateCache(sender, { { Currency_Token_Id, TTraits::Adjust(Amount(234)) } });

		// Assert:
		AssertValidationResult<TValidatorTraits>(TTraits::Expected_Result, cache, notification);
	}

	VARIABLE_BALANCE_TEST(OtherToken) {
		// Arrange:
		auto sender = test::GenerateRandomByteArray<Address>();
		auto recipient = test::GenerateRandomUnresolvedAddress();
		auto notification = model::BalanceTransferNotification(sender, recipient, test::UnresolveXor(TokenId(12)), Amount(234));
		auto cache = test::CreateCache(sender, { { TokenId(12), TTraits::Adjust(Amount(234)) } });
