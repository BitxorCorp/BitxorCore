...
#define TEST_CLASS PublicKeyValidatorTests
	...
	namespace {
		...
		void AddAccount(cache::BitxorCoreCache& cache, const TIdentifier& identifier) {
			auto cacheDelta = cache.createDelta();
			cacheDelta.sub<cache::AccountStateCache>().addAccount(identifier, Height(1));
			cache.commit(Height(1));
		}

		void AssertValidationResult(ValidationResult expectedResult, const Key& publicKey, const cache::BitxorCoreCache& cache) {
