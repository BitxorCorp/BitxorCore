...
#define TEST_CLASS MaxTransactionsValidatorTests
	...
	TEST(TEST_CLASS, SuccessWhenBlockContainsNoTransactions) {
		for (auto blockType : { model::Entity_Type_Block_Genesis, model::Entity_Type_Block_Normal, model::Entity_Type_Block_Importance })
	...
	TEST(TEST_CLASS, SuccessWhenBlockContainsMoreThanMaxTransactions_Genesis) {
		auto blockType = model::Entity_Type_Block_Genesis;
		AssertValidationResult(Success_Result, blockType, Max_Transactions + 1);
	...
	TEST(TEST_CLASS, FailureWhenBlockContainsMoreThanMaxTransactions_NonGenesis) {
