...
namespace bitxorcore { namespace observers {
	...
	namespace {
		...
		importance::ImportanceRollbackMode GetImportanceRollbackMode(const cache::BitxorCoreCacheDelta& delta) {
			return cache::BitxorCoreCacheDelta::Disposition::Attached == delta.disposition()
