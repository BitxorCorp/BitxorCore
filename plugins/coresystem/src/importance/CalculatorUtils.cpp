...
namespace bitxorcore { namespace importance {
	...
	void CalculateImportances(
		...
		auto tokenId = config.HarvestingTokenId;
		...
		stakeImportance *= accountSummary.pAccountState->Balances.get(tokenId).unwrap();
		stakeImportance *= (100 - importanceActivityPercentage);
		stakeImportance /= context.ActiveHarvestingTokens.unwrap() * 100;
		...
		if (0 < importanceActivityPercentage && 0u < context.TotalFeesPaid.unwrap()) {
			...
			feeImportance /= accountSummary.pAccountState->Balances.get(tokenId).unwrap();
		}
		...
		if (0 < importanceActivityPercentage && 0u < context.TotalBeneficiaryCount) {
			...
			beneficiaryCountImportance /= accountSummary.pAccountState->Balances.get(tokenId).unwrap();
		}
