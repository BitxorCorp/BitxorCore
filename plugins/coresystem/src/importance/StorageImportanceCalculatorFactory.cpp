...
#include "bitxorcore/cache_core/AccountStateCacheDelta.h"
...
#pragma pack(pop)
		...
		private:
			void writeToFile(model::ImportanceHeight importanceHeight, const cache::AccountStateCacheDelta& cache) const {
				...
				BITXORCORE_LOG(debug) << "writing importances to file " << filename << " for height " << importanceHeight;
			...
			PackedAccountEntry pack(const state::AccountState& accountState) const {
				...
				entry.HarvestingBalance = accountState.Balances.get(m_config.HarvestingTokenId);
				...
				auto isGenesisSnapshot = model::ImportanceHeight(1) == accountState.ImportanceSnapshots.height();
				auto isCorrespondingBucketEmpty = model::ImportanceHeight() == previousBucketIter->StartHeight;
				if (!isGenesisSnapshot && isCorrespondingBucketEmpty)
		...
		public:
			void recalculate(
					...
					cache::AccountStateCacheDelta& cache) const override {
				if (ImportanceRollbackMode::Disabled == mode)
					BITXORCORE_THROW_INVALID_ARGUMENT("cannot rollback importances when rollback is disabled");
				...
				BITXORCORE_LOG(debug) << "restoring older importances from file for height " << importanceHeight;
		...
		private:
			...
			void processFile(model::ImportanceHeight importanceHeight, const consumer<const PackedAccountEntry&>& process) const {
				auto filename = m_directory.file(GetFilename(importanceHeight));
				BITXORCORE_LOG(debug) << "restoring older importances from file " << filename << " for height " << importanceHeight;
	...
	std::unique_ptr<ImportanceCalculator> StorageImportanceCalculatorFactory::createWriteCalculator(
			std::unique_ptr<ImportanceCalculator>&& pCalculator,
			const config::BitxorCoreDirectory& directory) const {
