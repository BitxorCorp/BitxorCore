...
#include "bitxorcore/crypto/SharedKey.h"
#include "bitxorcore/crypto/Signer.h"
#include "bitxorcore/extensions/Bip32.h"
#include "bitxorcore/extensions/Bip39.h"
...
namespace bitxorcore { namespace tools { namespace testvectors {
	...
	namespace {
		...
		auto Get(const pt::ptree& tree, const std::string& key) {
			...
			if (!value.is_initialized()) {
				...
				BITXORCORE_THROW_RUNTIME_ERROR(message.str().c_str());
			}
		...
		auto ParseTokenId(const RawString& str, size_t testCaseNumber) {
			AssertSize("token id", 2 * sizeof(TokenId::ValueType), str, testCaseNumber);
			auto buffer = ParseVector(str, testCaseNumber, sizeof(TokenId::ValueType));

			TokenId::ValueType rawTokenId = 0;
			for (auto byte : buffer) {
				rawTokenId <<= 8;
				rawTokenId += byte;
			}

			return TokenId(rawTokenId);
		}
		...
		std::optional<bool> SignTester(const std::string&, const pt::ptree& testCase, size_t testCaseNumber) {
			...
			if (publicKey != keyPair.publicKey())
				BITXORCORE_THROW_RUNTIME_ERROR_1("public key mismatch", testCaseNumber);
		...
		std::optional<bool> TokenIdDerivationTester(const std::string&, const pt::ptree& testCase, size_t testCaseNumber) {
			// Arrange:
			auto nonce = TokenNonce(Get<TokenNonce::ValueType>(testCase, "tokenNonce"));
			...
			auto expectedTokenIdMainnet = ParseTokenId(Get<>(testCase, "tokenId_Public"), testCaseNumber);
			auto expectedTokenIdTestnet = ParseTokenId(Get<>(testCase, "tokenId_PublicTest"), testCaseNumber);

			// Act:
			auto tokenIdMainnet = model::GenerateTokenId(addressMainnet, nonce);
			auto tokenIdTestnet = model::GenerateTokenId(addressTestnet, nonce);

			// Assert:
			return expectedTokenIdMainnet == tokenIdMainnet && expectedTokenIdTestnet == tokenIdTestnet;
		}
		...
		public:
			...
			int run(const Options&) override {
				...
				runTest(5, "5.test-token-id", "token id derivation", TokenIdDerivationTester);
		...
		private:
			...
			void runTest(
				...
				if (!shouldExecute(testId)) {
					BITXORCORE_LOG(debug) << testName << " SKIPPED";
					return;
				}
				...
				if (numFailed)
					BITXORCORE_LOG(warning) << testName << " test: " << numFailed << " failures out of " << testCaseNumber;
				else
					BITXORCORE_LOG(info) << testName << " test: " << testCaseNumber << " successes";
			}
...
int main(int argc, const char** argv) {
	bitxorcore::tools::testvectors::TestVectorsTool tool;
