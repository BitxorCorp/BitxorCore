...
namespace bitxorcore { namespace tools { namespace verify {
	...
	namespace {
		...
		private:
			...
			static bool RunAnalyzer(const std::string& name, TArgs&&... args) {
				BITXORCORE_LOG(info) << "analyzing " << name;

				TAnalyzer analyzer(std::forward<TArgs>(args)...);
				auto errors = analyzer.analyze();
				if (errors.empty()) {
					BITXORCORE_LOG(important) << name << " analysis PASSED";
					return true;
				}

				BITXORCORE_LOG(error) << errors.str();
				return false;
			}
		};
...
int main(int argc, const char** argv) {
	bitxorcore::tools::verify::VerifyTool tool;
