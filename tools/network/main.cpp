...
#include "bitxorcore/api/RemoteNodeApi.h"
#include "bitxorcore/extensions/RemoteDiagnosticApi.h"
#include "bitxorcore/utils/Functional.h"
#include <algorithm>
#include <cctype>

namespace bitxorcore { namespace tools { namespace network {
	...
	namespace {
		...
		void PrettyPrint(const std::vector<NodeInfoPointer>& nodeInfos) {
			...
			BITXORCORE_LOG(info) << "--- NODE INFO for known peers ---";
			for (const auto& pNodeInfo : nodeInfos)
				BITXORCORE_LOG(info) << "> " << pNodeInfo->Local << Summarize(pNodeInfo->Remote, pNodeInfo->RemoteIdentity);

			// 2. output summary information of peers reported by each node
			BITXORCORE_LOG(info) << "--- PARTNER INFO for known peers ---";
			for (const auto& pNodeInfo : nodeInfos)
				BITXORCORE_LOG(info) << "> " << pNodeInfo->Local << SummarizePartners(*pNodeInfo, identityNameMap);

			// 3. output table (only for static nodes)
			PeersTableGenerator generator(nodeInfos, maxNameSize);
			BITXORCORE_LOG(info) << "--- PARTNER INFO TABLE for known peers ---";
			BITXORCORE_LOG(info) << std::endl << generator.str();
		}
...
int main(int argc, const char** argv) {
	bitxorcore::tools::network::NetworkTool tool;
