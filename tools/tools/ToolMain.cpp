...
#include "bitxorcore/config/LoggingConfiguration.h"
...
namespace bitxorcore { namespace tools {
	...
	int ToolMain(int argc, const char** argv, Tool& tool) {
		...
		auto pLoggingGuard = bitxorcore::tools::SetupLogging(LoadLoggingConfiguration(options.LoggingConfigurationPath));
		std::cout << std::endl;
