...
#include "bitxorcore/utils/DiagnosticCounterId.h"
...
namespace bitxorcore { namespace tools { namespace health {
	...
	namespace {
		...
		void PrettyPrint(const std::vector<NodeInfoPointer>& nodeInfos) {
			BITXORCORE_LOG(info) << "--- COUNTERS for known peers ---";
			for (const auto& pNodeInfo : nodeInfos)
				PrettyPrintCounters(*pNodeInfo);

			BITXORCORE_LOG(info) << "--- SUMMARY for known peers ---";
...
int main(int argc, const char** argv) {
	bitxorcore::tools::health::HealthTool tool;
