...
namespace bitxorcore { namespace tools { namespace health {
...
namespace boost { namespace asio {
	...
	template<>
	struct is_match_condition<bitxorcore::tools::health::BalancedBraceMatcher> : public boost::true_type {};
}}
...
namespace bitxorcore { namespace tools { namespace health {
	...
	namespace {
		...
		bool ShouldAbort(const boost::system::error_code& ec, const std::string& host, const char* operation) {
			...
			BITXORCORE_LOG(error) << "failed when " << operation << " '" << host << "': " << ec.message();
			return true;
		}
...
#pragma warning(pop)
		...
		private:
			...
			void handleConnect(const boost::system::error_code& ec) {
				...
				BITXORCORE_LOG(info) << "connected to " << m_host << " [" << m_endpoint << "]";
		...
		private:
			void complete(Result result) {
				...
				m_promise.set_exception(std::make_exception_ptr(bitxorcore_runtime_error(out.str().c_str())));
			}
