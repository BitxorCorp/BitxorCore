...
#include "bitxorcore/builders/AccountKeyLinkBuilder.h"
#include "bitxorcore/builders/VotingKeyLinkBuilder.h"
#include "bitxorcore/builders/VrfKeyLinkBuilder.h"
#include "bitxorcore/extensions/TransactionExtensions.h"
#include "bitxorcore/io/RawFile.h"
#include "bitxorcore/utils/HexParser.h"
#include <filesystem>

namespace bitxorcore { namespace tools { namespace linker {
	...
	namespace {
		...
		public:
			...
			int run(const Options& options) override {
				...
				BITXORCORE_LOG(info) << "saved transaction from " << signer.publicKey() << " with hash: " << transactionHash;
				return 0;
			}
		...
		private:
			void validateOptions(const Options& options) {
				if (options["secret"].empty())
					BITXORCORE_THROW_INVALID_ARGUMENT("missing secret key");

				if (options["output"].empty())
					BITXORCORE_THROW_INVALID_ARGUMENT("missing output name path");

				if (std::filesystem::exists(options["output"].as<std::string>()))
					BITXORCORE_THROW_INVALID_ARGUMENT("output file already exists");
			}
		};
	}
}}}
...
int main(int argc, const char** argv) {
	bitxorcore::tools::linker::LinkerTool tool;
