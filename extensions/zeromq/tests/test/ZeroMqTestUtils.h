...
#include "bitxorcore/model/TransactionRegistry.h"
...
namespace bitxorcore { namespace test {
	...
	public:
		...
		explicit MqContext(const std::string& listenInterface = std::string("127.0.0.1"))
				...
				, m_pZeroMqEntityPublisher(std::make_shared<zeromq::ZeroMqEntityPublisher>(
						...
						model::CreateNotificationPublisher(m_registry, UnresolvedTokenId(), Height())))
	...
	public:
		...
		void waitForReceiveSuccess() {
			...
			for (auto i = 0u; i < 10; ++i) {
				receiveSuccess = message.recv(m_zmqSocket);
				if (receiveSuccess)
					BITXORCORE_LOG(info) << "received message with " << message.size() << " parts while emptying message queue";

				test::Sleep(5);
			}

			if (Max_Attempts <= counter)
				BITXORCORE_LOG(warning) << "waitForReceiveSuccess failed with " << counter << " attempts";
		}
	...
	public:
		...
		explicit MqContextT(const SubscriberCreator& subscriberCreator)
				: m_pNotificationPublisher(model::CreateNotificationPublisher(registry(), UnresolvedTokenId(), Height()))
