...
#include "bitxorcore/model/TransactionUtils.h"
...
namespace bitxorcore { namespace zeromq {
	...
	public:
		...
		void flush(zmq::socket_t& zmqSocket) {
			...
			if (!result)
				BITXORCORE_LOG(warning) << m_errorMessageGenerator();
		}
	...
	void ZeroMqEntityPublisher::publish(
		...
		if (addresses.empty())
			BITXORCORE_LOG(warning) << "no addresses are associated with transaction " << transactionInfo.EntityHash;
