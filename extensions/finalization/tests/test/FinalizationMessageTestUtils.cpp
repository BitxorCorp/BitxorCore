...
namespace bitxorcore { namespace test {
	...
	std::vector<AccountKeyPairDescriptor> AddAccountsWithBalances(
		...
		for (auto balance : balances) {
			...
			accountState.Balances.credit(tokenId, balance);
		}
	...
	namespace {
		...
		std::vector<AccountKeyPairDescriptor> AddAccountsWithBalances(
			...
			auto keyPairDescriptors = test::AddAccountsWithBalances(*delta, height, Harvesting_Token_Id, balances);
			cache.commit();
			return keyPairDescriptors;
		}
	}
	...
	std::pair<model::FinalizationContext, std::vector<AccountKeyPairDescriptor>> CreateFinalizationContext(
			...
			Height height,
			const std::vector<Amount>& balances) {
		auto accountStateCacheOptions = CreateDefaultAccountStateCacheOptions(TokenId(1111), Harvesting_Token_Id);
