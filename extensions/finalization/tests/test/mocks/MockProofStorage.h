...
namespace bitxorcore { namespace mocks {
	...
	public:
		/// Creates a proof storage initialized with a genesis proof.
	...
	public:
		...
		std::shared_ptr<const model::FinalizationProof> loadProof(FinalizationEpoch epoch) const override {
			if (FinalizationEpoch() == epoch || epoch > m_epoch)
				BITXORCORE_THROW_INVALID_ARGUMENT("epoch must be nonzero and no greater than finalizationPoint");
		...
		std::shared_ptr<const model::FinalizationProof> loadProof(Height height) const override {
			if (Height() == height || height > m_height)
				BITXORCORE_THROW_INVALID_ARGUMENT("height must be nonzero and no greater than finalizedHeight");
