...
namespace bitxorcore { namespace test {
	...
	private:
		...
		private:
			...
			static void FakeFinalizationHeightMapping(const std::string& destination, uint64_t numFinalizationPoints) {
				const std::string genesisDirectory = "/00000";
				const std::string genesisHashFilename = destination + genesisDirectory + "/proof.heights.dat";
	...
	private:
		static void AssertCannotSaveProofAtFinalizationRound(const model::FinalizationRound& newFinalizationRound) {
			...
			// Act + Assert:
			EXPECT_THROW(pStorage->saveProof(*pProof), bitxorcore_invalid_argument);
		}
	...
	public:
		...
		static void AssertCannotLoadProofAtFinalizationEpochZero() {
			...
			// Act + Assert:
			EXPECT_THROW(pStorage->loadProof(FinalizationEpoch()), bitxorcore_invalid_argument);
		}
		...
		static void AssertCannotLoadProofAtFinalizationEpochGreaterThanCurrentFinalizationEpoch() {
			...
			// Act + Assert:
			EXPECT_THROW(pStorage->loadProof(FinalizationEpoch(11)), bitxorcore_invalid_argument);
		}
		...
		static void AssertCannotLoadProofAtHeightZero() {
			...
			// Act + Assert:
			EXPECT_THROW(pStorage->loadProof(Height(0)), bitxorcore_invalid_argument);
		}
		...
		static void AssertCannotLoadProofAtHeightWithoutProofWhenSingleProofIsSaved() {
			...
			for (auto height : { Height(2), Height(49), Height(50), Height(51), Height(100) })
				EXPECT_THROW(pStorage->loadProof(height), bitxorcore_invalid_argument) << height;
		}
