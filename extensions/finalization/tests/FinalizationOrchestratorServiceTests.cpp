	...
	namespace {
		...
		public:
			...
			explicit TestContext(const TestContextOptions& options, VoterType voterType = VoterType::Large1)
				...
				auto dataDirectory = config::BitxorCoreDataDirectory(userConfig.DataDirectory);
		...
		public:
			...
			auto votingStatus() {
				const auto& userConfig = testState().state().config().User;
				auto votingStatusFilename = config::BitxorCoreDataDirectory(userConfig.DataDirectory).rootDir().file("voting_status.dat");
		...
		private:
			...
			static void SeedVotingPrivateKeyTree(const config::BitxorCoreDirectory& directory, const crypto::VotingKeyPair& votingKeyPair) {
			...
			static void SeedVotingStatus(const config::BitxorCoreDirectory& directory, const model::FinalizationRound& round) {
