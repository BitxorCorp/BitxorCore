...
namespace bitxorcore { namespace chain {
	...
	void MultiRoundMessageAggregatorModifier::setMaxFinalizationRound(const model::FinalizationRound& round) {
		if (m_state.MinFinalizationRound > round)
			BITXORCORE_THROW_INVALID_ARGUMENT("cannot set max finalization round below min");

		m_state.MaxFinalizationRound = round;

		BITXORCORE_LOG(trace) << "set max finalization round to " << m_state.MaxFinalizationRound;
	}
	...
	void MultiRoundMessageAggregatorModifier::prune(FinalizationEpoch epoch) {
		...
		BITXORCORE_LOG(trace) << "set min finalization round to " << m_state.MinFinalizationRound << " after pruning at epoch " << epoch;
	}
	...
	MultiRoundMessageAggregator::MultiRoundMessageAggregator(
		...
		BITXORCORE_LOG(debug) << "creating multi round message aggregator starting at round " << round;
	}
