...
namespace bitxorcore { namespace chain {
	...
	bool RoundContext::isCompletable() const {
		auto bestPrevoteResultPair = tryFindBestPrevote();
		if (!bestPrevoteResultPair.second) {
			BITXORCORE_LOG(trace) << "not completable - no best prevote";
			return false;
		}
		...
		if (estimateResultPair.second && bestPrevoteResultPair.first != estimateResultPair.first) {
			BITXORCORE_LOG(trace) << "completable - Erv < g(Vrv)";
			return true;
		}
		...
		BITXORCORE_LOG(trace) << "completable - Erv == g(Vrv) and no descendant can reach g(Crv)";
		return true;
	}
