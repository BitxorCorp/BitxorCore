	...
	namespace {
		...
		public:
			DefaultFinalizationStageAdvancer(
				...
				BITXORCORE_LOG(debug) << "creating finalization stage advancer for round " << round << " at " << time;
			}

		public:
			bool canSendPrevote(Timestamp time) const override {
				if (m_timer.isElapsed(time, 1)) {
					BITXORCORE_LOG(trace) << "can send prevote - time elapsed";
					return true;
				}

				return requireRoundContext([](const auto&, const auto& roundContext) {
					if (roundContext.isCompletable()) {
						BITXORCORE_LOG(trace) << "can send prevote - completable";
						return true;
					}

					BITXORCORE_LOG(trace) << "cannot send prevote - not completable";
					return false;
				});
			}
			...
			bool canSendPrecommit(Timestamp time, model::HeightHashPair& target) const override {
				return requireRoundContext([this, time, &target](const auto& messageAggregatorView, const auto& roundContext) {
					...
					if (!bestPrevoteResultPair.second) {
						BITXORCORE_LOG(trace) << "cannot send precommit - no best prevote";
						return false;
					}
					...
					if (m_timer.isElapsed(time, 2)) {
						BITXORCORE_LOG(trace) << "can send precommit - time elapsed";
					} else if (roundContext.isCompletable()) {
						BITXORCORE_LOG(trace) << "can send precommit - completable";
					} else {
						BITXORCORE_LOG(trace) << "cannot send precommit - neither time elapsed nor completable";
						return false;
					}
			...
			bool canStartNextRound() const override {
				return requireRoundContext([](const auto&, const auto& roundContext) {
					if (roundContext.isCompletable()) {
						BITXORCORE_LOG(trace) << "can start next round - completable";
						return true;
					}

					BITXORCORE_LOG(trace) << "cannot start next round - not completable";
					return false;
				});
			}
