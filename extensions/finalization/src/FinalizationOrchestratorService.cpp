...
#include "bitxorcore/crypto_voting/AggregateBmPrivateKeyTree.h"
...
namespace bitxorcore { namespace finalization {
	...
	namespace {
		...
		void SaveMessageToDisk(const config::BitxorCoreDirectory& dataDirectory, const model::FinalizationMessage& message) {
		...
		public:
			void poll(Timestamp time) {
				...
				if (EpochStatus::Advance == epochStatusResult.first) {
					...
					BITXORCORE_LOG(debug) << "advancing to epoch " << orchestratorRound;
				}
		...
		private:
			...
			static crypto::AggregateBmPrivateKeyTree CreateVotingPrivateKeyTree(const config::UserConfiguration& userConfig) {
				auto factory = CreateBmPrivateKeyTreeFactory(config::BitxorCoreDirectory(userConfig.VotingKeysDirectory));
			...
			static supplier<std::unique_ptr<crypto::BmPrivateKeyTree>> CreateBmPrivateKeyTreeFactory(
				...
				return [treeSequenceId, pKeyTreeStream, directory]() mutable {
					...
					BITXORCORE_LOG(debug) << "loading voting private key tree from " << keyTreeFilename;

					if (!std::filesystem::exists(keyTreeFilename)) {
						BITXORCORE_LOG(error) << "could not load voting private key tree from " << keyTreeFilename;
		...
		private:
			...
			config::BitxorCoreDirectory m_dataDirectory;
