...
#include "bitxorcore/chain/ProcessingNotificationSubscriber.h"
...
namespace bitxorcore { namespace harvesting {
	...
	private:
		class CacheFacade {
		public:
			explicit CacheFacade(const cache::BitxorCoreCache& cache)
	...
	public:
		...
		std::unique_ptr<model::Block> commit(const model::BlockHeader& blockHeader, const model::Transactions& transactions) {
			if (!m_pCacheFacade)
				BITXORCORE_THROW_INVALID_ARGUMENT("facade has already committed a block");
			...
			for (const auto& transaction : pBlock->Transactions()) {
				...
				if (Amount(0) != surplus) {
					...
					state::ApplyFeeSurplus(accountStateIter.get(), { m_blockchainConfig.CurrencyTokenId, surplus }, importanceHeight);
				}
			}
	...
	void HarvestingUtFacade::unapply() {
		if (m_transactionInfos.empty())
			BITXORCORE_THROW_OUT_OF_RANGE("cannot call unapply when no transactions have been applied");
	...
	std::unique_ptr<model::Block> HarvestingUtFacade::commit(const model::BlockHeader& blockHeader) {
		if (height() != blockHeader.Height)
			BITXORCORE_THROW_RUNTIME_ERROR("commit block header is inconsistent with facade state");
