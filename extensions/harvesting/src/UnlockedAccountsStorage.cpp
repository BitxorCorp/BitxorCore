...
namespace bitxorcore { namespace harvesting {
	...
	namespace {
		...
		size_t ReadLastRequestIdentifier(const std::string& filename, HarvestRequestIdentifier& requestIdentifier) {
			...
			if (0 != input.size() % encryptedPayloadSize) {
				BITXORCORE_LOG(warning) << filename << " is corrupt with size (" << input.size() << ")";
				BITXORCORE_THROW_RUNTIME_ERROR_1("file contains incomplete encrypted payloads", filename);
			}
		...
		void TryRemoveLastRequest(const std::string& filename, const HarvestRequestIdentifier& expectedRequestIdentifier) {
			if (!std::filesystem::exists(filename)) {
				BITXORCORE_LOG(warning) << filename << " does not exist";
				return;
			}

			if (0 == std::filesystem::file_size(filename)) {
				BITXORCORE_LOG(debug) << filename << " is empty";
				return;
			}
	...
	void UnlockedAccountsStorage::add(
		...
		if (HarvestRequest::EncryptedPayloadSize() != encryptedPayload.Size)
			BITXORCORE_THROW_INVALID_ARGUMENT_1("encrypted payload has invalid size", encryptedPayload.Size);

		if (contains(requestIdentifier))
			BITXORCORE_THROW_INVALID_ARGUMENT_1("cannot add same request identifier to storage multiple times", requestIdentifier);
	...
	void UnlockedAccountsStorage::remove(const HarvestRequestIdentifier& requestIdentifier) {
		...
		if (!tryRemoveRequest(requestIdentifier)) {
			BITXORCORE_LOG(debug) << "cannot remove request identifier " << requestIdentifier << " because it is not in map";
			return;
		}
	}
	...
	void UnlockedAccountsStorage::load(
		...
		while (inputFile.position() != inputFile.size()) {
			...
			if (!decryptedPair.second)
				BITXORCORE_THROW_RUNTIME_ERROR("malformed harvesters file");
		...
		BITXORCORE_LOG(info) << "loading done, closing file";
	}
