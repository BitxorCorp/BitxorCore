...
#define TEST_CLASS ScheduledHarvesterTaskTests
	...
	namespace {
		...
		void AddStatistic(cache::BitxorCoreCache& cache, const model::Block& block) {
		...
		KeyPair AddImportantAccount(cache::BitxorCoreCache& cache) {
		...
		struct HarvesterContext {
			HarvesterContext(const model::Block& lastBlock)
					...
					, Cache(test::CreateEmptyBitxorCoreCache(Config))
