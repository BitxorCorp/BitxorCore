...
#define TEST_CLASS UnlockedAccountsStorageTests
	...
	TEST(TEST_CLASS, AddThrowsWhenRequestHasInvalidSize) {
		...
		EXPECT_THROW(storage.add(requestIdentifier, encryptedBuffer, keyPair.publicKey()), bitxorcore_invalid_argument);
	}
	...
	namespace {
		void AssertRemoveThrowsWhenInputHasIncompleteEncryptedPayloads(size_t numEncryptedPayloads) {
			...
			// Act + Assert:
			EXPECT_THROW(storage.remove(test::GetRequestIdentifier(*encryptedPayloads.cbegin())), bitxorcore_runtime_error);
		}
	}
	...
	TEST(TEST_CLASS, CannotLoadWhenInputDoesNotHaveEnoughData) {
		...
		// Act + Assert:
		UnlockedAccountsStorage storage(guard.name());
		EXPECT_THROW(storage.load(keyPair, [](const auto&) {}), bitxorcore_file_io_error);
	}
	...
	TEST(TEST_CLASS, LoadThrowsWhenDataIsInvalid) {
		...
		// Act + Assert:
		UnlockedAccountsStorage storage(guard.name());
		EXPECT_THROW(storage.load(keyPair, [](const auto&) {}), bitxorcore_runtime_error);
	}
