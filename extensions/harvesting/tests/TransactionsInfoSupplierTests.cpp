	...
	namespace {
		...
		public:
			explicit TestContext(TransactionSelectionStrategy strategy, uint32_t utCacheSize = 0)
					: m_bitxorcoreCache(test::CreateBitxorCoreCacheWithMarkerAccount(Height(7)))
					, m_utFacadeFactory(m_bitxorcoreCache, CreateBlockchainConfiguration(), m_executionConfig.Config, EmptyHashSupplier)
