...
#define TEST_CLASS HarvestingUtFacadeFactoryTests
	...
	namespace {
		...
		void SetDependentState(cache::BitxorCoreCache& bitxorcoreCache) {
			...
			bitxorcoreCache.commit(Default_Height);
		}
		...
		void RunUtFacadeTest(TAction action) {
			...
			auto bitxorcoreCache = test::CreateBitxorCoreCacheWithMarkerAccount(Default_Height);
			SetDependentState(bitxorcoreCache);

			test::MockExecutionConfiguration executionConfig;
			HarvestingUtFacadeFactory factory(bitxorcoreCache, CreateBlockchainConfiguration(), executionConfig.Config, EmptyHashSupplier);
	...
	TEST(TEST_CLASS, CannotUnapplyTransactionsWhenEmpty) {
		...
		RunUtFacadeTest([](auto& facade, const auto&) {
			...
			// Act + Assert:
			EXPECT_THROW(facade.unapply(), bitxorcore_out_of_range);
		});
	}
	...
	namespace {
		...
		auto CreateAccounts(cache::AccountStateCacheDelta& cache, Height height, const std::vector<Amount>& balances) {
			...
			for (auto i = 0u; i < balances.size(); ++i) {
				...
				accountState.Balances.credit(Harvesting_Token_Id, balances[i]);
		...
		auto RunCommitSetsImportanceHeaderFields(
			...
			auto bitxorcoreCache = test::CreateEmptyBitxorCoreCache(config);
			{
				auto cacheDelta = bitxorcoreCache.createDelta();
				CreateAccounts(cacheDelta.sub<cache::AccountStateCache>(), importanceMultipleHeight, balances);
				bitxorcoreCache.commit(importanceMultipleHeight - Height(1));
			}

			test::MockExecutionConfiguration executionConfig;
			HarvestingUtFacadeFactory factory(bitxorcoreCache, config, executionConfig.Config, [](auto height) {
	...
	namespace {
		...
		private:
			void notify(const model::BalanceTransferNotification& notification, observers::ObserverContext& context) const {
				...
				auto tokenId = TokenId(notification.TokenId.unwrap());
				if (observers::NotifyMode::Commit == context.Mode)
					Transfer(senderState, recipientState, tokenId, notification.Amount);
				else
					Transfer(recipientState, senderState, tokenId, notification.Amount);
			}
		};
		...
		public:
			void publish(const model::WeakEntityInfo& entityInfo, model::NotificationSubscriber& subscriber) const override {
				...
				subscriber.notify(model::BalanceTransferNotification(
						...
						UnresolvedTokenId(Harvesting_Token_Id.unwrap()),
	...
	TEST(TEST_CLASS, CommitFailsWhenBlockHeaderIsInconsistentWithCacheState) {
		...
		RunUtFacadeTest(4, [](auto& facade, const auto& transactionInfos, const auto&) {
			...
			EXPECT_THROW(facade.commit(*pBlockHeader), bitxorcore_runtime_error);
		});
	}
	...
	TEST(TEST_CLASS, CommitCannotBeCalledMultipleTimes) {
		...
		RunUtFacadeTest(0, [](auto& facade, const auto&, const auto&) {
			...
			EXPECT_THROW(facade.commit(*pBlockHeader), bitxorcore_invalid_argument);
		});
	}
	...
	namespace {
		...
		public:
			FacadeTestContext(
					...
					, m_height(height)
					, m_cache(test::CreateEmptyBitxorCoreCache(m_config, CreateCacheConfiguration(m_dbDirGuard.name()))) {
	...
	namespace {
		...
		public:
			explicit TestStateHashCalculator(const cache::BitxorCoreCache& cache)
		...
		public:
			void creditSurpluses(
				...
				for (auto& transactionInfo : transactionInfos) {
					...
					accountState.Balances.credit(Currency_Token_Id, surplus);
		...
		private:
			cache::BitxorCoreCacheDetachableDelta m_cacheDetachableDelta;
			cache::BitxorCoreCacheDetachedDelta m_cacheDetachedDelta;
			std::unique_ptr<cache::BitxorCoreCacheDelta> m_pCacheDelta;
		};
	...
	TEST(TEST_CLASS, CommitAcquiresResourcesInCorrectOrderSoAsToNotDeadlockWithBlockchainSyncConsumer) {
		...
		auto hashSupplier = [&blockStorage = *pBlockStorage](auto) {
			BITXORCORE_LOG(debug) << "harvester: acquire storage read lock";
		...
		test::MockExecutionConfiguration executionConfig;
		HarvestingUtFacadeFactory factory(bitxorcoreCache, config, executionConfig.Config, hashSupplier);

		auto pFacade = factory.create(Default_Time);
		ASSERT_TRUE(!!pFacade);

		// - simulate consumer
		auto consumerThread = std::thread([&bitxorcoreCache, &blockStorage = *pBlockStorage]() {
			BITXORCORE_LOG(debug) << "consumer: acquire storage write lock";
			auto blockStorageModifier = blockStorage.modifier();
			test::Pause();

			BITXORCORE_LOG(debug) << "consumer: acquire cache write lock";
			auto cacheDelta = bitxorcoreCache.createDelta();
			bitxorcoreCache.commit(Height(1));
		});

		// Act: create a block without deadlock
		BITXORCORE_LOG(debug) << "harvester: acquire cache read lock";
		auto pBlockHeader = CreateImportanceBlockHeader(importanceMultipleHeight);
		auto pBlock = pFacade->commit(*pBlockHeader);
		BITXORCORE_LOG(debug) << "harvester: created block";
