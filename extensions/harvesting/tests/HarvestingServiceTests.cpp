...
#define TEST_CLASS HarvestingServiceTests
	...
	namespace {
		...
		public:
			...
			explicit TestContext(const HarvestingConfiguration& config)
					: BaseType(test::CreateEmptyBitxorCoreCache(test::CreatePrototypicalBlockchainConfiguration()))
					, m_config(config) {
				setHooks();
			}
	...
	TEST(TEST_CLASS, UnlockedAccountsServiceIsLoadingUnlockedHarvestersFile) {
		...
		auto filename = config::BitxorCoreDataDirectory(directoryGuard.name()).rootDir().file("harvesters.dat");
