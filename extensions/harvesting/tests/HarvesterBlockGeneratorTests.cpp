	...
	namespace {
		...
		public:
			explicit TestContext(model::TransactionSelectionStrategy strategy)
					...
					, m_bitxorcoreCache(test::CreateEmptyBitxorCoreCache(m_config, CreateCacheConfiguration(m_dbDirGuard.name())))
					, m_transactionRegistry(mocks::CreateDefaultTransactionRegistry(mocks::PluginOptionFlags::Contains_Embeddings))
					, m_utFacadeFactory(m_bitxorcoreCache, m_config, m_executionConfig.Config, [](auto) { return Hash256(); })
				...
				m_bitxorcoreCache.commit(Cache_Height);
			}
		...
		public:
			...
			Hash256 calculateExpectedStateHash(const std::vector<Amount>& transactionSignerBalances) const {
				if (m_transactionInfos.size() != transactionSignerBalances.size())
					BITXORCORE_THROW_INVALID_ARGUMENT("unexpected number of transaction signer balances");

				auto cacheDetachableDelta = m_bitxorcoreCache.createDetachableDelta();
				...
				for (auto i = 0u; i < transactionSignerBalances.size(); ++i) {
					...
					accountStateCacheDelta.find(signer).get().Balances.credit(m_config.CurrencyTokenId, balance);
				}
		...
		private:
			static model::BlockchainConfiguration CreateBlockchainConfiguration() {
				...
				config.CurrencyTokenId = TokenId(123);
