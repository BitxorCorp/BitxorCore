...
using bitxorcore::crypto::KeyPair;
...
#define TEST_CLASS HarvesterTests
	...
	namespace {
		...
		constexpr auto Harvesting_Token_Id = TokenId(1234);
		...
		public:
			...
			explicit HarvesterContext(Height height)
					: Cache(test::CreateEmptyBitxorCoreCache(CreateConfiguration()))
		...
		private:
			static void CreateAccounts(
				...
				for (auto i = 0u; i < Num_Accounts; ++i) {
					...
					accountState.Balances.credit(Harvesting_Token_Id, Min_Voter_Balance + Amount(multiplier * (i + 1)));
	...
	TEST(TEST_CLASS, HarvesterRespectsCustomBlockchainConfiguration) {
		...
		BITXORCORE_LOG(debug) << "harvested blocks: H1 = " << numHarvester1Blocks << ", H2 = " << numHarvester2Blocks;
