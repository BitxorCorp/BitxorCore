...
#include "bitxorcore/chain/TransactionUpdateResultUtils.h"
#include "bitxorcore/chain/UtUpdater.h"
#include "bitxorcore/config/BitxorCoreDataDirectory.h"
#include "bitxorcore/consumers/AuditConsumer.h"
#include "bitxorcore/consumers/BlockConsumers.h"
#include "bitxorcore/consumers/ConsumerUtils.h"
#include "bitxorcore/consumers/ReclaimMemoryInspector.h"
#include "bitxorcore/consumers/TransactionConsumers.h"
#include "bitxorcore/consumers/UndoBlock.h"
#include "bitxorcore/crypto/SecureRandomGenerator.h"
...
#include "bitxorcore/extensions/NodeInteractionUtils.h"
...
using namespace bitxorcore::consumers;
using namespace bitxorcore::disruptor;
	...
	namespace {
		...
		std::unique_ptr<ConsumerDispatcher> CreateConsumerDispatcher(
			...
			auto inspector = [&nodeSubscriber, &statusSubscriber, &nodes = state.nodes(), &localNetworks, reclaimMemoryInspector](
				...
				if (ConsumerResultSeverity::Fatal == completionResult.ResultSeverity) {
					...
					if (config::IsLocalHost(identity.Host, localNetworks))
						BITXORCORE_LOG(debug) << "bypassing banning of " << identity << " because host is contained in local networks";
					else
			...
			if (config.Node.EnableDispatcherInputAuditing) {
				...
				BITXORCORE_LOG(debug) << "enabling auditing to " << auditPath;

				config::BitxorCoreDirectory(auditPath).createAll();
		...
		BlockchainProcessor CreateSyncProcessor(
			...
			BlockHitPredicateFactory blockHitPredicateFactory = [&blockchainConfig](const cache::ReadOnlyBitxorCoreCache& cache) {
		...
		BlockchainSyncHandlers CreateBlockchainSyncHandlers(extensions::ServiceState& state, RollbackInfo& rollbackInfo) {
			...
			syncHandlers.DifficultyChecker = [&rollbackInfo, blockchainConfig](const auto& blocks, const cache::BitxorCoreCache& cache) {
			...
			auto dataDirectory = config::BitxorCoreDataDirectory(state.config().User.DataDirectory);
			...
			state.hooks().setTransactionRangeConsumerFactory([shouldProcessTransactions, &dispatcher, &nodes](auto source) {
				return [shouldProcessTransactions, &dispatcher, &nodes, source](auto&& range) {
					if (!shouldProcessTransactions()) {
						BITXORCORE_LOG(trace) << "ignoring push due to should process transactions predicate";
						return;
					}

					if (nodes.view().isBanned(range.SourceIdentity)) {
						BITXORCORE_LOG(trace) << "ignoring push from banned node " << range.SourceIdentity;
						return;
					}
