...
#define TEST_CLASS PredicateUtilsTests
	...
	namespace {
		...
		config::BitxorCoreConfiguration CreateBitxorCoreConfigurationFromSettings(const ThrottleTestSettings& settings) {
		...
		chain::UtUpdater::ThrottleContext CreateThrottleContext(
				const cache::ReadOnlyBitxorCoreCache& readOnlyBitxorCoreCache,
				const cache::UtCacheModifierProxy& utCacheModifier) {
			return { chain::UtUpdater::TransactionSource::New, Height(), readOnlyBitxorCoreCache, utCacheModifier };
		}
		...
		void AssertUtUpdaterThrottleResult(
			...
			// Arrange:
			auto config = CreateBitxorCoreConfigurationFromSettings(settings);

			// - create a read only bitxorcore cache (importance grouping needs to be nonzero)
			auto bitxorcoreCache = test::CreateEmptyBitxorCoreCache(config.Blockchain);
			auto bitxorcoreCacheView = bitxorcoreCache.createView();
			auto readOnlyBitxorCoreCache = bitxorcoreCacheView.toReadOnly();
			...
			auto throttleContext = CreateThrottleContext(readOnlyBitxorCoreCache, utCacheModifier);
