...
#define TEST_CLASS TransactionSpamThrottleTests
	...
	namespace {
		...
		cache::BitxorCoreCache CreateBitxorCoreCacheWithImportanceGrouping(uint64_t importanceGrouping) {
			auto blockchainConfig = model::BlockchainConfiguration::Uninitialized();
			blockchainConfig.ImportanceGrouping = importanceGrouping;
			return test::CreateEmptyBitxorCoreCache(blockchainConfig);
		}
		...
		class TestContext {
		public:
			TestContext(cache::BitxorCoreCache&& bitxorcoreCache, Height height)
					: m_bitxorcoreCache(std::move(bitxorcoreCache))
					, m_bitxorcoreCacheView(m_bitxorcoreCache.createView())
					, m_readOnlyBitxorCoreCache(m_bitxorcoreCacheView.toReadOnly())
					, m_height(height)
		...
		public:
			...
			chain::UtUpdater::ThrottleContext throttleContext(TransactionSource source = TransactionSource::New) const {
				return { source, m_height, m_readOnlyBitxorCoreCache, m_transactionsCacheModifier };
			}
		...
		private:
			...
			cache::BitxorCoreCacheView m_bitxorcoreCacheView;
			cache::ReadOnlyBitxorCoreCache m_readOnlyBitxorCoreCache;
			Height m_height;
		...
		void AssertThrottling(const ThrottleTestSettings& settings, bool expectedResult) {
			...
			auto bitxorcoreCache = CreateBitxorCoreCacheWithImportanceGrouping(100);
			std::vector<Key> publicKeys;
			Key signerPublicKey;
			{
			...
			TestContext context(std::move(bitxorcoreCache), Height(1));
		...
		void AssertThrottling(const ThrottleTestSettings& settings, AccountPolicy accountPolicy, uint32_t expectedCacheSize) {
			...
			while (!isFiltered && config.MaxCacheSize > utils::FileSize::FromBytes(cacheMemorySize)) {
				...
				if (!isFiltered && !context.transactionsCacheModifier().add(transactionInfo))
					BITXORCORE_THROW_RUNTIME_ERROR_1("transaction could not be added to cache at size", cacheSize);
