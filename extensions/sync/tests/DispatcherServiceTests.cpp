...
using ValidationResult = bitxorcore::validators::ValidationResult;
...
#define TEST_CLASS DispatcherServiceTests
	...
	namespace {
		...
		crypto::KeyPair GetBlockSignerKeyPair() {
			...
			// since the cache is setup in InitializeBitxorCoreCacheForDispatcherTests,
			// this account does not need to be a 'real' genesis account
		...
		crypto::KeyPair GetBlockSignerVrfKeyPair() {
			// random key is fine because signer vrf is configured in InitializeBitxorCoreCacheForDispatcherTests
			return crypto::KeyPair::FromString("1A8BF1F961C6EB875D7C909E314DDA6D43AFAC182745D449B0671B8C455806C3");
		}

		cache::BitxorCoreCache CreateBitxorCoreCacheForDispatcherTests() {
			...
			// create the cache
			return test::CreateEmptyBitxorCoreCache<test::CoreSystemCacheFactory>(config);
		}

		void InitializeBitxorCoreCacheForDispatcherTests(
			...
			// set a difficulty for the genesis block
		...
		std::shared_ptr<model::Block> CreateValidBlockForDispatcherTests(const crypto::KeyPair& signer) {
			...
			model::PreviousBlockContext context(*pGenesisBlockElement);
		...
		public:
			...
			explicit TestContext(const supplier<Timestamp>& timeSupplier)
					: BaseType(CreateBitxorCoreCacheForDispatcherTests(), timeSupplier)
				...
				InitializeBitxorCoreCacheForDispatcherTests(state.cache(), GetBlockSignerKeyPair(), GetBlockSignerVrfKeyPair().publicKey());
	...
	TEST(TEST_CLASS, SuccessfulRollbackChangesCommittedCounters) {
		// Arrange: create a chain with one block on top of genesis
		TestContext context;
		context.boot();
...
#define CONSUMER_FACTORY_TRAITS_BASED_TEST(TEST_NAME) \
	...
	namespace {
		model::TransactionRange CreateSignedTransactionEntityRange(size_t numTransactions) {
			...
			for (auto& transaction : range) {
				...
				extensions::TransactionExtensions(test::GetGenesisGenerationHashSeed()).sign(keyPair, transaction);
			}
	...
	TEST(TEST_CLASS, CanConsumeTransactionRange_ValidElement) {
		...
		extensions::TransactionExtensions(test::GetGenesisGenerationHashSeed()).sign(signer, *pValidTransaction);
	...
	TEST(TEST_CLASS, Dispatcher_TransactionRangeFromBannedNodeIsIgnored) {
		...
		extensions::TransactionExtensions(test::GetGenesisGenerationHashSeed()).sign(signer, *pTransaction);
