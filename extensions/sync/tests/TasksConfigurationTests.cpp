...
#define TEST_CLASS TasksConfigurationTests
	...
	TEST(TEST_CLASS, Tasks_LoadFromBagFailsWhenAnyTaskHasExtraValue) {
		...
		// Act + Assert:
		EXPECT_THROW(TasksConfiguration::LoadFromBag(bag), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, Tasks_LoadFromPathFailsWhenFileDoesNotExist) {
		...
		EXPECT_THROW(TasksConfiguration::LoadFromPath("../no-resources"), bitxorcore_runtime_error);
	}
