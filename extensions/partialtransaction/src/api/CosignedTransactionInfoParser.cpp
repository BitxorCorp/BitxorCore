...
#include "bitxorcore/ionet/PacketReader.h"
...
#define BITXORCORE_LOG_PARSE_ERROR(DESC) BITXORCORE_LOG(warning) << "unable to parse '" << DESC << "'"
		...
		bool ReadCosignedTransactionInfo(
			...
			if (!pTag || !ReadZeros(reader, 6)) {
				BITXORCORE_LOG_PARSE_ERROR("tag");
				return false;
			}
			...
			if (*pTag & 0x8000) {
				...
				if (!pTransaction || !isValid(*pTransaction) || !ReadZeros(reader, utils::GetPaddingSize(pTransaction->Size, 8))) {
					BITXORCORE_LOG_PARSE_ERROR("transaction") << " (failed validation = " << !!pTransaction << ")";
					return false;
				}
				...
				if (!pHash) {
					BITXORCORE_LOG_PARSE_ERROR("hash");
					return false;
				}
			...
			for (uint16_t i = 0; i < numCosignatures; ++i) {
				...
				if (!pCosignature) {
					BITXORCORE_LOG_PARSE_ERROR("cosignature") << " at " << i;
					return false;
				}
...
#undef BITXORCORE_LOG_PARSE_ERROR
	}
