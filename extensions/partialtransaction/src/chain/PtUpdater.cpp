...
namespace bitxorcore { namespace chain {
	...
	public:
		thread::future<PtUpdateResult> update(const model::TransactionInfo& transactionInfo) {
			if (model::Entity_Type_Aggregate_Bonded != transactionInfo.pEntity->Type)
				BITXORCORE_THROW_INVALID_ARGUMENT("PtUpdater only supports bonded aggregate transactions");
	...
	private:
		...
		CheckEligibilityResult checkEligibility(const model::DetachedCosignature& cosignature) const {
			...
			BITXORCORE_LOG(debug) << "detected stale cosignature for transaction " << cosignature.ParentHash;
