...
#include "bitxorcore/validators/ValidatingNotificationSubscriber.h"
	...
	namespace {
		...
		public:
			Result<bool> validatePartial(const model::WeakEntityInfoT<model::Transaction>& transactionInfo) const override {
				...
				BITXORCORE_LOG_LEVEL(validators::MapToLogLevel(result)) << "partial transaction failed validation with " << result;
				return { result, false };
			}
