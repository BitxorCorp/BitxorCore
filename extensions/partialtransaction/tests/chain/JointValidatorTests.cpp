	...
	namespace {
		...
		public:
			...
			TestContext(const std::string& statelessName, const std::string& statefulName)
					...
					, m_cache(test::CreateBitxorCoreCacheWithMarkerAccount())
		...
		public:
			std::unique_ptr<const stateless::NotificationValidator> create(FailureMode failureMode = FailureMode::Default) {
				m_pluginManager.addTokenResolver([](const auto& cache, const auto& unresolved, auto& resolved) {
					resolved = TokenId(unresolved.unwrap() * (test::IsMarkedCache(cache) ? 2 : 0));
					return true;
				});
		...
		private:
			...
			void assertStatefulSingle(const model::Notification& notification) {
				...
				EXPECT_EQ(TokenId(2), params.ResolvedTokenIdOne);
			}
