...
#include "bitxorcore/cache_core/AccountStateCacheView.h"
...
namespace bitxorcore { namespace timesync {
	...
	TimeOffset TimeSynchronizer::calculateTimeOffset(
		...
		if (samples.empty()) {
			BITXORCORE_LOG(warning) << "no synchronization samples available to calculate network time";
			return TimeOffset(0);
		}
		...
		if (0 == cumulativeImportance) {
			BITXORCORE_LOG(warning) << "cannot calculate network time, cumulativeImportance is zero";
			return TimeOffset(0);
		}
	...
	double TimeSynchronizer::sumScaledOffsets(
		...
		return utils::Sum(samples, [&importanceView, height, scaling, totalChainImportance, warningThresholdMillis](const auto& sample) {
			...
			BITXORCORE_LOG_LEVEL(MapToLogLevel(warningThresholdMillis, offset))
