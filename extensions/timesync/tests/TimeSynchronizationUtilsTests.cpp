...
#define TEST_CLASS TimeSynchronizationUtilsTests
	...
	namespace {
		...
		public:
			thread::future<TimeSyncRequestResultPair> operator()(const ionet::Node& node) {
				if (m_index >= m_communicationTimestampsContainer.size())
					BITXORCORE_THROW_RUNTIME_ERROR("out of communication timestamps");
		...
		public:
			Timestamp operator()() {
				if (m_index / 2 >= m_communicationTimestampsContainer.size())
					BITXORCORE_THROW_RUNTIME_ERROR("out of network timestamps");
		...
		private:
			static cache::BitxorCoreCache CreateCache(Importance totalChainImportance) {
