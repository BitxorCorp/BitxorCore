...
namespace bitxorcore { namespace state { class MetadataEntry; } }
...
namespace bitxorcore { namespace test {
	...
	/// Token metadata traits for tests.
	struct TokenMetadataTestTraits {
		/// Number of expected document fields.
		...
		static void AssertAdditionalData(const TTransaction& transaction, const bsoncxx::document::view& view) {
			EXPECT_EQ(transaction.TargetTokenId, UnresolvedTokenId(GetUint64(view, "targetTokenId")));
		}
	};
