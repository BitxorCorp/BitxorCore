...
#define TEST_CLASS MongoAccountRestrictionCacheStorageTests
	...
	namespace {
		...
		struct AccountRestrictionCacheTraits {
			...
			static void Add(cache::BitxorCoreCacheDelta& delta, const ModelType& restrictions) {
				auto& restrictionCacheDelta = delta.sub<cache::AccountRestrictionCache>();
				restrictionCacheDelta.insert(restrictions);
			}

			static void Remove(cache::BitxorCoreCacheDelta& delta, const ModelType& restrictions) {
				auto& restrictionCacheDelta = delta.sub<cache::AccountRestrictionCache>();
				restrictionCacheDelta.remove(restrictions.address());
			}

			static void Mutate(cache::BitxorCoreCacheDelta& delta, ModelType& restrictions) {
				// update expected
