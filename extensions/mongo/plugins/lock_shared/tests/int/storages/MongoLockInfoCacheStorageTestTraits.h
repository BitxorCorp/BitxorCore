...
namespace bitxorcore { namespace mongo { namespace plugins {
	...
	struct MongoLockInfoCacheStorageTestTraits : public TLockInfoTraits {
		...
		static void Add(cache::BitxorCoreCacheDelta& delta, const typename TLockInfoTraits::ModelType& lockInfo) {
		...
		static void Remove(cache::BitxorCoreCacheDelta& delta, const typename TLockInfoTraits::ModelType& lockInfo) {
			auto& lockInfoCacheDelta = delta.sub<typename TLockInfoTraits::CacheType>();
			lockInfoCacheDelta.remove(TLockInfoTraits::GetId(lockInfo));
		}

		/// Modifies \a lockInfo in \a delta.
		static void Mutate(cache::BitxorCoreCacheDelta& delta, typename TLockInfoTraits::ModelType& lockInfo) {
			// update expected
