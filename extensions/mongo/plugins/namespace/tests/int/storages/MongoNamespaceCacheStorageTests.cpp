...
#define TEST_CLASS MongoNamespaceCacheStorageTests
	...
	namespace {
		...
		state::NamespaceAlias GetNamespaceAlias(NamespaceId id) {
			...
			switch (id.unwrap() % 3) {
			case 1:
				return state::NamespaceAlias(TokenId(3 * id.unwrap()));
		...
		struct NamespaceCacheTraits {
			...
			static void Add(cache::BitxorCoreCacheDelta& delta, const ModelType& descriptor) {
			...
			static void Remove(cache::BitxorCoreCacheDelta& delta, const ModelType& descriptor) {
				auto& namespaceCacheDelta = delta.sub<cache::NamespaceCache>();
	...
	struct NamespaceCacheRootModificationTraits : public NamespaceCacheTraits {
		static NamespaceDescriptor Mutate(cache::BitxorCoreCacheDelta& delta, ModelType& descriptor) {
			// change owner
