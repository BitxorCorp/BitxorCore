...
#define TEST_CLASS NamespaceDescriptorMapperTests
	...
	namespace {
		...
		struct TokenAliasTraits {
			static auto CreateAlias() {
				return state::NamespaceAlias(test::GenerateRandomValue<TokenId>());
			}
		};
...
#define ALIAS_TRAITS_BASED_TEST(TEST_NAME) \
	...
	TEST(TEST_CLASS, TEST_NAME##_TokenAlias) { TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<TokenAliasTraits>(); } \
