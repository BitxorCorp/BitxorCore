...
#define TEST_CLASS MongoBlockStorageTests
	...
	namespace {
		...
		struct BlockElementCounts {
			...
			size_t NumTokenResolutionStatements = 0;
			...
			void AddCounts(const model::BlockElement blockElement) {
				...
				if (blockElement.OptionalStatement) {
					...
					NumTokenResolutionStatements += blockElement.OptionalStatement->TokenResolutionStatements.size();
				}
			}

			size_t NumTotalStatements() const {
				return NumTransactionStatements + NumAddressResolutionStatements + NumTokenResolutionStatements;
			}
		};
		...
		void AssertCollectionSizes(const BlockElementCounts& blockElementCounts) {
			...
			test::AssertCollectionSize("tokenResolutionStatements", blockElementCounts.NumTokenResolutionStatements);
		}
		...
		void AssertBlockStatement(const model::BlockStatement& blockStatement, Height height, mongocxx::database& database) {
			...
			auto cursor3 = database["tokenResolutionStatements"].find(filter.view(), options2);
			AssertResolutionStatements<test::TokenResolutionTraits>(height, blockStatement.TokenResolutionStatements, cursor3);
		}
		...
		void AssertNoBlockStatement(Height height, mongocxx::database& database) {
			...
			EXPECT_EQ(0u, GetNumEntitiesAtHeight("tokenResolutionStatements", "height", height, database));
		}
		...
		model::TokenResolutionStatement CreateTokenResolutionStatement(UnresolvedTokenId tokenId, uint32_t index) {
			model::TokenResolutionStatement statement(tokenId);
			statement.addResolution(test::GenerateRandomValue<TokenId>(), model::ReceiptSource(index, index + 1));
			return statement;
		}
		...
		void AddStatements(model::BlockElement& blockElement, const BlockElementCounts& blockElementCounts) {
			...
			for (uint8_t i = 0u; i < blockElementCounts.NumTokenResolutionStatements; ++i) {
				UnresolvedTokenId tokenId(i);
				pBlockStatement->TokenResolutionStatements.emplace(tokenId, CreateTokenResolutionStatement(tokenId, i));
			}
	...
	TEST(TEST_CLASS, CanSaveBlockWithTransactionsAndOnlyTokenResolutionStatements) {
	...
	TEST(TEST_CLASS, CannotSaveSameBlockTwiceWhenErrorModeIsStrict) {
		...
		// Act + Assert:
		EXPECT_THROW(pStorage->saveBlock(blockElement), bitxorcore_invalid_argument);
	}
	...
	namespace {
		void AssertCannotSaveOutOfOrderBlock(MongoErrorPolicy::Mode errorPolicyMode) {
			...
			// Act + Assert:
			EXPECT_THROW(pStorage->saveBlock(blockElement2), bitxorcore_invalid_argument);
		}
	}
	...
	TEST(TEST_CLASS, CanDropBlocks_OneBatchToGenesis) {
