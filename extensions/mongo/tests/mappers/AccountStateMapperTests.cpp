...
#define TEST_CLASS AccountStateMapperTests
	...
	namespace {
		...
		auto CreateAccountState(Height publicKeyHeight, std::initializer_list<model::Token> tokens, const RandomSeed& seed) {
		...
		void AssertCanMapAccountState(
				...
				std::initializer_list<model::Token> tokens,
				const RandomSeed& seed = RandomSeed()) {
			// Arrange:
			auto accountState = CreateAccountState(publicKeyHeight, tokens, seed);
	...
	// region mapping - toggling public key and tokens

	TEST(TEST_CLASS, CanMapAccountStateWithNeitherPublicKeyNorTokens) {
		AssertCanMapAccountState(Height(0), {});
	}

	TEST(TEST_CLASS, CanMapAccountStateWithPublicKeyButWithoutTokens) {
		AssertCanMapAccountState(Height(456), {});
	}

	TEST(TEST_CLASS, CanMapAccountStateWithoutPublicKeyButWithSingleToken) {
		AssertCanMapAccountState(Height(0), { { TokenId(1234), Amount(234) } });
	}

	TEST(TEST_CLASS, CanMapAccountStateWithoutPublicKeyButWithMultipleTokens) {
		AssertCanMapAccountState(
				Height(0),
				{ { TokenId(1234), Amount(234) }, { TokenId(1357), Amount(345) }, { TokenId(31), Amount(45) } });
	}

	TEST(TEST_CLASS, CanMapAccountStateWithPublicKeyAndSingleToken) {
		AssertCanMapAccountState(Height(456), { { TokenId(1234), Amount(234) } });
	}

	TEST(TEST_CLASS, CanMapAccountStateWithPublicKeyAndMultipleTokens) {
		AssertCanMapAccountState(
				Height(456),
				{ { TokenId(1234), Amount(234) }, { TokenId(1357), Amount(345) }, { TokenId(31), Amount(45) } });
	}
	...
	TEST(TEST_CLASS, CanMapAccountStateWithSupplementalPublicKeys) {
		...
		ForEachRandomSeed([](const auto& seed) {
			// Act + Assert:
			AssertCanMapAccountState(Height(456), { { TokenId(1234), Amount(234) } }, seed);
		});
	}
