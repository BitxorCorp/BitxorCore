...
#include "bitxorcore/model/EmbeddedTransaction.h"
#include "bitxorcore/model/Receipt.h"
...
#define TRAIT_BASED_TO_INT_TEST(TEST_NAME) \
	...
	namespace {
		...
		void AssertDbBinaryToModelArrayFailsIfOutputArrayHasUnexpectedSize() {
			...
			EXPECT_THROW(DbBinaryToModelArray(output, dbBinary), bitxorcore_invalid_argument);
		}
	}
	...
	namespace {
		...
		void AssertCanStreamToken() {
			// Arrange:
			auto id = test::GenerateRandomValue<TTokenId>();
			auto amount = test::GenerateRandomValue<Amount>();

			// Act: serialize to mongo
			bson_stream::document builder;
			auto tokensArray = builder << "arr" << bson_stream::open_array;
			StreamToken(tokensArray, id, amount);
			...
			auto dbToken = dbArray.cbegin()->get_document().view();
			EXPECT_EQ(id, TTokenId(test::GetUint64(dbToken, "id")));
			EXPECT_EQ(amount, Amount(test::GetUint64(dbToken, "amount")));
		}
	}

	TEST(TEST_CLASS, CanStreamToken) {
		AssertCanStreamToken<TokenId>();
	}

	TEST(TEST_CLASS, CanStreamUnresolvedToken) {
		AssertCanStreamToken<UnresolvedTokenId>();
	}
