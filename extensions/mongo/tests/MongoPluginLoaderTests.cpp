...
#define TEST_CLASS MongoPluginLoaderTests
	...
	namespace {
		constexpr auto Known_Plugin_Name = "bitxorcore.mongo.plugins.transfer";
		...
		void AssertCannotLoadUnknownPlugin(const std::string& directory) {
			...
			RunPluginManagerTest([&directory, &modules](auto& manager) {
				// Act + Assert:
				EXPECT_THROW(LoadPluginByName(manager, modules, directory, "bitxorcore.mongo.plugins.awesome"), bitxorcore_invalid_argument);
			});
		}
	}
