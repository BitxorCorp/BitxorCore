...
namespace bitxorcore { namespace test {
	...
	namespace {
		...
		void AssertEqualAccountPublicKeys(
			...
			for (auto keyType = PublicKeyType::Linked; keyType <= PublicKeyType::All; Advance(keyType)) {
				...
				default:
					BITXORCORE_THROW_INVALID_ARGUMENT_1("unexpected key type in AccountPublicKeys", static_cast<uint16_t>(keyType));
				}
	...
	void AssertEqualAccountState(const state::AccountState& accountState, const bsoncxx::document::view& dbAccount) {
		...
		auto dbTokens = dbAccount["tokens"].get_array().value;
		size_t numTokens = 0;
		for (const auto& tokenElement : dbTokens) {
			auto tokenDocument = tokenElement.get_document();
			auto id = TokenId(GetUint64(tokenDocument.view(), "id"));
			EXPECT_EQ(accountState.Balances.get(id), Amount(GetUint64(tokenDocument.view(), "amount")));
			++numTokens;
		}

		EXPECT_EQ(accountState.Balances.size(), numTokens);
	}
