...
#define TEST_CLASS MongoReceiptPluginFactoryTests
	...
	TEST(TEST_CLASS, CanStreamBalanceTransferReceipt) {
		...
		EXPECT_EQ(receipt.Token.TokenId, TokenId(test::GetUint64(view, "tokenId")));
		EXPECT_EQ(receipt.Token.Amount, Amount(test::GetUint64(view, "amount")));
	...
	TEST(TEST_CLASS, CanStreamBalanceChangeReceipt) {
		...
		model::BalanceChangeReceipt receipt(model::ReceiptType(), test::GenerateRandomByteArray<Address>(), TokenId(234), Amount(345));
	...
	TEST(TEST_CLASS, CanStreamInflationReceipt) {
		...
		model::InflationReceipt receipt(model::ReceiptType(), TokenId(234), Amount(345));
	...
	TEST(TEST_CLASS, CreateArtifactExpiryReceiptMongoPluginRespectsSuppliedType) {
		// Act:
		auto pPlugin = CreateArtifactExpiryReceiptMongoPlugin<TokenId>(static_cast<model::ReceiptType>(1234));
	...
	TEST(TEST_CLASS, CanStreamArtifactExpiryReceipt) {
		// Arrange:
		auto pPlugin = CreateArtifactExpiryReceiptMongoPlugin<TokenId>(model::ReceiptType());
		bsoncxx::builder::stream::document builder;

		model::ArtifactExpiryReceipt<TokenId> receipt(model::ReceiptType(), TokenId(234));
		...
		EXPECT_EQ(receipt.ArtifactId, TokenId(test::GetUint64(view, "artifactId")));
	}
