...
namespace bitxorcore { namespace mongo {
	...
	public:
		...
		void add(std::unique_ptr<TStorage>&& pStorage) {
			...
			if (m_storages[id])
				BITXORCORE_THROW_INVALID_ARGUMENT_1("storage has already been registered with id", id);
		...
		std::unique_ptr<ExternalCacheStorage> build() {
			...
			BITXORCORE_LOG(debug) << "creating ExternalCacheStorage with " << storages.size() << " storages";
