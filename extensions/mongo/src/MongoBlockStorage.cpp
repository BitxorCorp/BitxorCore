	...
	namespace {
		...
		void SaveBlockHeader(
			...
			if (0 == result.inserted_count())
				BITXORCORE_THROW_RUNTIME_ERROR("SaveBlockHeader failed: block header was not inserted");
		}
		...
		void SaveBlockStatement(
			...
			numExpectedInserts.emplace_back(blockStatement.TokenResolutionStatements.size());
			futures.emplace_back(bulkWriter.bulkInsert("tokenResolutionStatements", blockStatement.TokenResolutionStatements, [height](
					const auto& pair,
					auto) {
				return mappers::ToDbModel(height, pair.second);
			}));
		...
		private:
			...
			void dropAllAfter(Height height) {
				BITXORCORE_LOG(debug) << "dropping blocks after " << height;
				...
				dropDocuments("tokenResolutionStatements", "statement.height", height);
			}
			...
			void dropDocuments(const std::string& collectionName, const std::string& indexName, Height height) {
				...
				if (result)
					BITXORCORE_LOG(debug) << "deleted " << result->deleted_count() << " " << collectionName;
				else
					BITXORCORE_THROW_RUNTIME_ERROR("delete returned empty result");
			}
