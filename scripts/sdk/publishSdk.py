...
def fix_includes(path):
	for line in FileInput(path, inplace=True):
		line = re.sub(r'#include "src/', '#include "bitxorcore/', line.rstrip())
		line = re.sub(r'#include ".*/src/', '#include "bitxorcore/', line.rstrip())
		line = re.sub(r'#include "plugins/.*/src/', '#include "bitxorcore/', line.rstrip())
		print(line)
...
class Publisher:
	def __init__(self, root_directory, publish_directory):
		...
		self.root_bitxorcore_directory = os.path.join(root_directory, 'src', 'bitxorcore')
		self.publish_directory = os.path.abspath(publish_directory)
		self.publish_bitxorcore_directory = os.path.join(publish_directory, 'bitxorcore')

		self.verbose = False
		self.headers = []
		self.source_directories = []

		self.register_source_directory(self.root_bitxorcore_directory)
	...
	def publish_component(self, component):
		...
		# (notice that self.root_bitxorcore_directory is the source_directory of all components)
		component_directory = os.path.join(self.root_bitxorcore_directory, component)
	...
	def publish_merged_file(self, filename):
		merged_path = os.path.join(self.publish_bitxorcore_directory, filename)
		with open(merged_path, 'w', encoding='utf8') as merged_file:
			self.headers.append(os.path.join('bitxorcore', filename))
	...
	def flush_master_header(self, exclusions=None):
		master_header_path = os.path.join(self.publish_bitxorcore_directory, 'bitxorcore.h')
	...
	def publish_headers(self, source_directory, component):
		...
		publish_component_directory = os.path.join(self.publish_bitxorcore_directory, component)
	...
	def publish_header(self, source_directory, component, header):
		self.headers.append(os.path.join('bitxorcore', component, header))

		source_file = os.path.join(source_directory, header)
		publish_file = os.path.join(self.publish_bitxorcore_directory, component, header)
...
def publish_all():
	parser = argparse.ArgumentParser(description='publishes the bitxorcore sdk')
	...
	transactions = [
		'account_link', 'aggregate', 'lock_hash', 'lock_secret', 'metadata', 'token', 'multisig', 'namespace',
		'restriction_account', 'restriction_token', 'transfer'
	]
