...
namespace bitxorcore { namespace crypto {
	...
	namespace {
		...
		void BenchmarkVerify(benchmark::State& state) {
			...
			if (0 != numFailures)
				BITXORCORE_LOG(warning) << numFailures << " calls to Verify failed";
		}
		...
		void BenchmarkVerifyMulti(benchmark::State& state) {
			...
			if (0 != numFailures)
				BITXORCORE_LOG(warning) << numFailures << " calls to VerifyMulti failed";
		}
	}
}}
...
void RegisterTests();
void RegisterTests() {
	benchmark::RegisterBenchmark("BenchmarkVerify", bitxorcore::crypto::BenchmarkVerify)
			->UseRealTime()
			->Threads(1)
			->Threads(2)
			->Threads(4)
	...
	benchmark::RegisterBenchmark("BenchmarkVerifyMulti", bitxorcore::crypto::BenchmarkVerifyMulti)
			->UseRealTime()
			->Threads(1)
			->Threads(2)
			->Threads(4)
