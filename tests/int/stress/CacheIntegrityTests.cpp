...
#define TEST_CLASS CacheIntegrityTests
	...
	namespace {
		constexpr auto Transferable_Token_Id = TokenId(1234);
		...
		void RunMultithreadedReadWriteTest(size_t numReaders) {
				...
				threads.spawn([&, r] {
					...
					for (auto i = 0u; i < GetNumIterations(); ++i) {
						...
						while (true) {
							...
							sums[r] = sums[r] + accountStateIter.get().Balances.get(Transferable_Token_Id);
							break;
						}
					}
				});
			...
			threads.spawn([&] {
				...
				for (auto i = 0u; i < GetNumIterations(); ++i) {
					...
					accountState.Balances.credit(Transferable_Token_Id, Amount(i * 100'000));
					cache.commit();
				}
			});
	...
	namespace {
		...
		public:
			...
			~Stopwatch() {
				BITXORCORE_LOG(warning) << m_message << " needs " << nanos() / m_numIterations << "ns";
			}
		...
		void PopulateCache(HashCache& cache, size_t count, const Generator& generator) {
			...
			for (auto i = 0u; i < count; ++i) {
				...
				if (0 == (i + 1) % (count / 10))
					BITXORCORE_LOG(info) << "hash cache size is " << delta->size();
			}
