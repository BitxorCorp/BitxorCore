...
#define TEST_CLASS HarvesterIntegrityTests
	...
	namespace {
		...
		std::shared_ptr<plugins::PluginManager> CreatePluginManager(const config::BitxorCoreConfiguration& config) {
		...
		cache::BitxorCoreCache CreateBitxorCoreCache(const std::string& databaseDirectory) {
		...
		public:
			HarvesterTestContext()
					: m_config(test::CreatePrototypicalBitxorCoreConfiguration(CreateConfiguration(), m_tempDataDir.name()))
					...
					, m_cache(CreateBitxorCoreCache(config::BitxorCoreDataDirectory(m_tempDataDir.name()).dir("db").str()))
		...
		public:
			...
			void prepareAndUnlockSenderAccount(crypto::KeyPair&& signingKeyPair) {
				...
				accountStateIter.get().Balances.credit(test::Default_Currency_Token_Id, Amount(GetNumIterations()));
				accountStateIter.get().Balances.credit(test::Default_Harvesting_Token_Id, Amount(10'000'000));
			...
			void prepareSenderAccountAndTransactions(crypto::KeyPair&& keyPair, Timestamp deadline) {
				...
				for (auto i = 0u; i < GetNumIterations(); ++i) {
					...
					auto transactionHash = model::CalculateHash(*pTransaction, test::GetGenesisGenerationHashSeed());
	...
	NO_STRESS_TEST(TEST_CLASS, HarvestIsThreadSafeWhenUtCacheIsChanging) {
		...
		BITXORCORE_LOG(debug) << numHarvests << "/" << numHarvestAttempts << " blocks harvested";
	...
	NO_STRESS_TEST(TEST_CLASS, HarvestIsThreadSafeWhenBlockStatisticCacheIsChanging) {
		...
		BITXORCORE_LOG(debug) << numHarvests << "/" << GetNumIterations() << " blocks harvested";
