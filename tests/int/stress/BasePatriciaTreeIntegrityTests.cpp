...
#define TEST_CLASS BasePatriciaTreeIntegrityTests
	...
	namespace {
		...
		AccountStateCacheTypes::Options CreateAccountStateCacheOptions() {
			...
			return test::CreateDefaultAccountStateCacheOptions(TokenId(0xE329'AD1C'BE7F'C60D), TokenId(2222));
		}
	...
	namespace {
		...
		void RunTimedStressAction(const char* description, TAction action) {
			BITXORCORE_LOG(debug) << "START: " << description;
			...
			BITXORCORE_LOG(debug) << "  END: " << description << " - " << elapsedMills << "ms (" << elapsedNanosPerAccount << "ns avg)";
		}

		void AssertCanApplyManyAddsToTree(size_t numBatches) {
			// Arrange: create a db-backed account state cache
			BITXORCORE_LOG(debug) << "creating patricia tree enabled cache";
