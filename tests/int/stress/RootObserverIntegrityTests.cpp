	...
	namespace {
		...
		public:
			explicit TestContext(uint32_t numAccounts)
					: m_pPluginManager(test::CreatePluginManagerWithRealPlugins(test::CreatePrototypicalBitxorCoreConfiguration(
				...
				config::BitxorCoreDataDirectoryPreparer::Prepare(m_tempDataDir.name());
				...
				// seed the "genesis" / transfer account (this account is used to fund all other accounts)
				...
				accountState.Balances.credit(Harvesting_Token_Id, GetTotalChainBalance(numAccounts));
				m_cache.commit(Height());
			}

		public:
			// accounts are funded by "genesis" account
			...
			void addAccounts(uint8_t startAccountShortId, uint8_t numAccounts, Height height, uint8_t baseUnit = 1) {
				...
				for (uint8_t i = startAccountShortId; i < startAccountShortId + numAccounts; ++i) {
					...
					auto pTransaction = mocks::CreateTransactionWithFeeAndTransfers(Amount(), {
						{ test::UnresolveXor(Harvesting_Token_Id), Amount(multiplier * baseUnit * 1'000'000) }
					});
			...
			// send entire balance of all accounts to "genesis" account
			void zeroBalances(uint8_t startAccountShortId, uint8_t numAccounts, Height height) {
				...
				for (uint8_t i = startAccountShortId; i < startAccountShortId + numAccounts; ++i) {
					...
					auto pTransaction = mocks::CreateTransactionWithFeeAndTransfers(Amount(), {
						{ test::UnresolveXor(Harvesting_Token_Id), accountState.Balances.get(Harvesting_Token_Id) }
					});
			...
			void moveBalance(uint8_t accountShortId1, uint8_t accountShortId2, Height height) {
				...
				auto pTransaction = mocks::CreateTransactionWithFeeAndTransfers(Amount(), {
					{ test::UnresolveXor(Harvesting_Token_Id), accountState1.Balances.get(Harvesting_Token_Id) }
				});
		...
		private:
			void commitImportanceWipFiles() {
				...
				auto destDirectory = config::BitxorCoreDataDirectory(m_tempDataDir.name()).dir("importance");
...
#define ROLLBACK_TEST(TEST_NAME) \
	...
	TEST(TEST_CLASS, UndoDoesNotCalculateDeepRollbacksCorrectly_FiniteRollback) {
		...
		context.notify(Height(615), [](const auto& blockElement, const auto& blockExecutionContext) {
			EXPECT_THROW(chain::RollbackBlock(blockElement, blockExecutionContext), bitxorcore_out_of_range);
		});
	}
