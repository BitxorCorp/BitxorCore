...
#define TEST_CLASS LocalNodeRequestTests
	...
	namespace {
		...
		// region TestBitxorCoreDataDirectory

		class TestBitxorCoreDataDirectory {
		public:
			explicit TestBitxorCoreDataDirectory(const std::filesystem::path& directory) : m_dataDirectory(directory)
			{}
		...
		public:
			...
			config::BitxorCoreDirectory dir(const std::string& name) const {
	...
	namespace {
		...
		void AssertCanPushBlockToLocalNode(test::NodeFlag nodeFlag, TCheckSyncFiles checkSyncFiles) {
			...
			auto dataDirectory = TestBitxorCoreDataDirectory(context.dataDirectory());
			test::WaitForBoot(context);

			// Sanity: two files are produced when booting with genesis height
