...
#define TEST_CLASS HappyBlockchainIntegrityTests
	...
	namespace {
		...
		void UpdateConfigurationForNode(config::BitxorCoreConfiguration& config, uint32_t id) {
		...
		ChainStatistics PushRandomBlockchainToNode(
			...
			chainStats.Score = model::ChainScore(1); // initial genesis chain score
			...
			chainStats.Score += model::ChainScore(chain::CalculateScore(pGenesisBlockElement->Block, *blocks[0]));
		...
		public:
			Hash256 calculateReceiptsHash(const model::Block& block) const {
				...
				model::BalanceChangeReceipt receipt(
						...
						test::Default_Currency_Token_Id,
						totalFee);
		...
		void AssertMultiNodeNetworkCanReachConsensus(TVerifyTraits&& verifyTraits, size_t networkSize) {
			...
			// Act: boot all nodes
			BITXORCORE_LOG(debug) << "booting nodes";
			...
			for (auto i = 0u; i < networkSize; ++i) {
				...
				BITXORCORE_LOG(debug) << "pushing initial chain to node " << i;
			...
			for (auto i = 0u; i < networkSize; ++i) {
				contexts[i]->boot();
				BITXORCORE_LOG(debug) << "node " << i << " rebooted";
			}
			...
			for (auto i = 0u; i < networkSize; ++i) {
				...
				BITXORCORE_LOG(debug) << "waiting for node " << node << " to get best chain (score = " << bestChainStats.Score << ")";
