...
namespace bitxorcore { namespace test {
	...
	BlockchainBuilder::BlockchainBuilder(
		...
		// seed the state hash calculator with the genesis state
		if (resourcesPath.empty()) {
			BITXORCORE_LOG(debug) << "initializing BlockchainBuilder from memory";
			mocks::MockMemoryBlockStorage storage;
			m_pParentBlockElement = storage.loadBlockElement(Height(1));

			// extend lifetime of genesis block (referenced by m_pParentBlockElement) beyond lifetime of storage
			// (FileBlockStorage automatically extends block lifetime)
			m_pGenesisBlock = storage.loadBlock(Height(1));
		} else {
			BITXORCORE_LOG(debug) << "initializing BlockchainBuilder from resources path: " << resourcesPath;
	...
	std::unique_ptr<model::Block> BlockchainBuilder::createBlock(
		...
		extensions::BlockExtensions(GetGenesisGenerationHashSeed()).signFullBlock(signerKeyPair, *pBlock);
	...
	crypto::KeyPair BlockchainBuilder::findBlockSigner(
		...
		chain::BlockHitPredicate hitPredicate(m_config, [](const auto& publicKey, auto) {
			// to simplfy tests, just return genesis importance
			return GetGenesisImportance(publicKey);
		});
		...
		BITXORCORE_THROW_RUNTIME_ERROR("no eligible harvesting accounts were found");
	}
