...
namespace bitxorcore { namespace test {
	...
	std::unique_ptr<model::Transaction> BasicTransactionsBuilder::generateAt(size_t index, Timestamp deadline) const {
		...
		BITXORCORE_THROW_INVALID_ARGUMENT_1("cannot generate unknown transaction type", pair.first);
	}
	...
	UnresolvedAddress RootAliasToAddress(const std::string& namespaceName) {
			...
			uint16_t seednetwork = utils::to_underlying_type(Network_Identifier) | 0x0100;
			std::memcpy(&address[0], &seednetwork, 2);
			//address[0] = utils::to_underlying_type(Network_Identifier) | 0x0001;
			std::memcpy(address.data() + 2, &namespaceId, sizeof(NamespaceId));
	...
	std::unique_ptr<model::Transaction> BasicTransactionsBuilder::SignWithDeadline(
		...
		extensions::TransactionExtensions(GetGenesisGenerationHashSeed()).sign(signerKeyPair, *pTransaction);
