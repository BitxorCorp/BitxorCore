...
namespace bitxorcore { namespace test {
	...
	void SeedStateHashCalculator(StateHashCalculator& stateHashCalculator, const BlockchainBuilder::Blocks& blocks) {
		// can load genesis from memory because it is only used for execution, so state hash can be wrong
		mocks::MockMemoryBlockStorage storage;
		auto pGenesisBlockElement = storage.loadBlockElement(Height(1));
		stateHashCalculator.execute(pGenesisBlockElement->Block);
	...
	void AssertCurrencyBalances(
		...
		for (const auto& expectedBalance : expectedBalances) {
			...
			EXPECT_EQ(expectedBalance.Balance, accountState.Balances.get(Default_Currency_Token_Id)) << message;
		}
	}
	...
	void AssertNamespaceCount(const local::LocalNode& localNode, size_t numExpectedNamespaces) {
		...
		EXPECT_EQ(numExpectedNamespaces + 2, numActiveNamespaces); // default namespace defined in genesis has two subnamespaces
	}
