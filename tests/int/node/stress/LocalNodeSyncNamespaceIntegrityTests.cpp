...
#define TEST_CLASS LocalNodeSyncNamespaceIntegrityTests
	...
	namespace {
		...
		Hash256 GetComponentStateHash(const test::PeerLocalNodeTestContext& context) {
			...
			return subCacheMerkleRoots.empty() ? Hash256() : subCacheMerkleRoots[1]; // { AccountState, *Namespace*, Token }
		}
	...
	namespace {
		...
		void SetMaxRollbackBlocks(const config::BitxorCoreConfiguration& config) {
	...
	NO_STRESS_TEST(TEST_CLASS, CanPruneNamespace) {
		// Arrange:
		test::StateHashDisabledTestContext context(test::NonGenesisTransactionPlugins::None, SetMaxRollbackBlocks);
	...
	NO_STRESS_TEST(TEST_CLASS, CanPruneNamespaceWithStateHashEnabled) {
		// Arrange:
		test::StateHashEnabledTestContext context(test::NonGenesisTransactionPlugins::None, SetMaxRollbackBlocks);
