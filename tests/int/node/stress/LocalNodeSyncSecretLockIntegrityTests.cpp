...
#define TEST_CLASS LocalNodeSyncSecretLockIntegrityTests
	...
	namespace {
		...
		Hash256 GetComponentStateHash(const test::PeerLocalNodeTestContext& context) {
			return GetComponentStateHash(context, 3); // { AccountState, Namespace, Token, *SecretLock* }
		}
		...
		void AssertDeactivatingSecretLock(const std::pair<std::vector<Hash256>, std::vector<Hash256>>& stateHashesPair) {
			...
			// - secret lock cache merkle root is initially zero (no locks in genesis)
	...
	NO_STRESS_TEST(TEST_CLASS, CanLockSecretLock) {
		// Arrange:
		test::StateHashDisabledTestContext context(test::NonGenesisTransactionPlugins::Lock_Secret);
	...
	NO_STRESS_TEST(TEST_CLASS, CanLockSecretLockWithStateHashEnabled) {
		// Arrange:
		test::StateHashEnabledTestContext context(test::NonGenesisTransactionPlugins::Lock_Secret);
	...
	namespace {
		...
		public:
			SecretLockRollbackTestContext()
					: m_context(test::NonGenesisTransactionPlugins::Lock_Secret, ConfigTransform)
					, m_accounts(4)
		...
		private:
			static void ConfigTransform(config::BitxorCoreConfiguration& config) {
		...
		private:
			...
			public:
				...
				std::unique_ptr<model::Transaction> generateAt(size_t index, Timestamp deadline) const override {
					...
					extensions::TransactionExtensions(test::GetGenesisGenerationHashSeed()).sign(keyPair, *pTransaction);
					return pTransaction;
				}
			};
