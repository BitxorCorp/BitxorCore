...
#define TEST_CLASS VotingSetTests
	...
	TEST(TEST_CLASS, CalculateVotingSetStartHeight_DoesNotSupportZeroGrouping) {
		EXPECT_THROW(CalculateVotingSetStartHeight(FinalizationEpoch(1), 0), bitxorcore_invalid_argument);
	}

	TEST(TEST_CLASS, CalculateVotingSetStartHeight_DoesNotSupportZeroEpoch) {
		EXPECT_THROW(CalculateVotingSetStartHeight(FinalizationEpoch(), 20), bitxorcore_invalid_argument);

		EXPECT_THROW(CalculateVotingSetStartHeight(FinalizationEpoch(), 50), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, CalculateVotingSetEndHeight_DoesNotSupportZeroGrouping) {
		EXPECT_THROW(CalculateVotingSetEndHeight(FinalizationEpoch(1), 0), bitxorcore_invalid_argument);
	}

	TEST(TEST_CLASS, CalculateVotingSetEndHeight_DoesNotSupportZeroEpoch) {
		EXPECT_THROW(CalculateVotingSetEndHeight(FinalizationEpoch(), 20), bitxorcore_invalid_argument);

		EXPECT_THROW(CalculateVotingSetEndHeight(FinalizationEpoch(), 50), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, CalculateFinalizationEpochForHeight_DoesNotSupportZeroGrouping) {
		EXPECT_THROW(CalculateFinalizationEpochForHeight(Height(1), 0), bitxorcore_invalid_argument);
	}

	TEST(TEST_CLASS, CalculateFinalizationEpochForHeight_DoesNotSupportZeroHeight) {
		EXPECT_THROW(CalculateFinalizationEpochForHeight(Height(), 20), bitxorcore_invalid_argument);

		EXPECT_THROW(CalculateFinalizationEpochForHeight(Height(), 50), bitxorcore_invalid_argument);
	}
