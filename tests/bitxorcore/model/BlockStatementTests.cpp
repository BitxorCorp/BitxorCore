...
#include "bitxorcore/model/BlockStatement.h"
#include "bitxorcore/crypto/MerkleHashBuilder.h"
...
#define MERKLE_TEST(TEST_NAME) \
	...
	namespace {
		...
		void Add(BlockStatement& statement, TokenResolutionStatement&& resolutionStatement) {
			auto unresolved = resolutionStatement.unresolved();
			statement.TokenResolutionStatements.emplace(unresolved, std::move(resolutionStatement));
		}
	}
	...
	MERKLE_TEST(CanCalculateMerkleForSingleTokenResolutionStatement) {
		AssertCanCalculateMerkleForSingleComponentStatement<TTraits>(TokenResolutionStatement(UnresolvedTokenId(100)));
	}
	...
	MERKLE_TEST(CanCalculateMerkleForMultipleTokenResolutionStatements) {
		AssertCanCalculateMerkleForMultipleComponentStatements<TTraits>(
				TokenResolutionStatement(UnresolvedTokenId(100)),
				TokenResolutionStatement(UnresolvedTokenId(200)),
				TokenResolutionStatement(UnresolvedTokenId(300)));
	}
	...
	MERKLE_TEST(CanCalculateMerkleForHeterogenousComponentStatementsMultipleEach) {
		...
		auto componentStatement6 = TokenResolutionStatement(UnresolvedTokenId(100));
		auto componentStatement7 = TokenResolutionStatement(UnresolvedTokenId(200));
	...
	namespace {
		struct CountTraits {
			static void RunStatementTest(
					...
					uint32_t numTokenStatements,
					uint32_t numTotalStatements) {
				// Arrange:
				std::vector<size_t> numStatements{ numTransactionStatements, numAddressStatements, numTokenStatements };
			...
			static void RunStatementTest(
				...
				for (uint32_t i = 0u; i <= maxStatements; ++i) {
					...
					if (maxStatements == i) {
						...
						AssertResolutionStatementSources(
								numTokenStatements,
								2 * std::min(i, numTokenStatements),
								blockStatementCopy.TokenResolutionStatements,
								message + " (token resolution statements)");
					}
				}
			}
...
#define SUB_STATEMENT_TEST(TEST_NAME) \
	...
	SUB_STATEMENT_TEST(BlockStatementWithOnlyTokenResolutions) {
