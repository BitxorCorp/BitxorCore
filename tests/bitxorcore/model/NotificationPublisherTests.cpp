...
#define TEST_CLASS NotificationPublisherTests
	...
	namespace {
		constexpr auto Currency_Token_Id = UnresolvedTokenId(1234);
		...
		void PublishAll(const WeakEntityInfo& entityInfo, PublicationMode mode, TAssertSubFunc assertSub) {
			...
			auto pPub = CreateNotificationPublisher(registry, Currency_Token_Id, Fork_Height, mode);
		...
		void PublishOne(const WeakEntityInfo& entityInfo, TAssertNotification assertNotification) {
			...
			auto registry = mocks::CreateDefaultTransactionRegistry(Plugin_Option_Flags);
			auto pPub = CreateNotificationPublisher(registry, Currency_Token_Id, Fork_Height);
	...
	TEST(TEST_CLASS, CanPublishTransactionFeeDebitNotifications) {
		...
		PublishOne<BalanceDebitNotification>(*pTransaction, [&transaction = *pTransaction](const auto& notification) {
			...
			EXPECT_EQ(Currency_Token_Id, notification.TokenId);
	...
	TEST(TEST_CLASS, CannotRaiseAnyNotificationsForUnknownEntities) {
		...
		// Act:
		EXPECT_THROW(PublishOne<SourceChangeNotification>(entity, [](const auto&) {}), bitxorcore_runtime_error);
	}
	...
	TEST(TEST_CLASS, CannotRaiseAnyNotificationsForUnknownEntitiesWithModeBasic) {
		...
		// Act:
		EXPECT_THROW(PublishAll(entity, PublicationMode::Basic, [](const auto&) {}), bitxorcore_runtime_error);
	}
	...
	TEST(TEST_CLASS, CannotRaiseAnyNotificationsForUnknownEntitiesWithModeCustom) {
		...
		// Act:
		EXPECT_THROW(PublishAll(entity, PublicationMode::Custom, [](const auto&) {}), bitxorcore_runtime_error);
	}
