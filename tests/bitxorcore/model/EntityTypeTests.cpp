...
#define TEST_CLASS EntityTypeTests
	...
	TEST(TEST_CLASS, CanConvertKnownEntityTypeToBasicEntityType) {
		// Assert: blocks
		EXPECT_EQ(BasicEntityType::Block, ToBasicEntityType(Entity_Type_Block_Genesis));
	...
	TEST(TEST_CLASS, CanOutputBlockEnumValues) {
		EXPECT_EQ("Block_Genesis", test::ToString(Entity_Type_Block_Genesis));
	...
	TEST(TEST_CLASS, CanOutputPluginEnumValues) {
		...
		EXPECT_EQ("Token_Metadata", test::ToString(ToEntityType(0x4244)));
		EXPECT_EQ("Hash_Lock", test::ToString(ToEntityType(0x4148)));
		EXPECT_EQ("Account_Key_Link", test::ToString(ToEntityType(0x414C)));
		EXPECT_EQ("Token_Supply_Change", test::ToString(ToEntityType(0x424D)));
		EXPECT_EQ("Namespace_Registration", test::ToString(ToEntityType(0x414E)));
		EXPECT_EQ("Account_Token_Restriction", test::ToString(ToEntityType(0x4250)));
		EXPECT_EQ("Token_Global_Restriction", test::ToString(ToEntityType(0x4151)));
