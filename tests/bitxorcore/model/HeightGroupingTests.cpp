...
#define TEST_CLASS HeightGroupingTests
	...
	TEST(TEST_CLASS, CannotConvertHeightToGroupedHeight_Zero) {
		...
		for (const auto& pair : SingleGroupingTraits::GetHeightToGroupedHeightMap()) {
			...
			// Act + Assert:
			EXPECT_THROW(CalculateGroupedHeight<GroupedHeight>(inputHeight, 0), bitxorcore_invalid_argument) << "for height " << inputHeight;
		}
	}
...
#define GROUPING_TRAITS_BASED_TEST(TEST_NAME) \
	...
	TEST(TEST_CLASS, HeightGroupingFacade_CannotCreateAroundInvalidGroupedHeight) {
		for (auto rawHeight : std::initializer_list<Height::ValueType>{ 0, 2, 99, 101, 150 })
			EXPECT_THROW(HeightGroupingFacade<GroupedHeight>(GroupedHeight(rawHeight), 100), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, HeightGroupingFacade_CannotCreateAroundZeroGrouping) {
		for (auto rawHeight : std::initializer_list<Height::ValueType>{ 1, 100, 200 })
			EXPECT_THROW(HeightGroupingFacade<GroupedHeight>(GroupedHeight(rawHeight), 0), bitxorcore_invalid_argument);
	}

	TEST(TEST_CLASS, HeightGroupingFacade_CanMovePreviousFromGenesis) {
		// Arrange:
	...
	TEST(TEST_CLASS, HeightGroupingFacade_CanMoveNextFromGenesis) {
		// Arrange:
		HeightGroupingFacade<GroupedHeight> facade(GroupedHeight(1), 100);
	...
	TEST(TEST_CLASS, HeightGroupingFacade_CanMoveNextFromGenesisWhenGroupingHeightIsOne) {
		// Arrange:
	...
	TEST(TEST_CLASS, HeightGroupingFacade_CanMovePreviousFromNonGenesis) {
		// Arrange:
	...
	TEST(TEST_CLASS, HeightGroupingFacade_CanMoveNextFromNonGenesis) {
		// Arrange:
		HeightGroupingFacade<GroupedHeight> facade(GroupedHeight(900), 100);
	...
	TEST(TEST_CLASS, HeightGroupingFacade_CanMoveNextFromNonGenesisWhenGroupingHeightIsOne) {
		// Arrange:
