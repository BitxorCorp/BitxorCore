...
#include "bitxorcore/model/ResolutionStatement.h"
...
#define TEST_CLASS ResolutionStatementTests
	...
	namespace {
		...
		struct TokenTraits {
			using ResolutionStatementType = TokenResolutionStatement;

			static constexpr uint8_t Statement_Type_Code = 2;

			static UnresolvedTokenId CreateRandomUnresolved() {
				return test::GenerateRandomValue<UnresolvedTokenId>();
			}

			static TokenId CreateRandomResolved() {
				return test::GenerateRandomValue<TokenId>();
			}

			template<typename TTokenId>
			static RawBuffer ToBuffer(const TTokenId& tokenId) {
				return { reinterpret_cast<const uint8_t*>(&tokenId), sizeof(TTokenId) };
			}
		};
	}
...
#define RESOLUTION_STATEMENT_TEST(TEST_NAME) \
	...
	RESOLUTION_STATEMENT_TEST(CannotResolveOutOfOrder) {
		...
		// Act + Assert:
		EXPECT_THROW(resolutionStatement.addResolution(TTraits::CreateRandomResolved(), { 11, 11 }), bitxorcore_invalid_argument);
		EXPECT_THROW(resolutionStatement.addResolution(TTraits::CreateRandomResolved(), { 10, 12 }), bitxorcore_invalid_argument);
	}
