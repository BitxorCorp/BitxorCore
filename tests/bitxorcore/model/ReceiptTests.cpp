...
#define RECEIPT_FIELDS FIELD(Token) FIELD(SenderAddress) FIELD(RecipientAddress)
...
#undef RECEIPT_FIELDS
	...
	TEST(TEST_CLASS, CanCreateBalanceTransferReceipt) {
		...
		// Act:
		BalanceTransferReceipt receipt(static_cast<ReceiptType>(123), sender, recipient, TokenId(88), Amount(452));
		...
		EXPECT_EQ(TokenId(88), receipt.Token.TokenId);
		EXPECT_EQ(Amount(452), receipt.Token.Amount);
...
#define RECEIPT_FIELDS FIELD(Token) FIELD(TargetAddress)
...
#undef RECEIPT_FIELDS
	...
	TEST(TEST_CLASS, CanCreateBalanceChangeReceipt) {
		...
		// Act:
		BalanceChangeReceipt receipt(static_cast<ReceiptType>(124), target, TokenId(88), Amount(452));
...
#define RECEIPT_FIELDS FIELD(Token)
...
#undef RECEIPT_FIELDS
	...
	TEST(TEST_CLASS, CanCreateInflationReceipt) {
		// Act:
		InflationReceipt receipt(static_cast<ReceiptType>(124), TokenId(88), Amount(452));
