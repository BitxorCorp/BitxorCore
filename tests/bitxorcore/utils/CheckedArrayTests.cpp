...
#include "bitxorcore/utils/CheckedArray.h"
...
#define TEST_CLASS CheckedArrayTests
	...
	TEST(TEST_CLASS, AddingElementThrowsWhenCapacityIsExceeded) {
		...
		// Act + Assert:
		EXPECT_THROW(array.push_back(123), bitxorcore_out_of_range);
	}
	...
	TEST(TEST_CLASS, SubscriptOperatorThrowsWhenIndexIsOutOfRange) {
		...
		// Act + Assert:
		EXPECT_THROW(array[Default_Size / 2], bitxorcore_out_of_range);
		EXPECT_THROW(array[Default_Size / 2 + 1], bitxorcore_out_of_range);
		EXPECT_THROW(array[Default_Size], bitxorcore_out_of_range);
		EXPECT_THROW(array[12345], bitxorcore_out_of_range);
	}
