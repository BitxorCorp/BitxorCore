...
#define TEST_CLASS ConfigurationValueParsersTests
	...
	TEST(TEST_CLASS, CanParseValidTokenId) {
		AssertUnsignedIntHexParseSuccess<TokenId::ValueType>(0xFFFF'FFFF'FFFF'FFFF, "", [](auto value) {
			return TokenId(static_cast<TokenId::ValueType>(value));
		});
	}

	TEST(TEST_CLASS, CannotParseInvalidTokenId) {
		AssertUnsignedIntHexParseFailure<TokenId, TokenId::ValueType>(TokenId(177), "");
	}
