...
#define TEST_CLASS ConfigurationUtilsTests
	...
	TEST(TEST_CLASS, GetIniPropertyNameThrowsWhenCppVariableNameIsTooShort) {
		EXPECT_THROW(GetIniPropertyName(nullptr), bitxorcore_invalid_argument);
		EXPECT_THROW(GetIniPropertyName(""), bitxorcore_invalid_argument);
		EXPECT_THROW(GetIniPropertyName("a"), bitxorcore_invalid_argument);
	}

	TEST(TEST_CLASS, GetIniPropertyNameThrowsWhenCppVariableNameDoesNotStartWithLetter) {
		EXPECT_THROW(GetIniPropertyName("0abcd"), bitxorcore_invalid_argument);
		EXPECT_THROW(GetIniPropertyName("9abcd"), bitxorcore_invalid_argument);
		EXPECT_THROW(GetIniPropertyName("!abcd"), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, LoadIniPropertyThrowsWhenCppVariableNameIsInvalid) {
		...
		// Act + Assert:
		uint32_t value;
		EXPECT_THROW(LoadIniProperty(bag, "foo", "0baz", value), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, LoadIniPropertyThrowsWhenBagDoesNotContainKey) {
		...
		// Act + Assert:
		uint32_t value;
		EXPECT_THROW(LoadIniProperty(bag, "foo", "bar", value), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, VerifyBagSizeExactThrowsWhenBagSizeIsNotEqualToExpectedSize) {
		...
		for (auto expectedSize : std::initializer_list<size_t>{ 0, 1, 4, 6, 9, 100 })
			EXPECT_THROW(VerifyBagSizeExact(bag, expectedSize), bitxorcore_invalid_argument) << expectedSize;
	}
