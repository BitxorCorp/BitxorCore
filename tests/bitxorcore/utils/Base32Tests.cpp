...
#define TEST_CLASS Base32Tests
	...
	TEST(TEST_CLASS, EncodeThrowsWhenArraySizeIsNotAMultipleOfFive) {
		for (auto i = 2u; i < 10; i += 2) {
			...
			// Act + Assert:
			EXPECT_THROW(Base32Encode(buffer), bitxorcore_runtime_error);
		}
	}
	...
	TEST(TEST_CLASS, EncodeThrowsWhenOutputBufferIsTooSmall) {
		...
		// Act + Assert:
		EXPECT_THROW(Base32Encode(input, output), bitxorcore_runtime_error);
	}
	...
	TEST(TEST_CLASS, DecodeThrowsWhenStringLengthIsNotAMultipleOfEight) {
		for (auto i = 1u; i < 8; ++i) {
			...
			// Act + Assert:
			EXPECT_THROW(Base32Decode(str), bitxorcore_runtime_error);
		}
	}
	...
	TEST(TEST_CLASS, DecodeThrowsWhenOutputBufferIsTooSmall) {
		...
		// Act + Assert:
		EXPECT_THROW(Base32Decode(input, output), bitxorcore_runtime_error);
	}
