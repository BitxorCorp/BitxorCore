	...
	namespace {
		struct ThrottledMessage {
			...
			static void Log(uint32_t value) {
				BITXORCORE_LOG_THROTTLE(warning, Throttle_Millis) << "throttled message " << value;
				++Num_Logs;
			}
		};
		...
		std::string CreateExpectedThrottledMessage(uint64_t numLogAttempts, uint32_t value) {
			...
			out << "<warning> (utils::ThrottleLoggerTests.cpp@135) [" << numLogAttempts << " log count] throttled message " << value;
			return out.str();
		}
	}
