...
#define TEST_CLASS BitxorCoreExceptionTests
	...
	namespace {
		...
		std::vector<std::string> GetLocationIndependentDiagnosticInformation(const boost::exception& ex) {
			...
			while ((newLinePos = info.find('\n', startPos)) != std::string::npos) {
				...
				BITXORCORE_LOG(trace) << infoLines.size() << ": " << infoLines.back();
			}
...
#ifdef _MSC_VER
#define CUSTOM_TAG_NAMESPACE "bitxorcore::`anonymous namespace'::"
...
#define PTRSUFFIX "*"
	...
	namespace {
		struct RuntimeErrorTraits {
			using ExceptionType = bitxorcore_runtime_error;
			static constexpr auto Exception_Fqn = CLASSPREFIX "bitxorcore::bitxorcore_error<" CLASSPREFIX "std::runtime_error>";
		};

		struct InvalidArgumentTraits {
			using ExceptionType = bitxorcore_invalid_argument;
			static constexpr auto Exception_Fqn = CLASSPREFIX "bitxorcore::bitxorcore_error<" CLASSPREFIX "std::invalid_argument>";
		};

		struct OutOfRangeTraits {
			using ExceptionType = bitxorcore_out_of_range;
			static constexpr auto Exception_Fqn = CLASSPREFIX "bitxorcore::bitxorcore_error<" CLASSPREFIX "std::out_of_range>";
		};

		struct FileIoErrorTraits {
			using ExceptionType = bitxorcore_file_io_error;
			using BaseExceptionType = bitxorcore_runtime_error;
			static constexpr auto Exception_Fqn =
				CLASSPREFIX "bitxorcore::bitxorcore_error<" CLASSPREFIX "bitxorcore::bitxorcore_error<" CLASSPREFIX "std::runtime_error> >";
		};
	}
	...
	TEST(TEST_CLASS, ExceptionHierarchyIsCorrect) {
		...
		EXPECT_THROW(throw bitxorcore_runtime_error("error"), std::runtime_error);
		EXPECT_THROW(throw bitxorcore_invalid_argument("error"), std::invalid_argument);
		EXPECT_THROW(throw bitxorcore_out_of_range("error"), std::out_of_range);

		// - second level exceptions
		EXPECT_THROW(throw bitxorcore_file_io_error("error"), bitxorcore_runtime_error);
		EXPECT_THROW(throw bitxorcore_file_io_error("error"), std::runtime_error);
	}
...
#define EXCEPTION_TRAITS_BASED_TEST(TEST_NAME) \
	...
	EXCEPTION_TRAITS_BASED_TEST(CanThrowExceptionWithCustomMessage) {
		try {
			// Act:
			BITXORCORE_THROW_EXCEPTION(typename TTraits::ExceptionType("custom error message"));
	...
	EXCEPTION_TRAITS_BASED_TEST(CanThrowExceptionWithCustomMessageAndCustomPodPayloads) {
		try {
			// Act:
			BITXORCORE_THROW_EXCEPTION(CreateExceptionWithCustomMessageAndInfo<typename TTraits::ExceptionType>());
		} catch (const typename TTraits::ExceptionType& ex) {
			// Assert:
	...
	EXCEPTION_TRAITS_BASED_TEST(CanCopyConstructException) {
		try {
			...
			auto ex2 = ex;
			BITXORCORE_THROW_EXCEPTION(ex2);
		} catch (const typename TTraits::ExceptionType& ex) {
			// Assert:
			auto expected = CreateExpectedDiagnosticsForExceptionWithCustomMessageAndInfo<TTraits>();
			expected.FunctionName = BOOST_CURRENT_FUNCTION;
	...
	EXCEPTION_TRAITS_BASED_TEST(CanThrowExceptionWithCustomMessageAndCustomStringPayload) {
		try {
			...
			BITXORCORE_THROW_EXCEPTION(typename TTraits::ExceptionType("custom error message") << customInfo);
		} catch (const typename TTraits::ExceptionType& ex) {
			// Assert:
			ExpectedDiagnostics<TTraits> expected;
			expected.What = "custom error message";
			expected.TagPairs.push_back(std::make_pair("bitxorcore::ErrorParam1", "string info"));
			expected.FunctionName = BOOST_CURRENT_FUNCTION;
			AssertExceptionInformation(ex, expected);
		}
	}
	...
	EXCEPTION_TRAITS_BASED_TEST(CanThrowExceptionWithCustomMessageAndCustomHexFormattedPayload) {
		try {
			...
			ExpectedDiagnostics<TTraits> expected;
			expected.What = "custom error message";
			expected.TagPairs.push_back(std::make_pair("bitxorcore::ErrorParam1", "68656C6C6F"));
			expected.FunctionName = BOOST_CURRENT_FUNCTION;
			AssertExceptionInformation(ex, expected);
		}
	}
...
#define ASSERT_THROW_MACRO_0(BITXORCORE_THROW, TRAITS) \
	do { \
		try { \
			BITXORCORE_THROW(#TRAITS); \
	...
	TEST(TEST_CLASS, CanThrowCustomMessageUsingExceptionMacro) {
		ASSERT_THROW_MACRO_0(BITXORCORE_THROW_RUNTIME_ERROR, RuntimeErrorTraits);
		ASSERT_THROW_MACRO_0(BITXORCORE_THROW_INVALID_ARGUMENT, InvalidArgumentTraits);
		ASSERT_THROW_MACRO_0(BITXORCORE_THROW_OUT_OF_RANGE, OutOfRangeTraits);
		ASSERT_THROW_MACRO_0(BITXORCORE_THROW_FILE_IO_ERROR, FileIoErrorTraits);
	}

#define ASSERT_THROW_MACRO_1(BITXORCORE_THROW, TRAITS) \
	do { \
		try { \
			BITXORCORE_THROW(#TRAITS, 12); \
		} catch (const TRAITS::ExceptionType& ex) { \
			ExpectedDiagnostics<TRAITS> expected; \
			expected.What = #TRAITS; \
			expected.FunctionName = BOOST_CURRENT_FUNCTION; \
			expected.TagPairs.push_back(std::make_pair("bitxorcore::ErrorParam1", "12")); \
			AssertExceptionInformation(ex, expected); \
		} \
	} while (false)

	TEST(TEST_CLASS, CanThrowCustomMessageWithOneParamterUsingExceptionMacro) {
		ASSERT_THROW_MACRO_1(BITXORCORE_THROW_RUNTIME_ERROR_1, RuntimeErrorTraits);
		ASSERT_THROW_MACRO_1(BITXORCORE_THROW_INVALID_ARGUMENT_1, InvalidArgumentTraits);
	}

#define ASSERT_THROW_MACRO_2(BITXORCORE_THROW, TRAITS) \
	do { \
		try { \
			BITXORCORE_THROW(#TRAITS, 12, 27); \
		} catch (const TRAITS::ExceptionType& ex) { \
			ExpectedDiagnostics<TRAITS> expected; \
			expected.What = #TRAITS; \
			expected.FunctionName = BOOST_CURRENT_FUNCTION; \
			expected.TagPairs.push_back(std::make_pair("bitxorcore::ErrorParam1", "12")); \
			expected.TagPairs.push_back(std::make_pair("bitxorcore::ErrorParam2", "27")); \
			AssertExceptionInformation(ex, expected); \
		} \
	} while (false)

	TEST(TEST_CLASS, CanThrowCustomMessageWithTwoParamtersUsingExceptionMacro) {
		ASSERT_THROW_MACRO_2(BITXORCORE_THROW_RUNTIME_ERROR_2, RuntimeErrorTraits);
		ASSERT_THROW_MACRO_2(BITXORCORE_THROW_INVALID_ARGUMENT_2, InvalidArgumentTraits);
	}
	...
	TEST(TEST_CLASS, CanRethrowWithAdditionalInfo) {
		try {
			try {
				...
				BITXORCORE_THROW_EXCEPTION(bitxorcore_runtime_error("original") << custom_info1(12));
			} catch (bitxorcore_runtime_error& ex) {
				// - add additional info and rethrow
				ex << custom_info2(45);
				throw;
			}
		} catch (const bitxorcore_runtime_error& ex) {
			// Assert:
	...
	TEST(TEST_CLASS, CanRethrowBaseExceptionAsMoreSpecificException) {
		try {
			try {
				...
				bitxorcore_file_io_error ex2(ex);
				ex2 << custom_info2(45);
				BITXORCORE_THROW_EXCEPTION(ex2);
			}
		} catch (const bitxorcore_runtime_error& ex) {
