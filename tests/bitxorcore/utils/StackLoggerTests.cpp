...
#define TEST_CLASS StackLoggerTests
	...
	TEST(TEST_CLASS, StackLoggerCanLogStackMessages) {
		...
		test::RunNonDeterministicTest("log stack messages", []() {
			...
			EXPECT_EQ("<warning> (utils::StackLogger.h@38) pushing scope 'test'", records[0].Message);

			auto elapsedMillis = ParseElapsedMillis(records[1].Message);
			if (!IsWithinSleepEpsilonRange(elapsedMillis)) {
				BITXORCORE_LOG(debug) << "elapsedMillis (" << elapsedMillis << ") outside of expected range";
				return false;
			}

			auto elapsedMillisString = " (" + std::to_string(elapsedMillis) + "ms)";
			EXPECT_EQ("<warning> (utils::StackLogger.h@44) popping scope 'test'" + elapsedMillisString, records[1].Message);
			return true;
		});
	}
	...
	TEST(TEST_CLASS, SlowOperationLoggerLogsWhenThresholdIsExceeded) {
		...
		test::RunNonDeterministicTest("log stack messages", []() {
			...
			auto elapsedMillisString = " (" + std::to_string(elapsedMillis) + "ms)";
			EXPECT_EQ("<warning> (utils::StackLogger.h@78) slow operation detected: 'test'" + elapsedMillisString, records[0].Message);
			return true;
		});
	}
	...
	TEST(TEST_CLASS, SlowOperationLoggerLogsWhenThresholdIsExceededWithSubOperations) {
		...
		test::RunNonDeterministicTest("log stack messages", []() {
			...
			expectedMessage
					<< "<warning> (utils::StackLogger.h@78) slow operation detected: 'test' (" << elapsedMillis << "ms)"
