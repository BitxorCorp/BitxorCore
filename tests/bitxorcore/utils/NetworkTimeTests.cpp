...
#define TEST_CLASS NetworkTimeTests
	...
	TEST(TEST_CLASS, ToNetworkTimeThrowsWhenSuppliedTimestampIsBeforeEpochTime) {
		...
		// Act + Assert:
		EXPECT_THROW(networkTime.toNetworkTime(ts1), bitxorcore_invalid_argument);
		EXPECT_THROW(networkTime.toNetworkTime(ts2), bitxorcore_invalid_argument);
		EXPECT_THROW(networkTime.toNetworkTime(ts3), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, ToUnixTimeDetectsOverflow) {
		...
		// Act + Assert:
		EXPECT_THROW(networkTime.toUnixTime(ts), bitxorcore_invalid_argument);
	}
