...
#define TEST_CLASS SecureByteArrayTests
	...
	namespace {
		...
		void AssertCannotCreateFromBufferWithSize(size_t size) {
			...
			EXPECT_THROW(ConcreteSecureByteArray::FromBuffer(buffer), bitxorcore_invalid_argument) << "buffer size: " << size;
		...
		void AssertCannotCreateFromSecureBufferWithSize(size_t size) {
			...
			EXPECT_THROW(ConcreteSecureByteArray::FromBufferSecure(buffer), bitxorcore_invalid_argument) << "buffer size: " << size;
		...
		void AssertCannotCreateFromStringWithSize(size_t size, char arrayFirstChar) {
			...
			EXPECT_THROW(ConcreteSecureByteArray::FromString(seedString), bitxorcore_invalid_argument) << "string size: " << size;
		...
		void AssertCannotCreateFromSecureStringWithSize(size_t size) {
			...
			EXPECT_THROW(ConcreteSecureByteArray::FromStringSecure(seedString), bitxorcore_invalid_argument) << "string size: " << size;
			EXPECT_EQ(std::string(seedString.size(), '\0'), seedString);
		}
	}
	...
	namespace {
		void AssertCannotCreateFromStringWithInvalidCharacter(char invalidChar) {
			...
			// Act + Assert: array creation should fail but string should not be cleared
			EXPECT_THROW(ConcreteSecureByteArray::FromString(seedString), bitxorcore_invalid_argument) << "invalid char: " << invalidChar;
		...
		void AssertCannotCreateFromSecureStringWithInvalidCharacter(char invalidChar) {
			...
			EXPECT_THROW(
					...
					bitxorcore_invalid_argument) << "invalid char: " << invalidChar;
			EXPECT_EQ(std::string(seedString.size(), '\0'), seedString);
		}
	}
