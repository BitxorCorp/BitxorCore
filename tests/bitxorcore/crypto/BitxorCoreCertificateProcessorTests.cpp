...
#include "bitxorcore/crypto/BitxorCoreCertificateProcessor.h"
...
#define TEST_CLASS BitxorCoreCertificateProcessorTests
	...
	namespace {
		...
		test::CertificatePointer CreateDefaultCertificate(const std::string& commonName) {
			...
			builder.setSubject("JP", "BXR", commonName);
			builder.setIssuer("JP", "BXR", commonName);
	...
	TEST(TEST_CLASS, CanCreateProcessor) {
		// Act:
		BitxorCoreCertificateProcessor processor;
	...
	TEST(TEST_CLASS, VerifyFailsWhenNoCertificateIsActive) {
		...
		// Act + Assert:
		EXPECT_THROW(processor.verify(true, *holder.pCertificateStoreContext), bitxorcore_invalid_argument);
	...
	TEST(TEST_CLASS, CanAddSinglePreverifiedCertificate) {
		...
		EXPECT_EQ("CN=Alice,O=BXR,C=JP", processor.certificate(0).Subject);
	}
	...
	TEST(TEST_CLASS, CanAddTwoPreverifiedCertificates) {
		...
		EXPECT_EQ("CN=Alice,O=BXR,C=JP", processor.certificate(0).Subject);
		EXPECT_EQ("CN=Bob,O=BXR,C=JP", processor.certificate(1).Subject);
	}
	...
	TEST(TEST_CLASS, UnparseableCertificateTakesPrecedenceOverPreverifiedStatus) {
		...
		test::CertificateBuilder builder;
		builder.setSubject("JP", "BXR", "Alice");
		builder.setIssuer("JP", "BXR", "Alice");
	...
	TEST(TEST_CLASS, ConsecutiveCertificatesWithSamePublicKeyAreNotCollapsed) {
		...
		builder1.setSubject("JP", "BXR", "Alice");
		builder1.setIssuer("JP", "BXR", "Alice");
	...
	TEST(TEST_CLASS, ConsecutiveCertificatesWithSameSubjectAreNotCollapsed) {
		...
		test::CertificateBuilder builder2;
		builder2.setSubject("JP", "BXR", "Alice");
		builder2.setIssuer("JP", "BXR", "Alice");
		...
		EXPECT_EQ("CN=Alice,O=BXR,C=JP", processor.certificate(0).Subject);
		EXPECT_EQ("CN=Alice,O=BXR,C=JP", processor.certificate(1).Subject);
	}
	...
	namespace {
		void AssertSelfSignedRootCertificateError(
			...
			// Arrange:
			test::CertificateBuilder builder;
			builder.setSubject("JP", "BXR", subjectCommonName);
			builder.setIssuer("JP", "BXR", issuerCommonName);
			builder.setPublicKey(*test::GenerateRandomCertificateKey());
