	...
	namespace {
		...
		Hashes Reduce(const Hashes& originalHashes) {
			if (0 != originalHashes.size() % 2)
				BITXORCORE_THROW_RUNTIME_ERROR_1("original hashes must have even size", originalHashes.size());
		...
		auto CreateMerkleTree(const Hashes& seedHashes) {
			...
			default:
				BITXORCORE_THROW_RUNTIME_ERROR_1("creating merkle tree not supported for this number of hashes", seedHashes.size());
			}
		}
