...
#include "tests/bitxorcore/validators/test/ValidationPolicyTestUtils.h"
	...
	namespace {
		...
		void ValidateMany(size_t numEntities, TAssertState assertState) {
			...
			if (0 == numEntities) {
				...
				BITXORCORE_LOG(fatal) << "Skipping test on single core system";
				return;
			}

			BITXORCORE_LOG(debug) << "Running test with " << numEntities << " entities";
