...
#define TEST_CLASS ValidationResultTests
	...
	TEST(TEST_CLASS, CanOutputKnownPluginEnumValues) {
		...
		EXPECT_EQ("Failure_LockHash_Invalid_Token_Amount", test::ToString(static_cast<ValidationResult>(0x80480002)));
		...
		EXPECT_EQ("Failure_Token_Invalid_Name", test::ToString(static_cast<ValidationResult>(0x804D0002)));
		...
		EXPECT_EQ("Failure_RestrictionToken_Max_Restrictions_Exceeded", test::ToString(static_cast<ValidationResult>(0x80510004)));
