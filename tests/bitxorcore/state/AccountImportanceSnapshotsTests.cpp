...
#define TEST_CLASS AccountImportanceSnapshotsTests
	...
	TEST(TEST_CLASS, CannotSetSnapshotAtLowerHeight) {
		...
		// Act + Assert:
		EXPECT_THROW(snapshots.set(Importance(246), model::ImportanceHeight(233)), bitxorcore_runtime_error);
		EXPECT_THROW(snapshots.set(Importance(246), model::ImportanceHeight(100)), bitxorcore_runtime_error);
	}
	...
	TEST(TEST_CLASS, CannotSetSnapshotAtSameHeight) {
		...
		// Act + Assert:
		EXPECT_THROW(snapshots.set(Importance(246), model::ImportanceHeight(234)), bitxorcore_runtime_error);
	}
