...
#include "bitxorcore/state/AccountPublicKeys.h"
...
#define PUBLIC_KEYS_ACCESSOR_TEST(NAME) TEST(TEST_CLASS, PublicKeysAccessor_##NAME)
	...
	PUBLIC_KEY_ACCESSOR_TEST(CannotResetKeyWithValue) {
		...
		// Act + Assert:
		EXPECT_THROW(accessor.set(key), bitxorcore_invalid_argument);
	}
	...
	PUBLIC_KEY_ACCESSOR_TEST(CannotAddKeyWithOverlappingRange) {
		...
		// Act + Assert:
		EXPECT_THROW(accessor.add({ { { 0x11 } }, Epoch(100), Epoch(150) }), bitxorcore_invalid_argument);
		EXPECT_THROW(accessor.add({ { { 0x11 } }, Epoch(100), Epoch(165) }), bitxorcore_invalid_argument);

		EXPECT_THROW(accessor.add({ { { 0x22 } }, Epoch(165), Epoch(200) }), bitxorcore_invalid_argument);
		EXPECT_THROW(accessor.add({ { { 0x22 } }, Epoch(180), Epoch(200) }), bitxorcore_invalid_argument);

		EXPECT_THROW(accessor.add({ { { 0x33 } }, Epoch(100), Epoch(200) }), bitxorcore_invalid_argument);
		EXPECT_THROW(accessor.add({ { { 0x33 } }, Epoch(160), Epoch(170) }), bitxorcore_invalid_argument);
	}
