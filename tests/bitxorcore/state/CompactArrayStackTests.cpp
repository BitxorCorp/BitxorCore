...
#include "bitxorcore/state/CompactArrayStack.h"
...
#define TEST_CLASS CompactArrayStackTests
	...
	TEST(TEST_CLASS, CannotPeekWhenEmpty) {
		...
		// Act + Assert:
		EXPECT_THROW(stack.peek(), bitxorcore_out_of_range);
		EXPECT_THROW(const_cast<const ValuePairStack&>(stack).peek(), bitxorcore_out_of_range);
	}
	...
	TEST(TEST_CLASS, CannotPopMoreValuesThanSet) {
		...
		stack.pop();
		EXPECT_THROW(stack.pop(), bitxorcore_out_of_range);
	}
	...
	TEST(TEST_CLASS, CannotAdvanceIteratorsPastEnd) {
		...
		// Act + Assert:
		EXPECT_THROW(++stack.end(), bitxorcore_out_of_range);
		EXPECT_THROW(stack.end()++, bitxorcore_out_of_range);
	}
	...
	TEST(TEST_CLASS, CannotDereferenceIteratorsAtEnd) {
		...
		// Act + Assert:
		EXPECT_THROW(*stack.end(), bitxorcore_out_of_range);
		EXPECT_THROW(stack.end().operator->(), bitxorcore_out_of_range);
	}
