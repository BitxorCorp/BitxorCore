...
#define TEST_CLASS AccountActivityBucketsTests
	...
	TEST(TEST_CLASS, CannotUpdateBucketAtLowerHeight) {
		...
		// Act + Assert:
		EXPECT_THROW(Set(buckets, model::ImportanceHeight(233), Amount(20), 11, 7), bitxorcore_runtime_error);
		EXPECT_THROW(Set(buckets, model::ImportanceHeight(100), Amount(20), 11, 7), bitxorcore_runtime_error);
	}
	...
	TEST(TEST_CLASS, CannotUpdateBucketAtLowerHeight_TryUpdate) {
		...
		// Act + Assert:
		EXPECT_THROW(TrySet(buckets, model::ImportanceHeight(233), Amount(20), 11, 7), bitxorcore_runtime_error);
		EXPECT_THROW(TrySet(buckets, model::ImportanceHeight(100), Amount(20), 11, 7), bitxorcore_runtime_error);
	}
