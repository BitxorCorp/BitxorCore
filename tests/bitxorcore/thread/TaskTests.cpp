...
#define TEST_CLASS TaskTests
	...
	TEST(TEST_CLASS, IncreasingDelayGenerator_RequiresMinDelayToBeLessThanMaxDelay) {
		EXPECT_THROW(CreateIncreasingDelayGenerator(TimeSpan::FromSeconds(3), 7, TimeSpan::FromSeconds(3), 5), bitxorcore_invalid_argument);
		EXPECT_THROW(CreateIncreasingDelayGenerator(TimeSpan::FromSeconds(4), 7, TimeSpan::FromSeconds(3), 5), bitxorcore_invalid_argument);
	}
