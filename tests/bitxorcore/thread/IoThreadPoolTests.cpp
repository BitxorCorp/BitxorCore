...
#define TEST_CLASS IoThreadPoolTests
	...
	TEST(TEST_CLASS, PoolCannotBeRestartedWhenRunning) {
		...
		EXPECT_THROW(pPool->start(), bitxorcore_runtime_error);
	}
			...
			for (auto i = 0u; i < 10; ++i) {
				boost::asio::post(pPool->ioContext(), [&]() {
					if (7u == ++numHandlerCalls)
						BITXORCORE_THROW_RUNTIME_ERROR("exception from thread pool thread");
				});
			}
...
#pragma clang diagnostic pop
	...
	namespace {
		...
		private:
			void stop() {
				BITXORCORE_LOG(debug) << "test completed - joining all threads";
				m_shouldWait = false;
				m_pool.join();
			}
