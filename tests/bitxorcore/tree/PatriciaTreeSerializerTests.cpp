...
#include "bitxorcore/tree/PatriciaTreeSerializer.h"
...
#define PTSERIALIZER_TRAITS_BASED_TEST(TEST_NAME) \
	...
	TEST(TEST_CLASS, SerializeNodeFailsWhenNodeIsEmpty) {
		EXPECT_THROW(Serializer::SerializeValue(TreeNode()), bitxorcore_invalid_argument);
	}
	...
	PTSERIALIZER_TRAITS_BASED_TEST(DeserializeFailsWhenThereIsNotEnoughData_Leaf) {
		...
		// Act + Assert:
		EXPECT_THROW(Serializer::DeserializeValue(buffer), bitxorcore_file_io_error);
	}
	...
	PTSERIALIZER_TRAITS_BASED_TEST(DeserializeFailsWhenMarkerIsInvalid_Leaf) {
		...
		// Act + Assert:
		EXPECT_THROW(Serializer::DeserializeValue(buffer), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, DeserializeValueFailsWhenPathSizeIsOddAndHasWrongFillerNibble) {
		...
		// Act + Assert:
		EXPECT_THROW(Serializer::DeserializeValue(buffer), bitxorcore_runtime_error);
	}
