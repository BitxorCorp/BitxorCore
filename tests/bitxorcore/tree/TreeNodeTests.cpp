...
#define TEST_CLASS TreeNodeTests
	...
	namespace {
		...
		TreeNodePathTestCase GetEvenTreeNodePathTestCase(PathType type) {
			...
			BITXORCORE_THROW_INVALID_ARGUMENT_1("unknown path type was requested", static_cast<int>(type));
		}
...
#define BRANCH_LINK_TEST(TEST_NAME) \
	...
	TEST(TEST_CLASS, BranchTreeNodeDoesNotRecalculateHashWhenNotDirty) {
		...
		test::RunNonDeterministicTest("BranchTreeNodeDoesNotRecalculateHashWhenNotDirty", []() {
			...
			BITXORCORE_LOG(debug) << "elaspedHashOneTime = " << elaspedHashOneTime << ", elaspedHashTwoTime = " << elaspedHashTwoTime;
	...
	namespace {
		void AssertEmptyTreeNode(const TreeNode& node) {
			...
			EXPECT_THROW(node.asLeafNode(), bitxorcore_runtime_error);
			EXPECT_THROW(node.asBranchNode(), bitxorcore_runtime_error);
	...
	TEST(TEST_CLASS, CannotChangeEmptyTreeNodePathViaTreeNode) {
		...
		EXPECT_THROW(node.setPath(newPath), bitxorcore_runtime_error);
	}
