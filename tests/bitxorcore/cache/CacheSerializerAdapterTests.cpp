...
#define TEST_CLASS CacheSerializerAdapterTests
	...
	TEST(TEST_CLASS, DeserializeValueFailsWhenThereIsNotEnoughData) {
		...
		// Act + Assert:
		EXPECT_THROW(Serializer::DeserializeValue(serialized), bitxorcore_file_io_error);
	}
	...
	TEST(TEST_CLASS, DeserializeValueFailsWhenVersionIsIncorrect) {
		...
		// Act + Assert:
		EXPECT_THROW(Serializer::DeserializeValue(serialized), bitxorcore_runtime_error);
	}
