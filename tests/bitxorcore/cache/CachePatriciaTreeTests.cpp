...
#include "tests/bitxorcore/cache/test/PatriciaTreeTestUtils.h"
...
#define TEST_CLASS CachePatriciaTreeTests
	...
	TEST(TEST_CLASS, Enabled_CannotInitializeWithUnknownRootHashInDb) {
		...
		// Act + Assert:
		EXPECT_THROW(CachePatriciaTree<DatabaseBasePatriciaTree>(true, holder.database(), 1), bitxorcore_runtime_error);
	}
