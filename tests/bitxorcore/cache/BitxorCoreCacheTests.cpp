...
#define TEST_CLASS BitxorCoreCacheTests

	namespace {
		void IncrementAllSubCaches(BitxorCoreCacheDelta& delta) {
			// Act:
		...
		void AddSubCacheWithId(BitxorCoreCacheBuilder& builder, test::SimpleCacheViewMode viewMode = test::SimpleCacheViewMode::Iterable) {
			builder.add<test::SimpleCacheStorageTraits>(std::make_unique<test::SimpleCacheT<CacheId>>(viewMode));
		}

		BitxorCoreCache CreateSimpleBitxorCoreCache() {
	...
	TEST(TEST_CLASS, CanCreateBitxorCoreCache_View) {
		// Act:
		auto cache = CreateSimpleBitxorCoreCache();
	...
	TEST(TEST_CLASS, CanCreateBitxorCoreCache_Delta) {
		// Act:
		auto cache = CreateSimpleBitxorCoreCache();
		auto delta = cache.createDelta();

		// Assert:
		EXPECT_EQ(BitxorCoreCacheDelta::Disposition::Attached, delta.disposition());
		AssertSubCacheSizes(delta, 0);
	}

	TEST(TEST_CLASS, CanCreateBitxorCoreCache_DetachedDelta) {
		// Act:
		auto cache = CreateSimpleBitxorCoreCache();
		...
		EXPECT_EQ(BitxorCoreCacheDelta::Disposition::Detached, pDetachedDelta->disposition());
	...
	namespace {
		struct ViewTraits {
			static auto CreateView(const BitxorCoreCache& cache) {
			...
			static auto CalculateStateHash(const BitxorCoreCacheView& view) {
				return view.calculateStateHash();
			}
		};

		struct DeltaTraits {
			static auto CreateView(BitxorCoreCache& cache) {
			...
			static auto CalculateStateHash(const BitxorCoreCacheDelta& view) {
...
#define VIEW_DELTA_TEST(TEST_NAME) \
	...
	namespace {
		BitxorCoreCache CreateSimpleBitxorCoreCacheForStateHashTests() {
	...
	VIEW_DELTA_TEST(StateHashIsNonzeroWhenStateCalculationIsEnabled) {
		// Arrange:
		auto cache = CreateSimpleBitxorCoreCacheForStateHashTests();
		auto view = TTraits::CreateView(cache);
	...
	namespace {
		void AssertCannotSetWrongNumberOfSubCacheMerkleRoots(uint32_t numHashes) {
			...
			// Act + Assert:
			EXPECT_THROW(view.setSubCacheMerkleRoots(hashes), bitxorcore_invalid_argument);
		}
	}
	...
	namespace {
		void CommitChangeToAllSubCaches(BitxorCoreCache& cache) {
			// Arrange:
			auto delta = cache.createDelta();
	...
	TEST(TEST_CLASS, CommitOfSubCacheInvalidatesDetachedDelta) {
		// Arrange:
		auto cache = CreateSimpleBitxorCoreCache();
		BitxorCoreCacheDetachedDelta cacheDetachedDelta(state::BitxorCoreState(), {});
		{
	...
	namespace {
		void CommitWithNoChanges(BitxorCoreCache& cache) {
			auto delta = cache.createDelta();
			cache.commit(Height());
		}
	}
	...
	TEST(TEST_CLASS, ReadLockBlocksCommitOfSubCache) {
		...
		test::AssertExclusiveLocks(
				[&cache]() { return BitxorCoreCacheView(cache.createView()); },
	...
	TEST(TEST_CLASS, DetachableDeltaBlocksCommitOfSubCache) {
		...
		// Assert:
		test::AssertExclusiveLocks(
				[&cache]() { return BitxorCoreCacheDetachableDelta(cache.createDetachableDelta()); },
				[&cache]() { CommitWithNoChanges(cache); });
	}
	...
	TEST(TEST_CLASS, DetachedDetachableDeltaBlocksCommitOfSubCache) {
		...
		test::AssertExclusiveLocks(
				[&cache]() {
					...
					return BitxorCoreCacheDetachableDelta(std::move(detachableDelta));
				},
				[&cache]() { CommitWithNoChanges(cache); });
	}
	...
	namespace {
		...
		void AssertDependentState(cache::BitxorCoreCache& cache, TAssertState assertState) {
			assertState(cache.createView().dependentState(), "view");
			assertState(cache.createDelta().dependentState(), "delta");
			assertState(const_cast<const BitxorCoreCacheDelta&&>(cache.createDelta()).dependentState(), "delta (const)");
	...
	TEST(TEST_CLASS, DependentStateIsInitiallyZero) {
		...
		AssertDependentState(cache, [](const auto& state, const auto& message) {
			test::AssertEqual(state::BitxorCoreState(), state, message);
		});
	}
	...
	TEST(TEST_CLASS, CanRoundtripCacheViaStorages) {
		...
		std::vector<std::vector<uint8_t>> serializedSubCaches;
		{
			...
			// Act: save all data
			auto storages = const_cast<const BitxorCoreCache&>(cache).storages();
			auto view = cache.createView();
	...
	namespace {
		BitxorCoreCache CreateSimpleBitxorCoreCacheWithSomeNonIterableSubCaches() {
			BitxorCoreCacheBuilder builder;
			AddSubCacheWithId<2>(builder);
	...
	TEST(TEST_CLASS, CanRoundtripCacheViaStoragesWhenSomeSubCachesDoNotSupportStorage) {
		...
		std::vector<std::vector<uint8_t>> serializedSubCaches;
		{
			...
			auto cache = CreateSimpleBitxorCoreCacheWithSomeNonIterableSubCaches();
			{
				auto delta = cache.createDelta();
				for (auto i = 1u; i <= 9; ++i)
					IncrementAllSubCaches(delta);
	...
	namespace {
		class BitxorCoreCacheProxy {
		public:
			BitxorCoreCacheProxy() : m_cache(CreateSimpleBitxorCoreCache())
			{}

		public:
			auto createView() const {
				return ViewProxy<BitxorCoreCacheView, test::SimpleCacheView>(m_cache.createView());
			}

			auto createDelta() {
				return ViewProxy<BitxorCoreCacheDelta, test::SimpleCacheDelta>(m_cache.createDelta());
			}
		...
		private:
			...
			public:
				ViewProxy()
						: m_view(BitxorCoreCacheDelta::Disposition::Detached, m_dependentState, {})
						, m_isValid(false)
				{}
			...
			private:
				state::BitxorCoreState m_dependentState;
				TView m_view;
				bool m_isValid;
			};
			...
			public:
				explicit DetachedDeltaProxy(BitxorCoreCacheDetachedDelta&& detachedDelta) : m_detachedDelta(std::move(detachedDelta))
				{}
			...
			public:
				auto tryLock() {
					...
					return pLockedDelta
							? ViewProxy<BitxorCoreCacheDelta, test::SimpleCacheDelta>(std::move(*pLockedDelta))
							: ViewProxy<BitxorCoreCacheDelta, test::SimpleCacheDelta>();
				}
		...
		private:
			BitxorCoreCache m_cache;
		};

		struct BitxorCoreCacheTraits {
		public:
			using CacheType = BitxorCoreCacheProxy;
	...
	DEFINE_CACHE_SYNC_TESTS(BitxorCoreCacheTraits,)
