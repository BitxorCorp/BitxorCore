...
#include "bitxorcore/cache/SynchronizedCache.h"
...
#define TEST_CLASS SynchronizedCacheTests
	...
	TEST(TEST_CLASS, CannotCommitChangesWhenNoDeltasAreOutstanding) {
		...
		// Act + Assert:
		EXPECT_THROW(cache.commit(), bitxorcore_runtime_error);
	}
	...
	TEST(TEST_CLASS, CannotCreateMultipleDeltas) {
		...
		test::SimpleCache cache;
		{
			...
			// Act + Assert:
			EXPECT_THROW(cache.createDelta(), bitxorcore_runtime_error);
		}
	...
	namespace {
		...
		public:
			~ThreadGroup() {
				BITXORCORE_LOG(debug) << ">>> joining all threads";
				for (auto& thread : m_threads)
					thread.join();

				BITXORCORE_LOG(debug) << "<<< joined all threads";
			}

		public:
			template<typename TFunc>
			void createThread(const std::string& name, TFunc func) {
				BITXORCORE_LOG(debug) << "+ creating thread: " << name;
				m_threads.emplace_back([name, func]() mutable {
					BITXORCORE_LOG(debug) << "> spawning thread: " << name;
					func();
					BITXORCORE_LOG(debug) << "< destroying thread: " << name;
				});
			}
		...
		void AssertCommitBlocksOperation(TBlockedOperationFunc blockedOperationFunc) {
			...
			threads.createThread("lock acquisition", [&commitLockGuard, &lockPromise]() mutable {
				...
				BITXORCORE_LOG(debug) << "lock acquisition thread locked cache";
			...
			BITXORCORE_LOG(debug) << "test thread waiting for flag";
			WAIT_FOR_ONE(flag); // waits for [A]
	...
	TEST(TEST_CLASS, SynchronizedCacheWithInit_CannotCallInitMoreThanOnce) {
		...
		// Act + Assert:
		EXPECT_THROW(cache.init("init value"), bitxorcore_runtime_error);
		EXPECT_THROW(cache.init("init value 2"), bitxorcore_runtime_error);
	}
