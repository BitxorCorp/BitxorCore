...
#include "bitxorcore/local/server/MemoryCounters.h"
...
#define TEST_CLASS MemoryCountersTests
	...
	namespace {
		...
		uint64_t GetValue(const Counters& counters, const std::string& name) {
			...
			BITXORCORE_THROW_INVALID_ARGUMENT_1("could not find counter with name", name);
		}
	...
	TEST(TEST_CLASS, CountersHaveNonzeroValues) {
		...
		// Act:
		for (const auto& counter : counters) {
			BITXORCORE_LOG(debug) << counter.id().name() << " : " << counter.value();
