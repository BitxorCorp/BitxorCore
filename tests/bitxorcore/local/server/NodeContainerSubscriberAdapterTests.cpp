...
#include "bitxorcore/local/server/NodeContainerSubscriberAdapter.h"
...
#define TEST_CLASS NodeContainerSubscriberAdapterTests
	...
	namespace {
		...
		void AssertSingleNode(
			...
			if (1 != nodesView.size())
				BITXORCORE_THROW_INVALID_ARGUMENT_1("expected container to only have single node", nodesView.size());
