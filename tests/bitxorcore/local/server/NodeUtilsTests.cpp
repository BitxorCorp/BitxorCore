...
#include "bitxorcore/local/server/NodeUtils.h"
...
#define TEST_CLASS NodeUtilsTests
	...
	namespace {
		void ValidateNodesWithLengths(size_t peerHostSize, size_t peerNameSize) {
			...
			BITXORCORE_LOG(debug) << "seed with lengths: " << peerHostSize << ", " << peerNameSize;
	...
	TEST(TEST_CLASS, ValidateNodes_FailsWhenMaxStringLengthsAreExceeded) {
		EXPECT_THROW(ValidateNodesWithLengths(256, 255), bitxorcore_invalid_argument);
		EXPECT_THROW(ValidateNodesWithLengths(255, 256), bitxorcore_invalid_argument);
	}

	namespace {
		auto CreateBitxorCoreConfiguration(const std::string& host, const std::string& name) {
		...
		void AddLocalNodeWithLengths(ionet::NodeContainer& nodes, size_t localHostSize, size_t localNameSize) {
			// Arrange: configure single node with specified lengths
			BITXORCORE_LOG(debug) << "seed with lengths: " << localHostSize << ", " << localNameSize;

			auto config = CreateBitxorCoreConfiguration(std::string(localHostSize, 'm'), std::string(localNameSize, 'l'));
	...
	TEST(TEST_CLASS, AddLocalNode_FailsWhenMaxStringLengthsAreExceeded) {
		...
		// Act + Assert:
		EXPECT_THROW(AddLocalNodeWithLengths(nodes, 256, 255), bitxorcore_invalid_argument);
		EXPECT_THROW(AddLocalNodeWithLengths(nodes, 255, 256), bitxorcore_invalid_argument);
	}
