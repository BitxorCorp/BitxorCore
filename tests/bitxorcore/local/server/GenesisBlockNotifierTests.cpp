...
#define TEST_CLASS GenesisBlockNotifierTests
	...
	namespace {
		...
		public:
			...
			auto reseedGenesisWithStatement() {
		...
		private:
			static model::BlockchainConfiguration CreateBlockchainConfiguration() {
				return test::CreateBitxorCoreConfigurationWithGenesisPluginExtensions("").Blockchain;
			}
		...
		private:
			...
			GenesisBlockNotifier m_notifier;
		};
	...
	TEST(TEST_CLASS, BlockChangeNotificationsAreNotRaisedWhenPreviousExecutionIsDetected) {
		...
		// Act:
		EXPECT_THROW(context.notifier().raise(subscriber), bitxorcore_runtime_error);
	...
	TEST(TEST_CLASS, BlockChangeNotificationsAreRaisedWhenPreviousExecutionIsNotDetected_WithStatement) {
		// Arrange:
		TestContext context(2);
		auto pBlockStatement = context.reseedGenesisWithStatement();
		mocks::MockBlockChangeSubscriber subscriber;
	...
	TEST(TEST_CLASS, FinalizationNotificationsAreRaisedWhenPreviousExecutionIsNotDetected) {
		...
		EXPECT_EQ(context.genesisEntityHash(), subscriberParams.Hash);
	}
	...
	TEST(TEST_CLASS, StateChangeNotificationsAreRaisedWhenPreviousExecutionIsNotDetected) {
		...
		// - register consumer because BitxorCoreCacheDelta wrapped by CacheChanges is temporary and will be out of scope below
		...
		// - check genesis and rental fee sinks
		EXPECT_TRUE(ContainsModifiedPrivate(addedAddresses, test::Test_Network_Genesis_Private_Key));
		EXPECT_TRUE(ContainsAddress(addedAddresses, model::StringToAddress(test::Namespace_Rental_Fee_Sink_Address)));
		EXPECT_TRUE(ContainsAddress(addedAddresses, model::StringToAddress(test::Token_Rental_Fee_Sink_Address)));
