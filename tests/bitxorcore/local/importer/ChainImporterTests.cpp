...
#pragma pack(pop)
		...
		public:
			...
			void seedBlocks(uint64_t numBlocks) {
				...
				for (auto i = 0u; i < numBlocks; ++i) {
					...
					auto blockWithAttributes = test::CreateBlock(genesisKeyPairs, recipients[i], rnd, height, timeSpacing);
		...
		public:
			void boot(
				...
				auto config = test::CreateBitxorCoreConfigurationWithGenesisPluginExtensions(m_dataDirectory.rootDir().str());
				...
				if (CacheDatabaseMode::Enabled == cacheDatabaseMode) {
					...
					test::ModifyGenesis(m_dataDirectory.rootDir().str(), [&config](auto& genesisBlock, const auto& genesisBlockElement) {
	...
	TEST(TEST_CLASS, CannotImportGenesisWhenSerializedStateIsPresent) {
		// Arrange:
		TestContext context;
	...
	TEST(TEST_CLASS, CanImportGenesisWithoutSubscribers) {
		// Arrange:
		TestContext context;
	...
	TEST(TEST_CLASS, CanImportGenesisWithBlockChangeSubscriber) {
		// Arrange:
		TestContext context;
	...
	TEST(TEST_CLASS, CanImportGenesisWithStateChangeSubscriber) {
		// Arrange:
		TestContext context;
	...
	TEST(TEST_CLASS, CanImportGenesisWithFinalizationSubscriber) {
		// Arrange:
		TestContext context;
