...
#define TEST_CLASS MultiBlockLoaderTests
	...
	namespace {
		void AddXorResolvers(plugins::PluginManager& pluginManager) {
			pluginManager.addTokenResolver([](const auto&, const auto& unresolved, auto& resolved) {
		...
		constexpr uint64_t CalculateExpectedScore(size_t height) {
			// - genesis difficulty is 0 and genesis time is 0
	...
	namespace {
		...
		auto CreateBlocks(size_t maxHeight) {
			...
			for (auto height = 2u; height <= maxHeight; ++height)
				blocks.push_back(test::CreateBlock(genesisKeyPairs, recipients[recipientIndex++], rnd, height).pBlock);

			return blocks;
		}

		void ExecuteGenesis(const extensions::LocalNodeStateRef& stateRef, const plugins::PluginManager& pluginManager) {
		...
		void SetBlockchainConfiguration(model::BlockchainConfiguration& config) {
			config.Plugins.emplace("bitxorcore.plugins.hashcache", utils::ConfigurationBag({ { "", { {} } } }));
		...
		config::BitxorCoreConfiguration CreateStateHashEnabledBitxorCoreConfiguration(const std::string& dataDirectory) {
			auto config = test::CreateBitxorCoreConfigurationWithGenesisPluginExtensions(dataDirectory);
		...
		void ExecuteWithStorage(io::BlockStorageCache& storage, TAction action) {
			...
			config::BitxorCoreDataDirectoryPreparer::Prepare(tempDataDirectory.name());

			auto config = CreateStateHashEnabledBitxorCoreConfiguration(tempDataDirectory.name());
			...
			auto localNodeConfig = test::CreatePrototypicalBitxorCoreConfiguration(std::move(blockchainConfig), tempDataDirectory.name());
			...
			ExecuteGenesis(stateRef, *pPluginManager);
