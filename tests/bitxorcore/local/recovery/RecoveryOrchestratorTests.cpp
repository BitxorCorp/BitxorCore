...
#include "bitxorcore/local/recovery/RecoveryOrchestrator.h"
...
#define TEST_CLASS RecoveryOrchestratorTests
	...
	namespace {
		...
		auto PrepareRandomBlocks(const config::BitxorCoreDirectory& dataDirectory, uint64_t numBlocks) {
			...
			auto height = 2u;
			std::mt19937_64 rnd;
			auto genesisKeyPairs = test::GetGenesisKeyPairs();

			// dummy genesis, just to make score calculations easier
			...
			for (auto i = 0u; i < numBlocks; ++i) {
				auto blockWithAttributes = test::CreateBlock(genesisKeyPairs, recipients[i], rnd, height, utils::TimeSpan::FromMinutes(1));
		...
		cache::SupplementalData CreateSupplementalDataWithDeterministicChainScore() {
			...
			supplementalData.State = test::CreateRandomBitxorCoreState();
		...
		void SeedCacheWithGenesis(const extensions::LocalNodeStateRef& stateRef, const plugins::PluginManager& pluginManager) {
			auto cacheDelta = stateRef.Cache.createDelta();
			extensions::GenesisBlockLoader loader(cacheDelta, pluginManager, pluginManager.createObserver());
			loader.executeAndCommit(stateRef, extensions::StateHashVerification::Disabled);
		}

		void RandomSeedCache(cache::BitxorCoreCache& bitxorcoreCache, Height cacheHeight, const state::BitxorCoreState& state) {
			// Arrange: seed the cache with random data
			{
				...
				bitxorcoreCache.commit(cacheHeight);
			}
		...
		void PrepareAndSaveState(
			...
			// - seed with genesis block, so that genesis accounts have proper balances
			test::LocalNodeTestState state(pluginManager.createCache());
			SeedCacheWithGenesis(state.ref(), pluginManager);
			RandomSeedCache(state.ref().Cache, cacheHeight, supplementalData.State);

			// - save state
			extensions::LocalNodeStateSerializer serializer(config::BitxorCoreDirectory(directory.str()));
			if (shouldUseCacheDatabase) {
				auto storages = const_cast<const cache::BitxorCoreCache&>(state.ref().Cache).storages();
		...
		public:
			...
			RecoveryOrchestratorTestContext(Flags flags, Height storageHeight, Height cacheHeight)
				...
				config::BitxorCoreDataDirectoryPreparer::Prepare(dataDirectory().rootDir().path());
			}
		...
		public:
			...
			config::BitxorCoreDirectory subDir(const std::string& name) const {
		...
		public:
			...
			config::BitxorCoreConfiguration createConfig() const {
				auto config = test::CreateBitxorCoreConfigurationWithGenesisPluginExtensions(dataDirectory().rootDir().str());
			...
			void boot() {
				...
				if (!std::filesystem::exists(dataDirectoryRoot.path() / "00000")) {
					...
					if (m_useCacheDatabaseStorage) {
						...
						test::ModifyGenesis(dataDirectoryRoot.str(), [&config](auto& genesisBlock, const auto& genesisBlockElement) {
							genesisBlock.StateHash = test::CalculateGenesisStateHash(genesisBlockElement, config);
						});
					}
				}
		...
		private:
			void prepareSavedStorage(const config::BitxorCoreConfiguration& config) {
				m_blockScores = PrepareRandomBlocks(dataDirectory().rootDir(), m_storageHeight.unwrap() - 1);
				BITXORCORE_LOG(debug) << "storage prepared";
				...
				BITXORCORE_LOG(debug) << "state prepared";
			}
		...
		private:
			...
			std::vector<state::BitxorCoreState> m_blockStates;
...
#define SUBSCRIBER_TRAITS_BASED_TEST(TEST_NAME) \
	...
	namespace {
		...
		public:
			MoveBlocksTestPreparer(Height startHeight, Height endHeight)
					...
					// m_transactionSignerKeyPair needs to have a balance in the genesis block because it makes a transfer in block 2
		...
		public:
			void prepareBlockStorage(const std::string& directory) {
				test::PrepareStorageWithoutGenesis(directory);
		...
		public:
			void prepareStateChanges(const RecoveryOrchestratorTestContext& context) {
				...
				auto senderAccountState = loadGenesisSenderAccountState(context);

				// 2. create a representative bitxorcore cache
				...
				// 3. update the caches
				// - seed the sender account (this is the only genesis account modified by this test and needs to be part of changes)
				...
				auto pStateChangeStorage = CreateFileStateChangeStorage(
						...
						[&bitxorcoreCache = state.ref().Cache]() { return bitxorcoreCache.changesStorages(); });
			...
			std::vector<Hash256> calculateAccountStateCacheHashes(const RecoveryOrchestratorTestContext& context) {
				// 1. create a representative bitxorcore cache
				auto pPluginManager = test::CreatePluginManagerWithRealPlugins(context.createConfig());

				test::LocalNodeTestState state(pPluginManager->createCache());
				auto cacheDelta = state.ref().Cache.createDelta();

				// 2. load the genesis block (this is necessary in order to be able to correctly calculate the state hashes)
				extensions::GenesisBlockLoader loader(cacheDelta, *pPluginManager, pPluginManager->createObserver());
		...
		private:
			state::AccountState loadGenesisSenderAccountState(const RecoveryOrchestratorTestContext& context) {
				// 1. create a representative bitxorcore cache
				auto pPluginManager = test::CreatePluginManagerWithRealPlugins(context.createConfig());
			...
			std::vector<Hash256> prepareAccountStateCacheChanges(cache::AccountStateCacheDelta& accountStateCacheDelta) {
				...
				for (auto height = m_startHeight; height <= m_endHeight; height = height + Height(1)) {
					...
					senderAccountStateIter.get().Balances.debit(test::Default_Currency_Token_Id, amount);
					recipientAccountStateIter.get().Balances.credit(test::Default_Currency_Token_Id, amount);
		...
		void PrepareSupplementalDataFile(const config::BitxorCoreDirectory& directory) {
	...
	TEST(TEST_CLASS, BlocksAreMovedFromBlockSyncWhenStepIsStateWritten) {
		...
		// Assert: genesis was executed and then all blocks were undone and reapplied
		...
		// - first state corresponds to genesis, which is processed *BEFORE* loading supplemental data
		// - bitxorcore state is loaded from supplemental data (using NumTotalTransactions as sentinel)
	...
	TEST(TEST_CLASS, BlocksAreNotMovedFromBlockSyncWhenStepIsBlocksWritten) {
		...
		// Assert: only genesis was executed
	...
	TEST(TEST_CLASS, CanRepairSpoolingDirectoriesWithStateChangeArtifactsAndBrokerMode) {
		...
		AssertCanRepairStateChangeDirectoryWithOutstandingMessages(Commit_Step, IngestMessagesMode::Broker, [](const auto& context) {
			...
			// - genesis block has been loaded, but no state changes so score equals 1
	...
	namespace {
		void AssertLoadChainTest(Height cacheHeight, Height storageHeight, Flags flags = Flags::Default) {
			...
			if (Height() == cacheHeight) {
				// - if full chain is loaded, genesis score is 1 and all blocks contribute to score
		...
		void AssertThrowsLoadChainTest(Flags flags, Height cacheHeight, Height storageHeight) {
			...
			// Act + Assert: exception is always rethrown by CreateAndBootHost as bitxorcore_runtime_error
			EXPECT_THROW(context.boot(), bitxorcore_runtime_error);
		}
	}

	TEST(TEST_CLASS, CanLoadChainWhenCacheIsEmptyAndOnlyGenesisIsInStorage_CacheDatabaseDisabled) {
	...
	namespace {
		void AssertLoadsOnlyGenesis(Height cacheHeight, Height storageHeight) {
			// Arrange:
			...
			// - no blocks are loaded from storage, so only genesis block contributes to score
			EXPECT_EQ(model::ChainScore(1), context.orchestrator().score());
		}
	...
	TEST(TEST_CLASS, CanLoadChainWhenCacheIsEmptyAndOnlyGenesisIsInStorage_CacheDatabaseEnabled) {
		AssertLoadsOnlyGenesis(Height(0), Height(1));
	}
