...
#include "bitxorcore/local/recovery/StateChangeRepairingSubscriber.h"
...
#define TEST_CLASS StateChangeRepairingSubscriberTests
	...
	namespace {
		...
		struct MockSimpleCacheExtensionStorageTraits {
			...
			static void Save(uint64_t, io::OutputStream&) {
				BITXORCORE_THROW_RUNTIME_ERROR("Save() unsupported in mock storage");
			}

			[[noreturn]]
			static uint64_t Load(io::InputStream&) {
				BITXORCORE_THROW_RUNTIME_ERROR("Load() unsupported in mock storage");
			}
	...
	TEST(TEST_CLASS, NotifyScoreChangeUpdatesLocalNodeChainScore) {
		// Arrange:
		auto cache = CreateBitxorCoreCache();
