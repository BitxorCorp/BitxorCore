...
using bitxorcore::model::ImportanceHeight;
using bitxorcore::model::ConvertToImportanceHeight;
...
#define TEST_CLASS ImportanceViewTests
	...
	namespace {
		...
		constexpr auto Default_Cache_Options = test::CreateDefaultAccountStateCacheOptions(TokenId(1111), Harvesting_Token_Id);
...
#define IMPROPER_LINKS_TRAITS_BASED_TEST(TEST_NAME) \
	...
	namespace {
		...
		void AssertImproperLink(TMutator mutator) {
			...
			// Act + Assert:
			EXPECT_THROW(TTraits::Act(*pView, publicKey), bitxorcore_runtime_error);
		}
	}
