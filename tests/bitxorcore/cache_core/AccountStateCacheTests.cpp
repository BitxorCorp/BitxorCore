...
#define TEST_CLASS AccountStateCacheTests
	...
	namespace {
		...
		constexpr auto Default_Cache_Options = test::CreateDefaultAccountStateCacheOptions(Currency_Token_Id, Harvesting_Token_Id);
	...
	namespace {
		...
		struct AccountStateCacheDeltaModificationPolicy : public test:: DeltaInsertModificationPolicy {
			static void Modify(DeltaProxy<TTraits>& delta, const state::AccountState& accountState) {
				...
				accountStateFromCache.Balances.credit(Harvesting_Token_Id, Amount(1));
			}
		};
	}
...
#define DEFINE_ACCOUNT_STATE_CACHE_TESTS(TRAITS, SUFFIX) \
	...
	namespace {
		...
		Amount GetBalance(const state::AccountState& accountState) {
			return accountState.Balances.get(Harvesting_Token_Id);
		}
		...
		void AddAccountToCacheDelta(AccountStateCacheDelta& delta, const typename TKeyTraits::Type& accountIdentifier, Amount balance) {
			...
			delta.find(accountIdentifier).get().Balances.credit(Harvesting_Token_Id, balance);
		}
...
#define ID_BASED_TEST(TEST_NAME) \
	...
	TEST(TEST_CLASS, CacheWrappersExposeHarvestingTokenId) {
		// Arrange:
		auto options = Default_Cache_Options;
		options.HarvestingTokenId = TokenId(11229988);
		AccountStateCache cache(CacheConfiguration(), options);

		// Act + Assert:
		EXPECT_EQ(TokenId(11229988), cache.createView()->harvestingTokenId());
		EXPECT_EQ(TokenId(11229988), cache.createDelta()->harvestingTokenId());
		EXPECT_EQ(TokenId(11229988), cache.createDetachedDelta().tryLock()->harvestingTokenId());
	}
	...
	// region find (optimizedTokenId bug regression)

	namespace {
		void AddThreeTokenBalances(state::AccountState& accountState) {
			accountState.Balances.credit(TokenId(1), Amount(10));
			accountState.Balances.credit(Harvesting_Token_Id, Amount(20));
			accountState.Balances.credit(Currency_Token_Id, Amount(30));
		}

		std::vector<TokenId> GetTokenIdsWithBalance(const state::AccountState& accountState) {
	...
	ID_BASED_TEST(FindByMutableAutomaticallyOptimizesCurrencyTokenAccessWhenPresent) {
		// Arrange:
		...
		std::vector<TokenId> tokenIdsFromDelta;
		{
			// - add an account with three tokens
			auto delta = cache.createDelta();
			delta->addAccount(accountIdentifier, TTraits::Default_Height);
			AddThreeTokenBalances(delta->find(accountIdentifier).get());
			tokenIdsFromDelta = GetTokenIdsWithBalance(delta->find(accountIdentifier).get());

			cache.commit();
		}

		// Act:
		auto tokenIdsFromView = GetTokenIdsWithBalance(cache.createView()->find(accountIdentifier).get());

		// Assert: tokens are ordered with currency token id first
		EXPECT_EQ(3u, tokenIdsFromDelta.size());
		EXPECT_EQ(std::vector<TokenId>({ Currency_Token_Id, TokenId(1), Harvesting_Token_Id }), tokenIdsFromDelta);

		EXPECT_EQ(3u, tokenIdsFromView.size());
		EXPECT_EQ(std::vector<TokenId>({ Currency_Token_Id, TokenId(1), Harvesting_Token_Id }), tokenIdsFromView);
	}

	ID_BASED_TEST(FindByMutableAutomaticallyOptimizesCurrencyTokenAccessWhenNotPresent) {
		...
		std::vector<TokenId> tokenIdsFromDelta;
		{
			// - add an account without any tokens
			auto delta = cache.createDelta();
			delta->addAccount(accountIdentifier, TTraits::Default_Height);
			cache.commit();
		}
		...
		{
			// - add three tokens to same account
			auto delta = cache.createDelta();
			AddThreeTokenBalances(delta->find(accountIdentifier).get());
			tokenIdsFromDelta = GetTokenIdsWithBalance(delta->find(accountIdentifier).get());
	...
	ID_BASED_TEST(AddAccountAutomaticallyOptimizesCurrencyTokenAccess) {
		// Arrange:
		AccountStateCache cache(CacheConfiguration(), Default_Cache_Options);
		auto delta = cache.createDelta();
		auto accountIdentifier = TTraits::GenerateAccountIdentifier();
		...
		// Assert:
		EXPECT_EQ(Currency_Token_Id, delta->find(accountIdentifier).get().Balances.begin()->first);
	}
	...
	TEST(TEST_CLASS, AddAccountAutomaticallyOptimizesCurrencyTokenAccess) {
		// Arrange:
		AccountStateCache cache(CacheConfiguration(), Default_Cache_Options);
		auto delta = cache.createDelta();
		auto accountState = CreateAccountStateWithMismatchedAddressAndPublicKey();
		...
		// Act:
		delta->addAccount(accountState);
		AddThreeTokenBalances(delta->find(accountState.Address).get());

		// Assert:
		EXPECT_EQ(Currency_Token_Id, delta->find(accountState.Address).get().Balances.begin()->first);
	}
	...
	TEST(TEST_CLASS, CanAddAccountViaAccountStateWithoutPublicKey) {
		...
		accountState.Balances.optimize(Currency_Token_Id);
	...
	TEST(TEST_CLASS, AddAccountAutomaticallyOverridesAnyExistingTokenIdBalanceOptimization) {
		// Arrange:
		auto accountState = CreateAccountStateWithRandomAddressAndPublicKey();
		AddThreeTokenBalances(accountState);
	...
	namespace {
		std::vector<Address> AddAccountsWithBalances(AccountStateCacheDelta& delta, const std::vector<Amount>& balances) {
			return test::AddAccountsWithBalances(delta, Harvesting_Token_Id, balances);
		}
	...
	TEST(TEST_CLASS, HighValueAddressesReturnsModifiedAccountsMeetingCriteria) {
		...
		auto deltaAction = [](const auto& addresses, auto& delta) {
			...
			for (const auto& address : addresses)
				delta->find(address).get().Balances.credit(Harvesting_Token_Id, Amount(1));
	...
	TEST(TEST_CLASS, HighValueAddressesReturnsAllAccountsMeetingCriteria) {
		...
		auto deltaAction = [](const auto& addresses, auto& delta) {
			...
			// - modify two [5 match]
			delta->find(addresses[1]).get().Balances.credit(Harvesting_Token_Id, Amount(100'000));
			delta->find(addresses[4]).get().Balances.debit(Harvesting_Token_Id, Amount(200'001));
	...
	namespace {
		...
		void RunProcessHighValueRemovedAccountsTest(TAction action) {
			...
			delta->find(addresses[1]).get().Balances.debit(Harvesting_Token_Id, Amount(1'200'000));
			delta->find(addresses[4]).get().Balances.debit(Harvesting_Token_Id, Amount(1'200'000));
	...
	namespace {
		void RunPruneTest(uint64_t grouping, Height pruneHeight, size_t expectedSizeAfterPrune) {
			...
			auto delta = cache.createDelta();
			delta->find(addresses[0]).get().Balances.debit(Harvesting_Token_Id, Amount(300'000));
			delta->updateHighValueAccounts(Height(4));

			delta->find(addresses[2]).get().Balances.debit(Harvesting_Token_Id, Amount(300'000));
