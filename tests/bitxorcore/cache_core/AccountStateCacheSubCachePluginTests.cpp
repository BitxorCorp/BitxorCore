...
#define TEST_CLASS AccountStateCacheSubCachePluginTests
	...
	namespace {
		...
		auto CreateConfigurationFromOptions(const AccountStateCacheTypes::Options& options) {
			...
			blockchainConfig.HarvestingTokenId = options.HarvestingTokenId;
	...
	namespace {
		...
		void PrepareCannotSaveTest(const CacheConfiguration& cacheConfig, TSaveFunc saveFunc) {
			...
			auto bitxorcoreCache = test::CoreSystemCacheFactory::Create(CreateConfigurationFromOptions(options));

			std::vector<uint8_t> buffer;
			mocks::MockMemoryStream stream(buffer);

			// Act + Assert:
			saveFunc(*pStorage, bitxorcoreCache, stream);
		}
	}

	TEST(TEST_CLASS, Full_CannotSaveSummary) {
		// Arrange:
		PrepareCannotSaveTest(CacheConfiguration(), [](auto& storage, auto& bitxorcoreCache, auto& stream) {
			auto cacheDelta = bitxorcoreCache.createDelta();

			// Act + Assert:
			EXPECT_THROW(storage.saveSummary(cacheDelta, stream), bitxorcore_invalid_argument);
		});
	}
	...
	TEST(TEST_CLASS, Summary_CannotSaveAll) {
		...
		PrepareCannotSaveTest(cacheConfig, [](auto& storage, auto& bitxorcoreCache, auto& stream) {
			auto cacheView = bitxorcoreCache.createView();

			// Act + Assert:
			EXPECT_THROW(storage.saveAll(cacheView, stream), bitxorcore_invalid_argument);
		});
	}
	...
	namespace {
		struct FullTraits {
			...
			public:
				bool save(const BitxorCoreCacheDelta&, io::OutputStream&) {
					return false;
				}

				void save(const BitxorCoreCacheView& cacheView, io::OutputStream& output) {
		...
		struct SummaryTraits {
			...
			public:
				bool save(const BitxorCoreCacheDelta& cacheDelta, io::OutputStream& output) {
					m_storage.saveSummary(cacheDelta, output);
					return true;
				}

				void save(const BitxorCoreCacheView&, io::OutputStream&)
				{}
		...
		void RunRoundtripTest(const AccountStateCacheTypes::Options& options, const RunRoundtripTestInput& input) {
			...
			SetFinalizationEpochs(votingPublicKeys.back(), 600, 900);
			{
				...
				auto bitxorcoreCache = test::CoreSystemCacheFactory::Create(CreateConfigurationFromOptions(options));
				{
					...
					bitxorcoreCache.commit(Height(3));
				}

				{
					...
					delta.find(addresses.front()).get().Balances.credit(Harvesting_Token_Id, Amount(100'000));

					// - decrease the third account's balance so that it is no longer important
					delta.find(addresses[2]).get().Balances.debit(Harvesting_Token_Id, Amount(100'000));
