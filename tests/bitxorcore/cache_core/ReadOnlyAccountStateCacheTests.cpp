...
#include "bitxorcore/cache_core/ReadOnlyAccountStateCache.h"
...
#define TEST_CLASS ReadOnlyAccountStateCacheTests
	...
	TEST(TEST_CLASS, HarvestingTokenIdIsExposed) {
		// Arrange:
		auto options = Default_Cache_Options;
		...
		// Act + Assert:
		EXPECT_EQ(TokenId(11229988), ReadOnlyAccountStateCache(*originalCache.createView()).harvestingTokenId());
		EXPECT_EQ(TokenId(11229988), ReadOnlyAccountStateCache(*originalCache.createDelta()).harvestingTokenId());
	}
...
#define ACCOUNT_KEY_BASED_TEST(TEST_NAME) \
	...
	ACCOUNT_KEY_BASED_TEST(ReadOnlyViewOnlyCanAccessCommittedElementsViaGet) {
		...
		EXPECT_THROW(readOnlyCache.find(TTraits::CreateKey(2)).get(), bitxorcore_invalid_argument);
		EXPECT_THROW(readOnlyCache.find(TTraits::CreateKey(3)).get(), bitxorcore_invalid_argument);
	}
	...
	namespace {
		std::vector<Address> AddAccountsWithBalances(AccountStateCacheDelta& delta, const std::vector<Amount>& balances) {
			return test::AddAccountsWithBalancesAndOverlappingVotingKeyLifetimes(delta, Harvesting_Token_Id, balances);
		}
	...
	TEST(TEST_CLASS, HighValueAccountStatistics_ReturnsAllAccountsMeetingCriteria) {
		...
		auto deltaAction = [](const auto& addresses, auto& delta) {
			...
			// - modify two [7 match]
			delta->find(addresses[1]).get().Balances.credit(Harvesting_Token_Id, Amount(500'000));
	...
	TEST(TEST_CLASS, HighValueAccountStatistics_RequiresVotingKeyForVotingEligibleAccounts) {
		...
		auto deltaAction = [](const auto& addresses, auto& delta) {
			...
			delta->find(addresses[5]).get().Balances.credit(Harvesting_Token_Id, Amount(1));
			delta->updateHighValueAccounts(Height(1));
