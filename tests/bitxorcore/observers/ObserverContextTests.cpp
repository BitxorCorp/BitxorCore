...
#define TEST_CLASS ObserverContextTests
	...
	namespace {
		...
		void AssertContext(
			...
			auto resolvedTokenId = resolvers.resolve(UnresolvedTokenId(24));
			EXPECT_EQ(TokenId(48), resolvedTokenId);
...
#define NOTIFY_MODE_TEST(TEST_NAME) \
	...
	NOTIFY_MODE_TEST(CanCreateObserverContextWithBlockStatementBuilder) {
		...
		RunTestWithCache([](auto& cacheDelta) {
			...
			EXPECT_EQ(1u, pStatement->TokenResolutionStatements.size()); // AssertContext
		});
	}
	...
	NOTIFY_MODE_TEST(CanCreateObserverContextWithBlockStatementBuilder_UndecoratedResolvers) {
		...
		RunTestWithCache([](auto& cacheDelta) {
			...
			EXPECT_EQ(0u, pStatement->TokenResolutionStatements.size()); // AssertContext
		});
	}
