...
#include "tests/bitxorcore/consumers/test/ConsumerInputFactory.h"
#include "tests/bitxorcore/consumers/test/ConsumerTestUtils.h"
...
using bitxorcore::disruptor::InputSource;
using bitxorcore::utils::HashSet;
using bitxorcore::validators::ValidationResult;
...
#define TEST_CLASS BlockchainSyncConsumerTests
	...
	namespace {
		...
		protected:
			void raise(const std::string& source) const {
				if (m_shouldRaiseError)
					BITXORCORE_THROW_RUNTIME_ERROR(("raising error from " + source).c_str());
			}
		...
		public:
			DifficultyCheckerParams(const std::vector<const model::Block*>& blocks, const cache::BitxorCoreCache& cache)
					: Blocks(blocks)
					, Cache(cache)
			{}
		...
		public:
			bool operator()(const std::vector<const model::Block*>& blocks, const cache::BitxorCoreCache& cache) const {
		...
		public:
			PreStateWrittenParams(const cache::BitxorCoreCacheDelta& cacheDelta, Height height)
		...
		public:
			void operator()(const cache::BitxorCoreCacheDelta& cacheDelta, Height height) const {
		...
		void LogInputSource(InputSource source) {
			BITXORCORE_LOG(debug) << "source " << source;
		}
		...
		// region BitxorCoreCacheFactory

		class BitxorCoreCacheFactory {
		public:
		...
		public:
			static cache::BitxorCoreCache Create(PruneIdentifiers& pruneIdentifiers) {
				...
				auto cache = cache::BitxorCoreCache(std::move(subCaches));
		...
		public:
			...
			ConsumerTestContext(std::unique_ptr<io::BlockStorage>&& pStorage, std::unique_ptr<io::PrunableBlockStorage>&& pStagingStorage)
					: Cache(BitxorCoreCacheFactory::Create(CachePruneIdentifiers))
		...
		public:
			BitxorCoreCacheFactory::PruneIdentifiers CachePruneIdentifiers;
			...
			std::vector<std::shared_ptr<model::Block>> OriginalBlocks; // original stored blocks (excluding genesis)
		...
		public:
			...
			void assertStored(const ConsumerInput& input, model::ChainScore::Delta expectedScoreDelta) const {
				...
				// - the cache was committed (add 1 to OriginalBlocks.size() because it does not include the genesis)
	...
	namespace {
		void AssertDeepUnwindAllowed(Height localHeight, Height maxRemoteHeight, Height lastFinalizedHeight) {
			...
			for (auto remoteHeight : { maxRemoteHeight + Height(10), maxRemoteHeight + Height(1), maxRemoteHeight }) {
				BITXORCORE_LOG(debug) << "remoteHeight = " << remoteHeight;
	...
	TEST(TEST_CLASS, RemoteChainFromRemotePullSourceCanUnwindAllUnfinalizedBlocksWhenOnlyGenesisIsFinalized) {
	...
	namespace {
		void SetFinalizedHeightInDependentState(cache::BitxorCoreCache& cache, Height height) {
	...
	TEST(TEST_CLASS, CommitStepsAreCorrectWhenWritingBlocksFails) {
		...
		// Act:
		EXPECT_THROW(context.Consumer(input), bitxorcore_runtime_error);
