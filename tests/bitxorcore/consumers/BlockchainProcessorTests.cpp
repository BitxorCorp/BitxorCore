...
#include "bitxorcore/consumers/BlockchainProcessor.h"
...
#include "bitxorcore/consumers/InputUtils.h"
...
using bitxorcore::disruptor::BlockElements;
	...
	namespace {
		...
		public:
			BlockHitPredicateParams(
					...
					const bitxorcore::GenerationHash& generationHash)
		...
		public:
			BlockHitPredicateFactoryParams(const cache::ReadOnlyBitxorCoreCache& cache)
		...
		public:
			BlockHitPredicate operator()(const cache::ReadOnlyBitxorCoreCache& cache) const {
		...
		public:
			...
			const bitxorcore::Timestamp Timestamp;
			const model::WeakEntityInfos EntityInfos;
		...
		public:
			ValidationResult Process(
				...
				auto cache = test::CreateBitxorCoreCacheWithMarkerAccount();
