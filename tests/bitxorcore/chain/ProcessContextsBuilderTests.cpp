...
#define TEST_CLASS ProcessContextsBuilderTests
	...
	namespace {
		...
		public:
			TestContext()
					: Cache(test::CreateBitxorCoreCacheWithMarkerAccount())
	...
	namespace {
		void AssertObserverContext(const TestContext& context, const observers::ObserverContext& observerContext) {
			...
			auto resolveResult = observerContext.Resolvers.resolve(UnresolvedTokenId(444));
			EXPECT_EQ(1u, context.ResolverCallPairs.first);
			EXPECT_EQ(1u, context.ResolverCallPairs.second);
			EXPECT_EQ(test::CreateResolverContextXor().resolve(UnresolvedTokenId(444)), resolveResult);
	...
	TEST(TEST_CLASS, CannotBuildObserverContextWithoutCache) {
		...
		// Act + Assert:
		EXPECT_THROW(context.Builder.buildObserverContext(), bitxorcore_invalid_argument);
	}
	...
	namespace {
		void AssertValidatorContext(const TestContext& context, const validators::ValidatorContext& validatorContext) {
			...
			// - check resolver wiring
			auto resolveResult = validatorContext.Resolvers.resolve(UnresolvedTokenId(444));
			EXPECT_EQ(1u, context.ResolverCallPairs.first);
			EXPECT_EQ(1u, context.ResolverCallPairs.second);
			EXPECT_EQ(test::CreateResolverContextXor().resolve(UnresolvedTokenId(444)), resolveResult);
	...
	TEST(TEST_CLASS, CannotBuildValidatorContextWithoutCache) {
		...
		// Act + Assert:
		EXPECT_THROW(context.Builder.buildValidatorContext(), bitxorcore_invalid_argument);
	}
