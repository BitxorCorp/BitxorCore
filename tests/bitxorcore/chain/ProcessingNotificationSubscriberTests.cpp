...
#define TEST_CLASS ProcessingNotificationSubscriberTests
	...
	namespace {
		...
		constexpr auto CreateResolverContext = test::CreateResolverContextWithCustomDoublingTokenResolver;
		...
		public:
			explicit TestContext(observers::NotifyMode executeMode = observers::NotifyMode::Commit)
				...
				BITXORCORE_LOG(debug) << "preparing test context with execute mode " << executeMode;
			}
			...
			void assertObserverCalls(
				...
				for (auto i = 0u; i < expectedTypes.size(); ++i) {
					...
					if (i < undoStartIndex) {
						...
						EXPECT_EQ(TokenId(22), observerContext.Resolvers.resolve(UnresolvedTokenId(11)));
					}
				}
	...
	namespace {
		void AssertCannotProcessNotificationWithInvalidSize(size_t size) {
			...
			// Act + Assert:
			EXPECT_THROW(context.sub().notify(notification), bitxorcore_invalid_argument) << "size: " << size;
		}
	}
	...
	TEST(TEST_CLASS, CannotUndoWhenUndoIsNotEnabled) {
		...
		// Act + Assert:
		EXPECT_THROW(context.sub().undo(), bitxorcore_runtime_error);
	}
