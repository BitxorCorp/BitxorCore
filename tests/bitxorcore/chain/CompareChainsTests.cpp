...
#include "bitxorcore/chain/CompareChains.h"
#include "bitxorcore/model/BlockUtils.h"
#include "tests/bitxorcore/chain/test/MockChainApi.h"
#include "tests/test/core/HashTestUtils.h"
#include "tests/TestHarness.h"

using bitxorcore::model::ChainScore;
using bitxorcore::mocks::MockChainApi;
...
#define TEST_CLASS CompareChainsTests
	...
	namespace {
		...
		void AssertLocalChainExceptionPropagation(MockChainApi::EntryPoint entryPoint) {
			...
			// Act + Assert:
			EXPECT_THROW(CompareChains(local, remote, CreateCompareChainsOptions(1000, 1)).get(), bitxorcore_runtime_error);
		}
...
#define HASH_TEST(TEST_NAME) \
	...
	TEST(TEST_CLASS, RemoteIsNotSyncedWhenChainsDivergeAndRemoteHasHigherScoreAndLastFinalizedBlockIsNotGenesis_OneBatch) {
	...
	TEST(TEST_CLASS, RemoteIsNotSyncedWhenChainsDivergeAndRemoteHasHigherScoreAndLastFinalizedBlockIsNotGenesis_MultiBatch) {
	...
	TEST(TEST_CLASS, LocalChainStatisticsExceptionIsPropagatedDuringLocalScoreCheck) {
		...
		// Act:
		EXPECT_THROW(CompareChains(local, remote, CreateCompareChainsOptions(20, 1)).get(), bitxorcore_runtime_error);
	}
