...
using bitxorcore::test::TempFileGuard;
...
#define WRITING_TRAITS_BASED_TEST(TEST_NAME) \
	...
	TEST(TEST_CLASS, OpeningNonexistentFileThrows) {
		...
		// Act + Assert:
		EXPECT_THROW(RawFile(guard.name(), OpenMode::Read_Only), bitxorcore_file_io_error);
	}
	...
	TEST(TEST_CLASS, MovedFileDoesNotSupportFileOperations) {
		...
		EXPECT_THROW(original.read(buffer), bitxorcore::bitxorcore_runtime_error);
		EXPECT_THROW(original.seek(5), bitxorcore::bitxorcore_runtime_error);
		EXPECT_THROW(original.write(buffer), bitxorcore::bitxorcore_runtime_error);
	...
	TEST(TEST_CLASS, ReadThrowsOnOobRead) {
		...
		EXPECT_THROW(rawFile.read(outputData1), bitxorcore_file_io_error);
	}
	...
	TEST(TEST_CLASS, WriteOnReadOnlyFileThrowsException) {
		...
		// Act + Assert:
		EXPECT_THROW(rawFile.write(buffer), bitxorcore_file_io_error);
	}
	...
	WRITING_TRAITS_BASED_TEST(OobSeekInWritableFileThrows) {
		...
		// Act + Assert:
		EXPECT_EQ(0ull, rawFile.size());
		EXPECT_THROW(rawFile.seek(10ull), bitxorcore_file_io_error);
	}
	...
	WRITING_TRAITS_BASED_TEST(CannotSeekBeyondWrittenData) {
		...
		// Act + Assert:
		EXPECT_THROW(rawFile.seek(inputData.size() + 1), bitxorcore_file_io_error);
	}
	...
	namespace {
		void AssertCannotOpenForIfOpened(OpenMode alreadyOpenedMode, OpenMode newMode) {
			...
			for (auto i = 0u; i < 3; ++i)
				EXPECT_THROW(RawFile(guard.name(), newMode), bitxorcore_file_io_error) << "attempt " << i;
		}
	}
