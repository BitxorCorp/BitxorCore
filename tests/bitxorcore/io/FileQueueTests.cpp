...
#define READER_TRAITS_BASED_TEST(TEST_NAME) \
	...
	namespace {
		...
		void ReadNever(const std::vector<uint8_t>&) {
			BITXORCORE_THROW_INVALID_ARGUMENT("consumer was unexpectedly called");
		}
	...
	READER_TRAITS_BASED_TEST(CannotReadWhenMessageAtReaderIndexDoesNotExist) {
		...
		// Act:
		EXPECT_THROW(TReaderTraits::TryReadNextMessage(context.reader(), ReadNever), bitxorcore_runtime_error);
	...
	READER_TRAITS_BASED_TEST(ReadDoesNotRemoveUnsuccessfullyProcessedDataFile) {
		...
		EXPECT_THROW(TReaderTraits::TryReadNextMessage(context.reader(), ReadNever), bitxorcore_invalid_argument);
