...
#define TEST_CLASS FixedSizeValueStorageTests
	...
	TEST(TEST_CLASS, StorageCannotOpenWithoutAnyValues_Save) {
		...
		// Act + Assert:
		EXPECT_THROW(context.hashFile().save(Height(0), value), bitxorcore_runtime_error);
	}
	...
	TEST(TEST_CLASS, StorageCannotOpenWithoutTwoValues_Save) {
		...
		// Act + Assert:
		EXPECT_THROW(context.hashFile().save(Height(1), value), bitxorcore_runtime_error);
	}
	...
	TEST(TEST_CLASS, StorageCannotOpenWithoutTwoValues_Load) {
		...
		// Act + Assert:
		EXPECT_THROW(context.hashFile().loadRangeFrom(Height(0), 1), bitxorcore_runtime_error);
	}
	...
	TEST(TEST_CLASS, StorageCannotSaveSkippingSomeKeys) {
		...
		// Act + Assert:
		EXPECT_THROW(context.hashFile().save(Height(4), ToValue(14)), bitxorcore_file_io_error);
	}
