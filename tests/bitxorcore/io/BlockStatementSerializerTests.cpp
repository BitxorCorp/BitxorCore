...
#pragma pack(push, 1)
		...
		struct TransactionStatementWithTwoReceipts : public TransactionStatementWithZeroReceipts {
			...
			model::ArtifactExpiryReceipt<TokenId> Receipt2;
		};
...
#pragma pack(pop)
		...
		void PrepareStatements(model::BlockStatement& blockStatement, TransactionStatementWithTwoReceipts& statement, uint8_t order = 0) {
			...
			statement.Receipt2.Size = sizeof(model::ArtifactExpiryReceipt<TokenId>);
		...
		auto& AddStatement(model::BlockStatement& blockStatement, UnresolvedTokenId tokenId) {
			return blockStatement.TokenResolutionStatements.emplace(tokenId, model::TokenResolutionStatement(tokenId)).first->second;
		}
		...
		template<typename TAction>
		void PrepareOnlyTokenResolutionsTest(bool shouldOrder, TAction action) {
			// Arrange:
			using ZeroEntryStatement = ResolutionStatementWithZeroEntries<UnresolvedTokenId>;
			using TwoEntryStatement = ResolutionStatementWithTwoEntries<UnresolvedTokenId, TokenId>;
		...
		void PrepareAllStatementsTest(bool shouldOrder, TAction action) {
			...
			PrepareOnlyTokenResolutionsTest(shouldOrder, [&](auto& blockStatement, const auto& buffer) {
				aggregateBlockStatement.TokenResolutionStatements = std::move(blockStatement.TokenResolutionStatements);
	...
	TEST(TEST_CLASS, CanWriteBlockStatementWithOnlyTokenResolutions) {
		// Act + Assert: ordering is required to have output in deterministic order
		PrepareOnlyTokenResolutionsTest(true, AssertWrite);
	}
	...
	TEST(TEST_CLASS, CanReadBlockStatementWithOnlyTokenResolutions) {
		PrepareOnlyTokenResolutionsTest(false, AssertRead);
	}
	...
	TEST(TEST_CLASS, CanRoundtripBlockStatementWithOnlyTokenResolutions) {
