...
#define TEST_CLASS FileDatabaseTests
	...
	namespace {
		...
		public:
			explicit TestContext(size_t batchSize = Batch_Size)
					: m_database(config::BitxorCoreDirectory(m_tempDir.name()), { batchSize, ".bin" })
			{}
	...
	TEST(TEST_CLASS, CannotCreateWithZeroBatchSize) {
		EXPECT_THROW(TestContext(0), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, InputStreamDoesNotCreateFile) {
		...
		// Act + Assert:
		EXPECT_THROW(context.database().inputStream(0), bitxorcore_file_io_error);
...
#define READ_TEST(TEST_NAME) \
	...
	READ_TEST(CannotReadPastPayloadInFile) {
		...
		// Act + Assert:
		EXPECT_THROW(pInputStream->read(readBuffer), bitxorcore_file_io_error);
	}
	...
	TEST(TEST_CLASS, CannotReadUnwrittenPayloadInPartiallyFullFile) {
		...
		// Act + Assert:
		EXPECT_THROW(context.database().inputStream(13), bitxorcore_file_io_error);
	}
