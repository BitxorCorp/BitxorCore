...
#define TEST_CLASS ConfigurationFileLoaderTests
	...
	TEST(TEST_CLASS, LoadIniConfigurationFailsWhenFileDoesNotExist) {
		...
		RunTestWithTemporaryDirectory([](const auto& path) {
			// Act + Assert:
			EXPECT_THROW(LoadIniConfiguration<TestConfiguration>(path / Not_Config_Filename), bitxorcore_runtime_error);
		});
	}
	...
	TEST(TEST_CLASS, LoadPeersConfigurationFailsWhenFileDoesNotExist) {
		...
		RunTestWithTemporaryDirectory([](const auto& path) {
			// Act + Assert:
			EXPECT_THROW(LoadPeersConfiguration(path / Not_Config_Filename, model::UniqueNetworkFingerprint()), bitxorcore_runtime_error);
		});
	}
