...
#define TEST_CLASS BitxorCoreConfigurationTests

	// region BitxorCoreConfiguration file io
	...
	namespace {
		...
		void AssertDefaultBlockchainConfiguration(const model::BlockchainConfiguration& config) {
			...
			EXPECT_EQ(
					...
					config.Network.GenesisSignerPublicKey);
			EXPECT_EQ(
			...
			EXPECT_EQ(test::Default_Currency_Token_Id, config.CurrencyTokenId);
			EXPECT_EQ(test::Default_Harvesting_Token_Id, config.HarvestingTokenId);
			...
			EXPECT_EQ(Amount(9'000'000'000'000'000), config.MaxTokenAtomicUnits);
			...
			EXPECT_EQ(model::StringToAddress("TESHM4G4RAQ2CKS6SLV4RSDQXF7Y3CAJLIOAOFA"), config.HarvestNetworkFeeSinkAddressPOS);
			EXPECT_EQ(model::StringToAddress("TESHM4G4RAQ2CKS6SLV4RSDQXF7Y3CAJLIOAOFA"), config.HarvestNetworkFeeSinkAddress);
	...
	TEST(TEST_CLASS, CannotLoadConfigWhenAnyConfigFileIsMissing) {
		...
		for (const auto& filenameToRemove : Config_Filenames) {
			...
			// - remove a file
			BITXORCORE_LOG(debug) << "removing " << filenameToRemove;
			EXPECT_TRUE(std::filesystem::remove(std::filesystem::path(tempDir.name()) / filenameToRemove));

			// Act + Assert: attempt to load the config
			EXPECT_THROW(BitxorCoreConfiguration::LoadFromPath(tempDir.name(), "server"), bitxorcore_runtime_error);
		}
	}
	...
	TEST(TEST_CLASS, ResourcesDirectoryContainsAllConfigFiles) {
		...
		for (const auto& directoryEntry : std::filesystem::directory_iterator(resourcesPath)) {
			BITXORCORE_LOG(debug) << "found " << directoryEntry.path();
	...
	TEST(TEST_CLASS, CanLoadConfigFromResourcesDirectoryWithServerExtensions) {
		...
		auto config = BitxorCoreConfiguration::LoadFromPath(Resources_Path, "server");

		// Assert:
		AssertDefaultBlockchainConfiguration(config.Blockchain);
		AssertDefaultNodeConfiguration(config.Node);
		AssertDefaultLoggingConfiguration(config.Logging, "bitxorcore_server%4N.log");
	...
	TEST(TEST_CLASS, CanLoadConfigFromResourcesDirectoryWithBrokerExtensions) {
		// Act: attempt to load from the "real" resources directory
		auto config = BitxorCoreConfiguration::LoadFromPath(Resources_Path, "broker");

		// Assert:
		AssertDefaultBlockchainConfiguration(config.Blockchain);
		AssertDefaultNodeConfiguration(config.Node);
		AssertDefaultLoggingConfiguration(config.Logging, "bitxorcore_broker%4N.log");
		AssertDefaultUserConfiguration(config.User);
		AssertDefaultExtensionsConfiguration(config.Extensions, {
	...
	TEST(TEST_CLASS, CanLoadConfigFromResourcesDirectoryWithRecoveryExtensions) {
		// Act: attempt to load from the "real" resources directory
		auto config = BitxorCoreConfiguration::LoadFromPath(Resources_Path, "recovery");

		// Assert:
		AssertDefaultBlockchainConfiguration(config.Blockchain);
		AssertDefaultNodeConfiguration(config.Node);
		AssertDefaultLoggingConfiguration(config.Logging, "bitxorcore_recovery%4N.log", utils::LogLevel::debug);
		AssertDefaultUserConfiguration(config.User);
	...
	namespace {
		...
		auto CreateBitxorCoreConfiguration() {
	...
	TEST(TEST_CLASS, CanExtractLocalNodeFromConfiguration) {
		// Arrange:
		auto config = CreateBitxorCoreConfiguration();
