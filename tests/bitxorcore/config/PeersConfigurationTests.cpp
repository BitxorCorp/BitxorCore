...
#include "bitxorcore/config/PeersConfiguration.h"
...
#define TEST_CLASS PeersConfigurationTests
	...
	TEST(TEST_CLASS, ConfigFileRequiresKnownPeers) {
		...
		// Act + Assert:
		EXPECT_THROW(LoadPeersFromStream(stream, GetNetworkFingerprint()), bitxorcore_runtime_error);
	}
	...
	TEST(TEST_CLASS, NodeRolesMustBeValid) {
		...
		stream << R"({
			"knownPeers":[
				{
					"publicKey":"1234567890ABCDEF1234567890ABCDEF1234567890ABCDEF1234567890ABCDEF",
					"endpoint":{ "host":"bob.bxor.ninja", "port":12345 },
	...
	TEST(TEST_CLASS, CanParseKnownPeers) {
		...
		AssertIdentity(node.identity(), expectedKey, "bob.bxor.ninja");
		AssertEndpoint(node.endpoint(), "bob.bxor.ninja", 12345);
		AssertMetadata(node.metadata(), "bob", ionet::NodeRoles::Api);
		EXPECT_EQ("bob @ bob.bxor.ninja:12345", test::ToString(node));
	}
	...
	TEST(TEST_CLASS, CanParseKnownPeersWithoutNames) {
		...
		EXPECT_EQ(std::string(expectedAddress) + " @ bob.bxor.ninja:12345", test::ToString(node));
	}
	...
	TEST(TEST_CLASS, CanLoadPeersFromResourcesDirectory) {
		...
		for (const auto filename : { "peers-p2p.json", "peers-api.json" }) {
			BITXORCORE_LOG(debug) << "parsing peers from " << filename;
