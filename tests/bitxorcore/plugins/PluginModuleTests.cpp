...
#define TEST_CLASS PluginModuleTests
	...
	namespace {
		constexpr auto Valid_Plugin_Name = "bitxorcore.plugins.transfer";
...
#define SCOPE_BASED_TEST(TEST_NAME) \
	...
	namespace {
		...
		void AssertCannotLoadPlugin(const std::string& directory, const std::string& name) {
			// Act + Assert:
			EXPECT_THROW(TModule(directory, name), bitxorcore_invalid_argument) << directory << " " << name;
		}
	}
	...
	SCOPE_BASED_TEST(CannotLoadUnkownPlugin_ExplicitDirectory) {
		AssertCannotLoadPlugin<typename TTraits::Module>(test::GetExplicitPluginsDirectory(), "bitxorcore.plugins.awesome");
	}

	SCOPE_BASED_TEST(CannotLoadUnkownPlugin_ImplicitDirectory) {
		AssertCannotLoadPlugin<typename TTraits::Module>("", "bitxorcore.plugins.awesome");
	}
	...
	SCOPE_BASED_TEST(CannotExtractUnknownSymbol_LoadedModule) {
		...
		EXPECT_THROW(module.template symbol<void*>("abc"), bitxorcore_runtime_error);
	}
	...
	namespace {
		...
		void AssertCannotExtractKnownSymbolAfterRelease(size_t numReleases) {
			...
			// Assert:
			EXPECT_FALSE(module.isLoaded());
			EXPECT_THROW(module.template symbol<void*>(Valid_Symbol_Name), bitxorcore_runtime_error);
		}
	}
