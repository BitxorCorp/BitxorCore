...
#define TEST_CLASS StateChangeReaderTests
	...
	namespace {
		...
		public:
			void saveAll(const cache::CacheChanges&, io::OutputStream&) const override {
				BITXORCORE_THROW_INVALID_ARGUMENT("saveAll - not supported in mock");
			}
			...
			void apply(const cache::CacheChanges&) const override {
				BITXORCORE_THROW_INVALID_ARGUMENT("apply - not supported in mock");
			}
		};
		...
		uint32_t ReadValueAt(const StateChangeInfo& stateChangeInfo) {
			...
			if (addedElements.empty())
				BITXORCORE_THROW_INVALID_ARGUMENT_1("addedElements is empty", CacheId);
	...
	TEST(TEST_CLASS, CannotReadSingleUnknownOperationType) {
		...
		// Act + Assert:
		EXPECT_THROW(ReadNextStateChange(stream, {}, subscriber), bitxorcore_invalid_argument);
	}
}}
