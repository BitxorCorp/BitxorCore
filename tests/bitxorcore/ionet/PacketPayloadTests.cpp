...
#include "bitxorcore/ionet/PacketPayload.h"
...
#define TEST_CLASS PacketPayloadTests
	...
	TEST(TEST_CLASS, CannotCreatePacketPayloadFromPacketWithInvalidData) {
		...
		// Act + Assert:
		EXPECT_THROW(PacketPayload(std::move(pPacket)), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, CannotMergePacketWithInvalidDataAndPayload) {
		...
		// Act + Assert:
		EXPECT_THROW(PacketPayload::Merge(std::move(pPacket), PacketPayload()), bitxorcore_invalid_argument);
	}
