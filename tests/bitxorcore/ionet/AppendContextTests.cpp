...
#include "bitxorcore/ionet/AppendContext.h"
...
#define TEST_CLASS AppendContextTests
	...
	TEST(TEST_CLASS, MutableBufferCannotBeAccessedAfterCommit) {
		...
		// Act + Assert:
		EXPECT_THROW(context.buffer(), bitxorcore_runtime_error);
	}
	...
	TEST(TEST_CLASS, CannotCommitMoreDataThanReserved) {
		...
		// Act + Assert:
		EXPECT_THROW(context.commit(101), bitxorcore_runtime_error);
	}
	...
	TEST(TEST_CLASS, CannotCommitDataMultipleTimes) {
		...
		// Act + Assert:
		EXPECT_THROW(context.commit(100), bitxorcore_runtime_error);
	}
