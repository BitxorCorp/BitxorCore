...
#include "bitxorcore/ionet/NodeVersion.h"
...
#define TEST_CLASS NodeVersionTests
	...
	TEST(TEST_CLASS, GetCurrentServerVersionReturnsCorrectVersion) {
		...
		// Assert:
		EXPECT_EQ(static_cast<uint32_t>(BITXORCORE_VERSION_MAJOR), (version.unwrap() >> 24) & 0xFF);
		EXPECT_EQ(static_cast<uint32_t>(BITXORCORE_VERSION_MINOR), (version.unwrap() >> 16) & 0xFF);
		EXPECT_EQ(static_cast<uint32_t>(BITXORCORE_VERSION_REVISION), (version.unwrap() >> 8) & 0xFF);
		EXPECT_EQ(static_cast<uint32_t>(BITXORCORE_VERSION_BUILD), (version.unwrap() & 0xFF));
	}
