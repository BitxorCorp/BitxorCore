...
#define TEST_CLASS NodeContainerTests
	...
	namespace {
		const model::NodeIdentity& GetFirstIdentity(const NodeContainerView& nodesView) {
			...
			if (!pNodeIdentity)
				BITXORCORE_THROW_RUNTIME_ERROR("could not find first node identity");
	...
	TEST(TEST_CLASS, NodeInfoIsInaccessibleForUnknownNode) {
		...
		// Act + Assert:
		EXPECT_THROW(container.view().getNodeInfo(ToIdentity(otherKey)), bitxorcore_invalid_argument);
	}
