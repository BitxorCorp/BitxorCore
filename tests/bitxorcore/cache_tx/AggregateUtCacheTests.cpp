	...
	namespace {
		...
		public:
			...
			utils::FileSize memorySize() const override {
				BITXORCORE_THROW_RUNTIME_ERROR("memorySize - not supported in mock");
			}

			bool add(const model::TransactionInfo&) override {
				BITXORCORE_THROW_RUNTIME_ERROR("add - not supported in mock");
			}

			model::TransactionInfo remove(const Hash256&) override {
				BITXORCORE_THROW_RUNTIME_ERROR("remove - not supported in mock");
			}

			utils::FileSize memorySizeForAccount(const Key&) const override {
				BITXORCORE_THROW_RUNTIME_ERROR("memorySizeForAccount - not supported in mock");
			}

			std::vector<model::TransactionInfo> removeAll() override {
				BITXORCORE_THROW_RUNTIME_ERROR("removeAll - not supported in mock");
			}
		};
