...
#include "bitxorcore/cache_tx/AccountWeights.h"
...
#define TEST_CLASS AccountWeightsTests
	...
	TEST(TEST_CLASS, CannotDecrementWeightBelowZero) {
		...
		// Act + Assert:
		EXPECT_THROW(weights.decrement(key, 10), bitxorcore_runtime_error);
		EXPECT_THROW(weights.decrement(key, 11), bitxorcore_runtime_error);
	}
