...
#define TEST_CLASS ProcessBootstrapperTests
	...
	namespace {
		ProcessBootstrapper CreateBootstrapper(const config::BitxorCoreConfiguration& config) {
		...
		void AssertCannotLoadUnknownPlugin(const std::string& directory) {
			...
			RunExtensionsTest(directory, "extension.awesome", [](auto& bootstrapper) {
				// Act + Assert:
				EXPECT_THROW(bootstrapper.loadExtensions(), bitxorcore_invalid_argument);
			});
		}
	}
	...
	namespace {
		...
		auto CreateBitxorCoreConfigurationWithCustomNetworkFingerprint() {
			test::MutableBitxorCoreConfiguration config;
	...
	TEST(TEST_CLASS, CanAddStaticNodes) {
		// Arrange:
		auto bootstrapper = CreateBootstrapper(CreateBitxorCoreConfigurationWithCustomNetworkFingerprint());
