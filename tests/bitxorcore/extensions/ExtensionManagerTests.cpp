...
#define TEST_CLASS ExtensionManagerTests
	...
	namespace {
		std::vector<std::string> GetDefaultSystemPluginNames() {
			return { "bitxorcore.plugins.coresystem", "bitxorcore.plugins.signature" };
		}
	}
	...
	TEST(TEST_CLASS, CannotSetNetworkTimeSupplierMultipleTimes) {
		...
		// Act + Assert:
		EXPECT_THROW(manager.setNetworkTimeSupplier([]() { return Timestamp(123); }), bitxorcore_invalid_argument);
	}
