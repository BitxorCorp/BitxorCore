...
#include "bitxorcore/extensions/BasicServerHooks.h"
...
#define TEST_CLASS BasicServerHooksTests
	...
	TEST(TEST_CLASS, SetOnceThrowsWhenDestIsSet) {
		...
		// Act + Assert:
		EXPECT_THROW(SetOnce(dest, source), bitxorcore_invalid_argument);
	...
	TEST(TEST_CLASS, RequireThrowsWhenFuncIsNotSet) {
		EXPECT_THROW(Require(BasicFunc()), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, CanAggregateZeroConsumers) {
		// Act:
		auto consumer = AggregateConsumers<bitxorcore::consumer<const int&>>({});
