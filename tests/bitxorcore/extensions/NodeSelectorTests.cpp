...
#include "bitxorcore/extensions/NodeSelector.h"
			...
			static KeyStatistics CreateStatistics(
				...
				for (auto i = 0u; i < numIterations; ++i) {
					...
					if (1u != selectedNodes.size())
						BITXORCORE_THROW_RUNTIME_ERROR_1("unexpected number of nodes were selected", selectedNodes.size());
		...
		std::pair<uint32_t, uint32_t> RunManyPairwiseSelections(const NodeInfos& nodeInfos) {
			...
			for (auto i = 0u; i < 1000; ++i) {
				...
				if (1 != result.AddCandidates.size())
					BITXORCORE_THROW_RUNTIME_ERROR("unexpected number of candidate nodes returned");
			...
			BITXORCORE_LOG(debug) << "selections (" << counts.first << ", " << counts.second << ")";
			return counts;
		}
			...
			test::RunNonDeterministicTest("pairwise node selection", [&]() {
				...
				for (auto i = 0u; i < 10; ++i) {
					...
					if (isSuccess(counts))
						...
						BITXORCORE_LOG(warning) << "failed pairwise node selection test due to unexpected ratio";
				}
