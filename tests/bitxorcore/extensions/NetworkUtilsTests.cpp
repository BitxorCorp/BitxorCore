...
#include "bitxorcore/extensions/Results.h"
#include "bitxorcore/net/ConnectionContainer.h"
#include "bitxorcore/net/PeerConnectResult.h"
...
#define TEST_CLASS NetworkUtilsTests
	...
	// region test utils (CreateBitxorCoreConfiguration)
	...
	namespace {
		bool RunVerifyCallback(const predicate<ionet::PacketSocketSslVerifyContext&>& verifyCallback) {
			...
			builder.setSubject("JP", "BXR", "Node");
			builder.setIssuer("JP", "BXR", "Root");
	...
	TEST(TEST_CLASS, CanExtractConnectionSettingsFromBitxorCoreConfiguration) {
		// Arrange:
	...
	TEST(TEST_CLASS, CanUpdateAsyncTcpServerSettingsFromBitxorCoreConfiguration) {
		// Arrange:
		auto config = CreateBitxorCoreConfiguration();
	...
	namespace {
		...
		public:
			size_t numActiveConnections() const override {
				BITXORCORE_THROW_RUNTIME_ERROR("numActiveConnections - not supported in mock");
			}

			model::NodeIdentitySet identities() const override {
				BITXORCORE_THROW_RUNTIME_ERROR("identities - not supported in mock");
			}
		...
		public:
			auto boot(const config::BitxorCoreConfiguration& config) {
				// Act:
			...
			auto boot() {
				return boot(CreateBitxorCoreConfiguration());
			}
	...
	namespace {
		void AssertBootServerConnectionAcceptedWhenAcceptSucceedsWithNotifySuccess(
			...
			auto key = test::GenerateRandomByteArray<Key>();
			BootServerContext context(net::PeerConnectCode::Accepted, key);
			auto pServer = context.boot(CreateBitxorCoreConfiguration(false, listenInterface));
	...
	namespace {
		...
		void RunRateLimitingTest(bool enableReadRateMonitoring, uint32_t bufferSize, TAction action) {
			...
			auto key = test::GenerateRandomByteArray<Key>();
			BootServerContext context(net::PeerConnectCode::Accepted, key);
			auto pServer = context.boot(CreateBitxorCoreConfiguration(enableReadRateMonitoring));
			...
			context.acceptor().socketInfos()[0].socket()->read([](auto code, const auto* pPacket) {
				BITXORCORE_LOG(debug) << "read completed with code " << code << " (size = " << (pPacket ? pPacket->Size : 0) << ")";
			});
