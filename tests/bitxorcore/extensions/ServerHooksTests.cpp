...
#define FACTORY_RETRIEVER_TEST(TEST_NAME) \
	...
	FACTORY_RETRIEVER_TEST(CannotAccessWhenUnset) {
		...
		// Act + Assert:
		EXPECT_THROW(TTraits::Get(hooks), bitxorcore_invalid_argument);
	}
	...
	FACTORY_RETRIEVER_TEST(CannotSetMultipleTimes) {
		...
		// Act + Assert:
		EXPECT_THROW(TTraits::Set(hooks, [](auto) { return TTraits::CreateResult(); }), bitxorcore_invalid_argument);
	}
...
#define SUPPLIER_TEST_WITH_DEFAULT(TEST_NAME) \
	...
	SUPPLIER_TEST(CannotSetMultipleTimes) {
		...
		// Act + Assert:
		EXPECT_THROW(TTraits::Set(hooks, []() { return TTraits::Custom_Value; }), bitxorcore_invalid_argument);
	}
