...
#include "bitxorcore/cache_db/RocksDatabase.h"
#include "bitxorcore/io/FileLock.h"
#include "bitxorcore/io/RawFile.h"
#include "tests/bitxorcore/cache_db/test/RdbTestUtils.h"
#include "tests/bitxorcore/cache_db/test/SliceTestUtils.h"
...
#define TEST_CLASS RocksDatabaseTests
	...
	TEST(TEST_CLASS, RdbThrowsWhenNoColumnsAreGiven) {
		EXPECT_THROW(RocksDatabase(CreateSettings({})), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, RdbThrowsWhenDbCannotBeOpened) {
		...
		// Act + Assert:
		EXPECT_THROW(RocksDatabase(CreateSettings({ "default" })), bitxorcore_runtime_error);
	}
	...
	TEST(TEST_CLASS, DefaultCreatedRdbDoesNotAllowGet) {
		...
		// Act + Assert:
		RdbDataIterator iter;
		EXPECT_THROW(database.get(0, "hello", iter), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, DefaultCreatedRdbDoesNotAllowPut) {
		...
		// Act + Assert:
		EXPECT_THROW(database.put(0, "hello", "amazing"), bitxorcore_invalid_argument);
	}
	...
	TEST(TEST_CLASS, DefaultCreatedRdbDoesNotAllowDelete) {
		...
		// Act + Assert:
		EXPECT_THROW(database.del(0, "hello"), bitxorcore_invalid_argument);
	}
