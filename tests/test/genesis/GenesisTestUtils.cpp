...
#include "GenesisTestUtils.h"
#include "plugins/txes/token/src/cache/TokenCache.h"
...
namespace bitxorcore { namespace test {
	...
	namespace {
		void AssertGenesisAccount(const cache::AccountStateCacheView& view) {
			auto genesisKeyPair = crypto::KeyPair::FromString(Test_Network_Genesis_Private_Key);
			auto address = model::PublicKeyToAddress(genesisKeyPair.publicKey(), Network_Identifier);
			...
			EXPECT_EQ(genesisKeyPair.publicKey(), accountState.PublicKey);

			EXPECT_EQ(Amount(0), accountState.Balances.get(Default_Currency_Token_Id));
			EXPECT_EQ(Amount(0), accountState.Balances.get(Default_Harvesting_Token_Id));
		...
		void AssertRentalFeeAccount(const cache::AccountStateCacheView& view, const Address& address) {
			...
			EXPECT_EQ(Amount(0), accountState.Balances.get(Default_Currency_Token_Id)) << message;
			EXPECT_EQ(Amount(0), accountState.Balances.get(Default_Harvesting_Token_Id)) << message;
		...
		void AssertRecipientAccount(const cache::AccountStateCacheView& view, const Key& publicKey, size_t index) {
			...
			auto expectedImportance = GetGenesisImportance(publicKey);
			EXPECT_EQ(Amount(409'090'909'000'000), accountState.Balances.get(Default_Currency_Token_Id)) << message;
			EXPECT_EQ(Amount(expectedImportance.unwrap() * 1000), accountState.Balances.get(Default_Harvesting_Token_Id)) << message;
	...
	void AssertGenesisAccountState(const cache::BitxorCoreCacheView& view) {
		AssertGenesisState(view.sub<cache::AccountStateCache>());
	}

	namespace {
		void AssertGenesisState(const cache::TokenCacheView& view) {
			// Assert:
			EXPECT_EQ(2u, view.size());

			// - check for known tokens
			ASSERT_TRUE(view.contains(Default_Currency_Token_Id));
			EXPECT_EQ(Amount(8'999'999'998'000'000), view.find(Default_Currency_Token_Id).get().supply());

			ASSERT_TRUE(view.contains(Default_Harvesting_Token_Id));
			EXPECT_EQ(Amount(17'000'000), view.find(Default_Harvesting_Token_Id).get().supply());
		}
	}

	void AssertGenesisTokenState(const cache::BitxorCoreCacheView& view) {
		AssertGenesisState(view.sub<cache::TokenCache>());
	}

	namespace {
		void AssertGenesisState(const cache::NamespaceCacheView& view) {
			// Assert:
	...
	void AssertGenesisNamespaceState(const cache::BitxorCoreCacheView& view) {
		AssertGenesisState(view.sub<cache::NamespaceCache>());
	}
}}
