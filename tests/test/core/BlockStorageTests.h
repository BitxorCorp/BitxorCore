...
namespace bitxorcore { namespace test {
	...
	struct LoadBlockTraits {
		...
		static void AssertLoadError(const io::BlockStorage& storage, Height height) {
			// Act + Assert:
			EXPECT_THROW(Load(storage, height), bitxorcore_invalid_argument);
		}
	};
	...
	public:
		...
		static void AssertCanSaveBlockWithOnlyTokenResolutions() {
	...
	private:
		static void AssertCannotSaveBlockAtHeight(size_t numSeedBlocks, Height newBlockHeight) {
			...
			EXPECT_THROW(pStorage->saveBlock(BlockToBlockElementWithStatements(*pNewBlock)), bitxorcore_invalid_argument);
		}
	...
	public:
		...
		static void AssertCanDropBlocksAfterHeight() {
			...
			EXPECT_THROW(pStorage->loadBlockElement(Height(9)), bitxorcore_invalid_argument);
			EXPECT_THROW(pStorage->loadBlockElement(Height(10)), bitxorcore_invalid_argument);
		}
		...
		static void AssertCanDropAllBlocks() {
			...
			EXPECT_THROW(pStorage->loadBlockElement(Height(1)), bitxorcore_invalid_argument);
		}

		// endregion

		// region loadBlockElement - genesis

		static void AssertStorageSeedInitiallyContainsGenesisBlock() {
			// Arrange:
			constexpr auto Source_Directory = "../seed/testnet";

			auto genesisBlockElement = BlockToBlockElement(GetGenesisBlock(), GetGenesisGenerationHashSeed());
			...
			AssertEqual(genesisBlockElement, *pBlockElement);
		...
		static void AssertPurgeDestroysStorage() {
			...
			EXPECT_THROW(pStorage->loadBlock(Height(1)), bitxorcore_invalid_argument);
			EXPECT_THROW(pStorage->loadBlockElement(Height(1)), bitxorcore_invalid_argument);
		}
...
#define DEFINE_BLOCK_STORAGE_TESTS(TRAITS_NAME) \
	...
	MAKE_BLOCK_STORAGE_TEST(TRAITS_NAME, CanSaveBlockWithOnlyTokenResolutions) \
	...
	\
	MAKE_BLOCK_STORAGE_TEST(TRAITS_NAME, StorageSeedInitiallyContainsGenesisBlock) \
	\
