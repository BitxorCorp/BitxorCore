...
namespace bitxorcore { namespace test {
	...
	/// Creates a resolver context that resolves unresolved token ids by doubling the ids.
	model::ResolverContext CreateResolverContextWithCustomDoublingTokenResolver();

	/// Creates a resolver context that resolves unresolved token ids and addresses by XORing.
	model::ResolverContext CreateResolverContextXor();

	/// Unresolves \a tokenId by XORing it.
