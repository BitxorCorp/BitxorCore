...
namespace bitxorcore { namespace mocks {
	...
	std::unique_ptr<MockTransaction> CreateTransactionWithFeeAndTransfers(Amount fee, const std::vector<UnresolvedToken>& transfers) {
		auto pTransaction = CreateMockTransaction(static_cast<uint16_t>(transfers.size() * sizeof(Token)));
		pTransaction->MaxFee = fee;
		pTransaction->Version = 0;

		auto* pTransferData = pTransaction->DataPtr();
		for (const auto& transfer : transfers) {
			std::memcpy(pTransferData, &transfer, sizeof(UnresolvedToken));
			pTransferData += sizeof(UnresolvedToken);
		}
	...
	namespace {
		...
		void Publish(
			...
			auto pTokens = reinterpret_cast<const UnresolvedToken*>(mockTransaction.DataPtr());
			for (auto i = 0u; i < mockTransaction.Data.Size / sizeof(UnresolvedToken); ++i) {
				...
				sub.notify(BalanceTransferNotification(context.SignerAddress, recipient, pTokens[i].TokenId, pTokens[i].Amount));
			}
		}
		...
		public:
			...
			const EmbeddedTransactionPlugin& embeddedPlugin() const override {
				if (!m_pEmbeddedTransactionPlugin)
					BITXORCORE_THROW_RUNTIME_ERROR("mock transaction is not embeddable");
