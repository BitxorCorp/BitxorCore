...
namespace bitxorcore { namespace mocks {
	...
	private:
		...
		void addTransfer(const model::BalanceTransferNotification& notification) {
			BITXORCORE_LOG(debug) << "visited " << notification.TokenId << " transfer of " << notification.Amount << " units";
	...
	public:
		...
		/// Returns \c true if a transfer of \a amount units of \a tokenId from \a sender to \a recipient was visited.
		bool contains(const Address& sender, const UnresolvedAddress& recipient, UnresolvedTokenId tokenId, Amount amount) const {
			auto targetTransfer = Transfer(sender, recipient, tokenId, amount);
			return std::any_of(m_transfers.cbegin(), m_transfers.cend(), [&targetTransfer](const auto& transfer) {
				return targetTransfer.Sender == transfer.Sender
						...
						&& targetTransfer.TokenId == transfer.TokenId
	...
	private:
		...
		public:
			...
			Transfer(const Address& sender, const UnresolvedAddress& recipient, UnresolvedTokenId tokenId, Amount amount)
					: Sender(sender)
					, Recipient(recipient)
					, TokenId(tokenId)
					, Amount(amount)
