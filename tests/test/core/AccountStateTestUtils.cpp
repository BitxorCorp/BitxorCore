...
namespace bitxorcore { namespace test {
	...
	void RandomFillAccountData(uint64_t seed, state::AccountState& accountState, size_t numTokens) {
		for (auto i = 0u; i < numTokens; ++i)
			accountState.Balances.credit(TokenId(10 + i + ((0 == i % 2) ? 100 : 0)), Amount(seed * 1000 + i + 1));
	...
	void AssertEqual(const state::AccountState& expected, const state::AccountState& actual, const std::string& message) {
		...
		EXPECT_EQ(expected.Balances.optimizedTokenId(), actual.Balances.optimizedTokenId()) << message;
		EXPECT_EQ(expected.Balances.size(), actual.Balances.size()) << message;
		for (const auto& pair : expected.Balances)
			EXPECT_EQ(pair.second, actual.Balances.get(pair.first)) << message << ": for token " << pair.first;
	}
	...
	std::shared_ptr<state::AccountState> CreateAccountStateWithoutPublicKey(uint64_t height) {
		...
		pState->Balances.credit(TokenId(1111), Amount(1));
