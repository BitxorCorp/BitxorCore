...
namespace bitxorcore { namespace test {
	...
	void AssertCannotLoadConfigurationFromBagWithAnyMissingProperty() {
		...
		for (const auto& sectionPair : TTraits::CreateProperties()) {
			...
			if (TTraits::IsSectionOptional(section)) {
				BITXORCORE_LOG(debug) << "skipping optional section " << section;
				continue;
			}

			for (const auto& namePair : sectionPair.second) {
				const auto& name = namePair.first;
				BITXORCORE_LOG(debug) << "attempting to load configuration without " << section << "::" << name;
	...
	void AssertCannotLoadConfigurationFromBagWithAnyUnknownProperty() {
		...
		for (const auto& sectionPair : TTraits::CreateProperties()) {
			...
			BITXORCORE_LOG(debug) << "attempting to load configuration with extra property in " << section;
			...
			// Act + Assert: the load failed
			EXPECT_THROW(TTraits::ConfigurationType::LoadFromBag(std::move(propertiesCopy)), bitxorcore_invalid_argument);
		}
	}
	...
	void AssertCannotLoadConfigurationFromBagWithAnyUnknownSection() {
		...
		// Act + Assert:
		EXPECT_THROW(TTraits::ConfigurationType::LoadFromBag(bag), bitxorcore_invalid_argument);
	}
