...
namespace bitxorcore { namespace test {
	...
	public:
		...
		void incrementCounterAndBlock(TCounter& counter, size_t id) {
			BITXORCORE_LOG(trace) << "entered lock " << id;
			++counter;
			++NumValueChanges;
			block();
			BITXORCORE_LOG(trace) << "exiting lock " << id;
		}
		...
		void setValueAndBlock(TValue& value, TValue desired) {
			BITXORCORE_LOG(trace) << "entered lock " << desired;
			value = desired;
			++NumValueChanges;
			block();
			BITXORCORE_LOG(trace) << "exiting lock " << desired;
		}
		...
		void waitForValueChangeWithPause() {
			...
			BITXORCORE_LOG(debug) << "waiting for increment";
			WAIT_FOR_EXPR(NumValueChanges > 0);
			BITXORCORE_LOG(debug) << "pausing";
			Pause();
			BITXORCORE_LOG(debug) << "done pausing";
		}
	};
	...
	void AssertLockGuaranteesExclusiveAccessAfterLockUnlockCycles(TLock& lock) {
		...
		for (auto i = 0u; i < Num_Default_Lock_Threads; ++i) {
			threads.spawn([&, i] {
				...
				BITXORCORE_LOG(debug) << "entered lock " << i;
				++counter;
				BITXORCORE_LOG(debug) << "exiting lock " << i;
			});
		}
