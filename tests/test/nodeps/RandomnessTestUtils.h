...
namespace bitxorcore { namespace test {
	...
	private:
		static double CalculateChiSquare(const std::array<uint64_t, Num_Buckets>& buckets, uint64_t expectedValue) {
			...
			BITXORCORE_LOG(debug) << "chiSquare = " << chiSquare << ", min = " << minValue << ", max = " << maxValue;
			return chiSquare;
		}
		...
		static double LookupProbability(double chiSquare) {
			...
			for (const auto& pair : chiSquareTable) {
				if (chiSquare > pair.first) {
					...
					BITXORCORE_LOG(debug) << "randomness hypothesis can be rejected with at least " << probability << " percent certainty";
					return probability;
				}
			}
