...
namespace bitxorcore { namespace test {
	...
	namespace {
		...
		std::filesystem::path PathFromDirectoryName(const std::string& directoryName) {
			if (std::string::npos != directoryName.find("/"))
				BITXORCORE_THROW_INVALID_ARGUMENT_1("TempDirectoryGuard only supports directory names", directoryName);
		...
		void DeleteDirectoryIfEmpty(const std::string& directoryName) {
			...
			BITXORCORE_LOG(debug) << "deleting empty directory " << directoryName;
	...
	TempDirectoryGuard::TempDirectoryGuard(const std::filesystem::path& directoryPath, bool) : m_directoryPath(directoryPath) {
		if (!StartsWithTempDirectoryRoot(directoryPath.generic_string()))
			BITXORCORE_THROW_INVALID_ARGUMENT_1("temp directory does not start with temp directory root", directoryPath.generic_string());

		if (exists())
			BITXORCORE_LOG(warning) << "creating directory " << m_directoryPath << " that already exists!";
		else
			BITXORCORE_LOG(debug) << "creating directory " << m_directoryPath;
	...
	namespace {
		bool TryFindPluginsDirectory(const std::string& directory, bool recurse, std::string& pluginsDirectory) {
			...
			for (auto iter = directory_iterator(directory); directory_iterator() != iter; ++iter) {
				if (std::string::npos != iter->path().generic_string().find("bitxorcore.plugins")) {
	...
	std::string GetExplicitPluginsDirectory() {
		...
		for (auto i = 0; i < global_argc; ++i) {
			...
			if (TryFindPluginsDirectory(global_argv[i], true, pluginsDirectory)) {
				BITXORCORE_LOG(debug) << "selecting '" << pluginsDirectory << "' as explicit directory";
				return pluginsDirectory;
			}
		}
#endif

		for (const auto& directory : { "lib", "." }) {
			std::string pluginsDirectory;
		...
		BITXORCORE_THROW_RUNTIME_ERROR("unable to find suitable explicit directory");
	}
