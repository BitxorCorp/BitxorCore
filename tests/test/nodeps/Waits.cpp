...
namespace bitxorcore { namespace test {
	...
	DeterministicTimeSpan RunDeterministicOperation(const action& operation) {
		...
		do {
			if (++i > Max_Attempts) {
				...
				BITXORCORE_THROW_RUNTIME_ERROR_1("RunDeterministicOperation timed out with duration (um)", duration.count());
			}
	...
	void RunNonDeterministicTest(const char* description, size_t numRetries, const predicate<size_t>& test) {
		...
		while (i < numRetries) {
			if (test(++i)) {
				BITXORCORE_LOG(debug) << description << " test was deterministic for iteration " << i;
				return;
			}

			BITXORCORE_LOG(warning) << description << " test was inconclusive, retrying ...";
		}

		auto message = std::string(description) + " test was inconclusive for all iterations";
		BITXORCORE_THROW_RUNTIME_ERROR_1(message.c_str(), numRetries);
	}
