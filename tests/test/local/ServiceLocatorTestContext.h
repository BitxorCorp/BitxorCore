...
namespace bitxorcore { namespace test {
	...
	public:
		...
		ServiceTestState() : ServiceTestState(cache::BitxorCoreCache({}))
		{}

		/// Creates the test state around \a cache.
		explicit ServiceTestState(cache::BitxorCoreCache&& cache) : ServiceTestState(std::move(cache), CreateDefaultNetworkTimeSupplier())
		{}

		/// Creates the test state around \a cache and \a timeSupplier.
		ServiceTestState(cache::BitxorCoreCache&& cache, const supplier<Timestamp>& timeSupplier)
				: m_config(CreatePrototypicalBitxorCoreConfiguration())
				, m_nodes(
						...
						timeSupplier,
						[](auto) { return true; })
				, m_bitxorcoreCache(std::move(cache))
	...
	private:
		static ionet::BanSettings GetBanSettings(const config::BitxorCoreConfiguration& config) {
	...
	public:
		...
		explicit ServiceLocatorTestContext(cache::BitxorCoreCache&& cache)
		...
		ServiceLocatorTestContext(cache::BitxorCoreCache&& cache, const supplier<Timestamp>& timeSupplier)
				: m_keys(test::GenerateRandomByteArray<Key>(), GenerateKeyPair())
				, m_pLocator(std::make_unique<extensions::ServiceLocator>(m_keys))
	...
	public:
		...
		uint64_t counter(const std::string& counterName) const {
			...
			BITXORCORE_THROW_INVALID_ARGUMENT_1("could not find counter with name", counterName);
		}
	...
	void RunTaskTestPostBoot(TTestContext& context, size_t numExpectedTasks, const std::string& taskName, TAction action) {
		...
		if (tasks.cend() == iter)
			BITXORCORE_THROW_RUNTIME_ERROR_1("unable to find task with name", taskName);
