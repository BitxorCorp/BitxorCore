...
#pragma once
#include "bitxorcore/api/ApiTypes.h"
...
namespace bitxorcore { namespace test {
	...
	private:
		static void AssertThrowsApiError(const action& action, const char* expectedMessage) {
			try {
				action();
				FAIL() << "Expected bitxorcore_api_error but no exception was thrown";
			} catch (const api::bitxorcore_api_error& ex) {
				EXPECT_STREQ(expectedMessage, ex.what());
			} catch (...) {
				FAIL() << "Expected bitxorcore_api_error but different exception was thrown";
			}
		}
