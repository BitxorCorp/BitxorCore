...
#include "bitxorcore/tree/TreeNode.h"
...
namespace bitxorcore { namespace test {
	...
	public:
		/// Gets a value.
		/// \throws bitxorcore_out_of_range if this iterator does not point to a valid value.
		const uint64_t& get() const {
			if (!m_isValid)
				BITXORCORE_THROW_OUT_OF_RANGE("invalid id supplied to get");
	...
	public:
		...
		void commit(const CacheDeltaType& delta) {
			if (m_pFlag) {
				BITXORCORE_LOG(debug) << "blocking in commit using auto set flag (" << m_pFlag << ")";
				m_pFlag->wait();
			}
	...
	public:
		...
		explicit SimpleCacheExtension(const AutoSetFlag& flag)
			...
			BITXORCORE_LOG(debug) << "created SimpleCache with auto set flag (" << &flag << ") with state " << flag.state()->isSet();
		}
	};
	...
	struct SimpleCacheExtensionStorageTraits {
		...
		static void LoadInto(uint64_t value, DestinationType& cacheDelta) {
			...
			if (value - 1 != cacheDelta.id())
				BITXORCORE_THROW_RUNTIME_ERROR_2("read value was unexpected (value, id)", value, cacheDelta.id());
		...
		static void Purge(uint64_t value, DestinationType& cacheDelta) {
			BITXORCORE_THROW_RUNTIME_ERROR_2("SimpleCacheExtensionStorage does not support Purge (value, id)", value, cacheDelta.id());
		}
	};
