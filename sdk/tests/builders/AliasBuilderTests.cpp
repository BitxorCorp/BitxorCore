...
#include "src/builders/TokenAliasBuilder.h"
...
#define TEST_CLASS AliasBuilderTests
	...
	namespace {
		...
		struct TokenAliasTestTraits {
			using AliasedType = TokenId;

			static constexpr auto Transaction_Type = model::Entity_Type_Alias_Token;

			template<typename TTransaction>
			static TokenId GetAliased(const TTransaction& transaction) {
				return transaction.TokenId;
			}

			template<typename TBuilder>
			static void SetAliased(TBuilder& builder, TokenId tokenId) {
		...
		public:
			...
			public:
				TransactionProperties(bitxorcore::NamespaceId namespaceId, model::AliasAction aliasAction)
		...
		using TokenAliasTraits = AliasTraits<TokenAliasBuilder, TokenAliasTestTraits>;
...
#define TRAITS_BASED_TEST(TEST_NAME) \
	...
	TEST(TEST_CLASS, TEST_NAME##_Token_Regular) { \
		TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<TokenAliasTraits, TokenAliasTraits::RegularTraits>(); \
	} \
	TEST(TEST_CLASS, TEST_NAME##_Token_Embedded) { \
		TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<TokenAliasTraits, TokenAliasTraits::EmbeddedTraits>(); \
	} \
	template<typename TAliasTraits, typename TTraits> void TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)()
