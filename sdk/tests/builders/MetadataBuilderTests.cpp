...
#include "src/builders/TokenMetadataBuilder.h"
...
#define TEST_CLASS MetadataBuilderTests
	...
	namespace {
		...
		struct TokenMetadataTraits {
			using TransactionTraits = TTraits;
			using BuilderType = TokenMetadataBuilder;

			static constexpr auto Entity_Type = model::Entity_Type_Token_Metadata;

			static void SetTargetId(BuilderType& builder, uint64_t rawTargetId) {
				builder.setTargetTokenId(UnresolvedTokenId(rawTargetId));
			}

			template<typename TTransaction>
			static void AssertAdditionalProperties(const TransactionProperties& expectedProperties, const TTransaction& transaction) {
				EXPECT_EQ(UnresolvedTokenId(expectedProperties.RawTargetId), transaction.TargetTokenId);
			}
		};
		...
		using TokenRegularTraits = TokenMetadataTraits<test::RegularTransactionTraits<model::TokenMetadataTransaction>>;
		using TokenEmbeddedTraits = TokenMetadataTraits<test::EmbeddedTransactionTraits<model::EmbeddedTokenMetadataTransaction>>;
...
#define TRAITS_BASED_TEST(TEST_NAME) \
	...
	TEST(TEST_CLASS, TEST_NAME##_Token_Regular) { TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<TokenRegularTraits>(); } \
	TEST(TEST_CLASS, TEST_NAME##_Token_Embedded) { TRAITS_TEST_NAME(TEST_CLASS, TEST_NAME)<TokenEmbeddedTraits>(); } \
