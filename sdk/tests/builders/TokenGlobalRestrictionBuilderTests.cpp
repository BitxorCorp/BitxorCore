...
#include "src/builders/TokenGlobalRestrictionBuilder.h"
#include "tests/builders/test/BuilderTestUtils.h"

namespace bitxorcore { namespace builders {

#define TEST_CLASS TokenGlobalRestrictionBuilderTests

	namespace {
		using RegularTraits = test::RegularTransactionTraits<model::TokenGlobalRestrictionTransaction>;
		using EmbeddedTraits = test::EmbeddedTransactionTraits<model::EmbeddedTokenGlobalRestrictionTransaction>;
		...
		public:
			...
			model::TokenRestrictionType PreviousRestrictionType;
			uint64_t NewRestrictionValue;
			model::TokenRestrictionType NewRestrictionType;
		};
		...
		void AssertTransactionProperties(const TransactionProperties& expectedProperties, const TTransaction& transaction) {
			...
			EXPECT_EQ(expectedProperties.ReferenceTokenId, transaction.ReferenceTokenId);
		...
		void AssertCanBuildTransaction(
				...
				const consumer<TokenGlobalRestrictionBuilder&>& buildTransaction) {
			// Arrange:
			auto networkIdentifier = static_cast<model::NetworkIdentifier>(0x62);
			auto signer = test::GenerateRandomByteArray<Key>();

			// Act:
			TokenGlobalRestrictionBuilder builder(networkIdentifier, signer);
			...
			EXPECT_EQ(model::Entity_Type_Token_Global_Restriction, pTransaction->Type);
...
#define TRAITS_BASED_TEST(TEST_NAME) \
	...
	TRAITS_BASED_TEST(CanSetReferenceTokenId) {
		// Arrange:
		auto expectedProperties = TransactionProperties();
		expectedProperties.ReferenceTokenId = UnresolvedTokenId(345);

		// Assert:
		AssertCanBuildTransaction<TTraits>(expectedProperties, [](auto& builder) {
			builder.setReferenceTokenId(UnresolvedTokenId(345));
		});
	}
	...
	TRAITS_BASED_TEST(CanSetOtherProperties) {
		...
		expectedProperties.PreviousRestrictionType = static_cast<model::TokenRestrictionType>(0x56);
		expectedProperties.NewRestrictionValue = 7890;
		expectedProperties.NewRestrictionType = static_cast<model::TokenRestrictionType>(0xAB);

		// Assert:
		AssertCanBuildTransaction<TTraits>(expectedProperties, [](auto& builder) {
			builder.setPreviousRestrictionValue(1234);
			builder.setPreviousRestrictionType(static_cast<model::TokenRestrictionType>(0x56));
			builder.setNewRestrictionValue(7890);
			builder.setNewRestrictionType(static_cast<model::TokenRestrictionType>(0xAB));
		});
	}
