...
#define TRAITS_BASED_TEST(TEST_NAME) \
	...
	TEST(TEST_CLASS, CannotSetEmptyName) {
		...
		RunBuilderTest([](auto& builder) {
			...
			// Act + Assert:
			EXPECT_THROW(builder.setName({ reinterpret_cast<const uint8_t*>(name.data()), name.size() }), bitxorcore_invalid_argument);
		});
	}
	...
	TEST(TEST_CLASS, CannotChangeName) {
		...
		RunBuilderTest([](auto& builder) {
			...
			// Act + Assert:
			EXPECT_THROW(builder.setName({ reinterpret_cast<const uint8_t*>(name.data()), name.size() }), bitxorcore_runtime_error);
		});
	}
