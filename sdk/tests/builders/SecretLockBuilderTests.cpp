...
#define TEST_CLASS SecretLockBuilderTests
	...
	namespace {
		...
		void AssertTransactionProperties(const TransactionProperties& expectedProperties, const TTransaction& transaction) {
			EXPECT_EQ(expectedProperties.Token.TokenId, transaction.Token.TokenId);
			EXPECT_EQ(expectedProperties.Token.Amount, transaction.Token.Amount);
...
#define TRAITS_BASED_TEST(TEST_NAME) \
	...
	TRAITS_BASED_TEST(CanSetToken) {
		// Arrange:
		auto expectedProperties = TransactionProperties();
		expectedProperties.Token = { UnresolvedTokenId(123), Amount(234) };

		// Assert:
		AssertCanBuildTransaction<TTraits>(expectedProperties, [](auto& builder) {
			builder.setToken({ UnresolvedTokenId(123), Amount(234) });
		});
	}
