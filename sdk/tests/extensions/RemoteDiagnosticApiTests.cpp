...
namespace bitxorcore { namespace extensions {
	...
	namespace {
		...
		private:
			static TIdentifier ToIdentifier(uint32_t value) {
				if constexpr (std::is_same_v<TokenId, TIdentifier> || std::is_same_v<NamespaceId, TIdentifier>)
		...
		// region TokenInfosTraits

		struct TokenInfosTraits : public InfosTraits<TokenId, ionet::PacketType::Token_Infos> {
			static constexpr auto Request_Entity_Size = sizeof(TokenId);

			static auto Invoke(const RemoteDiagnosticApi& api, RequestParamType&& param) {
				return api.tokenInfos(std::move(param));
			}
		};
	...
	using DiagnosticTokenInfosTraits = DiagnosticApiTraits<TokenInfosTraits>;
	...
	DEFINE_REMOTE_API_TESTS_EMPTY_RESPONSE_INVALID(RemoteDiagnosticApi, DiagnosticTokenInfos)
