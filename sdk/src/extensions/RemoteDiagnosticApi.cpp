...
namespace bitxorcore { namespace extensions {
	...
	namespace {
		...
		struct TokenInfosTraits : public InfosTraits<TokenId, ionet::PacketType::Token_Infos> {
			static constexpr auto Friendly_Name = "token infos";
		};
		...
		public:
			...
			FutureType<TokenInfosTraits> tokenInfos(model::EntityRange<TokenId>&& tokenIds) const override {
				return m_impl.dispatch(TokenInfosTraits(), std::move(tokenIds));
			}
