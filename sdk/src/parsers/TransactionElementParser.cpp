...
#include "bitxorcore/ionet/PacketPayloadParser.h"
...
namespace bitxorcore { namespace parsers {
	...
	bool TryParseTransactionElements(
		...
		if (packet.Size < Header_Size) {
			BITXORCORE_LOG(debug) << "packet does not contain full header (" << packet.Size << ", header size " << Header_Size << ")";
			return false;
		}

		auto hashesSize = reinterpret_cast<const uint32_t&>(*packet.Data());
		if (hashesSize > packet.Size - Header_Size) {
			BITXORCORE_LOG(debug) << "packet does not contain all reported hashes (" << packet.Size << ", hashes size " << hashesSize << ")";
			return false;
		}
		...
		if (0 == numHashes) {
			BITXORCORE_LOG(debug) << "packet data is invalid because no hashes were extracted (" << packet.Size << ")";
			return false;
		}
