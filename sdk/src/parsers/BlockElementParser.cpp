		...
		public:
			...
			const TEntity& readEntity() {
				...
				if (entity.Size < sizeof(TEntity))
					BITXORCORE_THROW_RUNTIME_ERROR("entity size must be at least entity header size");

				auto entityPayloadSize = entity.Size - sizeof(TEntity);
				if (m_buffer.Size - m_offset < entityPayloadSize)
					BITXORCORE_THROW_RUNTIME_ERROR("entity does not completely fit in buffer");
			...
			const TValue& read() {
				...
				if (m_offset > m_buffer.Size)
					BITXORCORE_THROW_RUNTIME_ERROR("value does not completely fit in buffer");
