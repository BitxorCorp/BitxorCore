...
set(BITXORCORE_BOOST_COMPONENTS atomic chrono date_time filesystem log log_setup program_options regex thread)

### set openssl definitions
add_definitions(-DOPENSSL_API_COMPAT=0x10100000L)

### set custom diagnostics
if(ENABLE_BITXORCORE_DIAGNOSTICS)
	add_definitions(-DENABLE_BITXORCORE_DIAGNOSTICS)
endif()

### forward docker build settings
if(BITXORCORE_TEST_DB_URL)
	add_definitions(-DBITXORCORE_TEST_DB_URL="${BITXORCORE_TEST_DB_URL}")
endif()
if(BITXORCORE_DOCKER_TESTS)
	add_definitions(-DBITXORCORE_DOCKER_TESTS)
endif()
...
elseif("${CMAKE_CXX_COMPILER_ID}" MATCHES "Clang")
	# - Wno-c++98-compat*: bitxorcore is not compatible with C++98
...
function(bitxorcore_add_tests_subdirectory DIRECTORY_NAME)
	if(ENABLE_TESTS)
...
function(bitxorcore_set_test_compiler_options)
...
# set BITXORCORE_VERSION_DESCRIPTION to a reasonable value
if(NOT BITXORCORE_BUILD_DEVELOPMENT)
	...
	if(BITXORCORE_BUILD_RELEASE_PUBLIC)
		set(BITXORCORE_VERSION_DESCRIPTION "(public)")
	else()
		set(BITXORCORE_VERSION_DESCRIPTION "${GIT_COMMIT_HASH} [${GIT_BRANCH}]")
	endif()
endif()
...
if(MSVC)
	function(set_win_version_definitions WIN_TARGET_NAME WIN_FILETYPE)
		...
		if(BITXORCORE_BUILD_RELEASE)
			add_definitions(-DBITXORCORE_BUILD_RELEASE=1)
		endif()
	endfunction()

	# embed the version rc file
	set(VERSION_RESOURCES ${CMAKE_SOURCE_DIR}/src/bitxorcore/version/win/win_version.rc)
else()
	# since strings are not referenced, in order for the linker to include them, they must be forcibly linked via an object file
	set(VERSION_RESOURCES $<TARGET_OBJECTS:bitxorcore.version.nix>)
endif()

### define target helper functions

# sets cxx std version
function(bitxorcore_set_cxx_std_version TARGET_NAME)
...
# used to define a bitxorcore target (library, executable) and automatically enables PCH for clang
function(bitxorcore_target TARGET_NAME)
	bitxorcore_set_cxx_std_version(${TARGET_NAME})
...
function(bitxorcore_find_all_target_files TARGET_TYPE TARGET_NAME)
...
# used to define a bitxorcore object library
function(bitxorcore_object_library TARGET_NAME)
...
# used to define a bitxorcore library, creating an appropriate source group and adding a library
function(bitxorcore_library TARGET_NAME)
	bitxorcore_find_all_target_files("lib" ${TARGET_NAME} ${ARGN})
	add_library(${TARGET_NAME} ${${TARGET_NAME}_FILES})
endfunction()

# combines bitxorcore_library and bitxorcore_target
function(bitxorcore_library_target TARGET_NAME)
	bitxorcore_library(${TARGET_NAME} ${ARGN})
	set_property(TARGET ${TARGET_NAME} PROPERTY POSITION_INDEPENDENT_CODE ON)
	bitxorcore_target(${TARGET_NAME})
endfunction()

# used to define a bitxorcore shared library, creating an appropriate source group and adding a library
function(bitxorcore_shared_library TARGET_NAME)
	bitxorcore_find_all_target_files("shared lib" ${TARGET_NAME} ${ARGN})
...
# combines bitxorcore_shared_library and bitxorcore_target
function(bitxorcore_shared_library_target TARGET_NAME)
	bitxorcore_shared_library(${TARGET_NAME} ${ARGN})
	bitxorcore_target(${TARGET_NAME})

	install(TARGETS ${TARGET_NAME})
endfunction()

# used to define a bitxorcore executable, creating an appropriate source group and adding an executable
function(bitxorcore_executable TARGET_NAME)
	bitxorcore_find_all_target_files("exe" ${TARGET_NAME} ${ARGN})
...
# used to define a bitxorcore header only target, creating an appropriate source group in order to allow VS to create an appropriate folder
function(bitxorcore_header_only_target TARGET_NAME)
	if(MSVC)
		bitxorcore_find_all_target_files("hdr" ${TARGET_NAME} ${ARGN})
...
# used to define a bitxorcore test executable
function(bitxorcore_test_executable TARGET_NAME)
...
# used to define a bitxorcore test executable for a bitxorcore library by combining bitxorcore_test_executable and
# bitxorcore_target and adding some library dependencies
function(bitxorcore_test_executable_target TARGET_NAME TEST_DEPENDENCY_NAME)
	...
	target_link_libraries(${TARGET_NAME} tests.bitxorcore.test.${TEST_DEPENDENCY_NAME} ${LIBRARY_UNDER_TEST} ${GTEST_LIBRARIES})
	bitxorcore_target(${TARGET_NAME})
endfunction()

# used to define a bitxorcore test executable for a header only bitxorcore library by combining bitxorcore_test_executable and
# bitxorcore_target and adding some library dependencies
function(bitxorcore_test_executable_target_header_only TARGET_NAME TEST_DEPENDENCY_NAME)
	bitxorcore_test_executable(${TARGET_NAME} ${ARGN})
	...
	target_link_libraries(${TARGET_NAME} tests.bitxorcore.test.${TEST_DEPENDENCY_NAME})
	bitxorcore_target(${TARGET_NAME})
endfunction()

# used to define a bitxorcore tool executable
function(bitxorcore_define_tool TOOL_NAME)
	set(TARGET_NAME bitxorcore.tools.${TOOL_NAME})

	bitxorcore_executable(${TARGET_NAME})
	target_link_libraries(${TARGET_NAME} bitxorcore.tools)
	bitxorcore_target(${TARGET_NAME})
